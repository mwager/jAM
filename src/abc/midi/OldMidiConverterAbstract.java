 ...
 */
public abstract class OldMidiConverterAbstract implements
			...
			while (i < staff.size()) {
				if (!inWrongEnding) {
					...
					if (staff.elementAt(i) instanceof abc.notation.Tempo)
						trackLengthInTicks = addEventsToTrack(
								track,
								getMidiEventsFor((Tempo) staff.elementAt(i),
										trackLengthInTicks));
					else
					// ====================================================================
					// KEY SIGNATURE
					...
					if (staff.elementAt(i) instanceof abc.notation.Tuplet) {
						...
						trackLengthInTicks = addEventsToTrack(
								track,
								getMidiEventsFor(tuplet, currentKey,
										trackLengthInTicks));
					...
					if (staff.elementAt(i) instanceof abc.notation.Note) {
						PositionableNote note = (PositionableNote) staff
								.elementAt(i);
						if (!note.isRest()) {
							MidiEvent[] array = { new MidiEvent(
									new NoteIndexMessage(i), trackLengthInTicks) };
							addEventsToTrack(track, array);
							trackLengthInTicks = addEventsToTrack(
									track,
									getMidiEventsFor(note, currentKey,
											trackLengthInTicks));
					...
					if ((staff.elementAt(i) instanceof abc.notation.MultiNote)) {
						PositionableMultiNote multiNote = (PositionableMultiNote) staff
								.elementAt(i);
						trackLengthInTicks = addEventsToTrack(
								track,
								getMidiEventsFor(multiNote, currentKey,
										trackLengthInTicks));
	...
	public abstract MidiEvent[] getMidiEventsFor(Note note, KeySignature key,
			long lastPosInTicks) throws InvalidMidiDataException;

	/** Returns the corresponding midi events for a tuplet. */
	public abstract MidiEvent[] getMidiEventsFor(Tuplet tuplet,
			KeySignature key, long lastPosInTicks)
			throws InvalidMidiDataException;

	/** Returns the corresponding midi events for a tempo change. */
	public abstract MidiEvent[] getMidiEventsFor(Tempo tempo,
			long lastPosInTicks) throws InvalidMidiDataException;

	/** Returns the corresponding midi events for a multi note. */
	public abstract MidiEvent[] getMidiEventsFor(MultiNote notes,
			KeySignature key, long lastPosInTicks)
			throws InvalidMidiDataException;
	 ...
	 */
	protected long getNoteLengthInTicks(Note note)// , DefaultNoteLength
													// defaultNoteLength)
	{
	 ...
	 */
	protected long getNoteLengthInTicks(MultiNote note)// , DefaultNoteLength
														// defaultNoteLength)
	{
	...
	protected byte getMidiNoteNumber(Note note, KeySignature key) {
		...
		Accidental accidental = new Accidental(note.getAccidental()
