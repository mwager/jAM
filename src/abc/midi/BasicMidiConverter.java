...
public class BasicMidiConverter extends MidiConverterAbstract {
	public MidiMessage[] getNoteOneMessageFor(Note note, KeySignature key)
			throws InvalidMidiDataException {
		MidiMessage[] events = new MidiMessage[1];
		ShortMessage myNoteOn = new ShortMessage();
		myNoteOn.setMessage(ShortMessage.NOTE_ON, getMidiNoteNumber(note, key),
				50);
		events[0] = myNoteOn;
		return events;
	}

	public MidiMessage[] getNoteOffMessageFor(Note note, KeySignature key)
			throws InvalidMidiDataException {
	...
	public MidiMessage[] getMidiMessagesFor(Tempo tempo)
			throws InvalidMidiDataException {
