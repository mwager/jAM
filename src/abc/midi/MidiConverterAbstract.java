...
/**
 * MidiConverter class defines various static methods to convert abc related
 * stuff to midi : notes, tunes etc...
 */
...
//			
//			System.out.println(instrument.getPatch()
//					.getProgram());
				...
				while (i < voice.size()) {
					if (!inWrongEnding) {
						...
						if (voice.elementAt(i) instanceof abc.notation.Tempo) {
							addTempoEventsFor(track, elapsedTime,
									getMidiMessagesFor((Tempo) voice
											.elementAt(i)));// ,
															// trackLengthInTicks));
						} else if (voice.elementAt(i) instanceof abc.notation.PartLabel) {
							...
							// new part appear, and restitute it when part is
							// played again
							abc.notation.PartLabel pl = (abc.notation.PartLabel) voice
									.elementAt(i);
							if (partsKey.get(pl.getLabel() + "") == null) {
								partsKey.put(pl.getLabel() + "", tuneKey);
							} else {
								tuneKey = (KeySignature) partsKey.get(pl
										.getLabel() + "");
							}
						} else
						// ====================================================================
						...
						if (voice.elementAt(i) instanceof abc.notation.KeySignature) {
							...
							currentKey = new KeySignature(
									tuneKey.getAccidentals());
						} else
						// ====================================================================
						// NOTE
						// Notes ending ties should be ignored. Already taken
						// into
						// account in getNoteLengthInTicks(Note)
						if (voice.elementAt(i) instanceof abc.notation.Note
								&& !((abc.notation.Note) voice.elementAt(i))
										.isEndingTie()) {
							...
							if (note.hasGeneralGracing()
								...
								for (int j = 0; j < d.length; j++) {
									...
									case Decoration.TURNX_INVERTED:
										...
										n.setAccidental(note
												.getAccidental(currentKey));
										Note o = new Interval(Interval.SECOND,
												Interval.MAJOR, Interval.UPWARD)
												.calculateSecondNote(n);
										Note m = new Interval(Interval.SECOND,
												Interval.MAJOR,
												Interval.DOWNWARD)
												.calculateSecondNote(n);
										// TODO ornament templates: regular,
										// musette, balkan...
							...
							if (note.hasGracingNotes()
									|| (decorationNotes.size() > 0)) {
								graceNotes = note.getGracingNotes();
								// gracing are eighth note for graphical
								// rendition
								...
								if (note.hasGracingNotes()) {
									for (int j = 0; j < graceNotes.length; j++) {
										noteDuration = getNoteLengthInTicks(
												graceNotes[j], staff) / divisor;
										graceNotesDuration += noteDuration;
										playNote(graceNotes[j], i, currentKey,
								...
								for (int j = 0; j < decorationNotes.size(); j++) {
									...
									playNote(
											(Note) decorationNotes.elementAt(j),
											i, currentKey, elapsedTime,
											noteDuration, track);
									elapsedTime += noteDuration;
								}
							}
							// The note duration if the note isn't part of a
							// tuplet.
							noteDuration = getNoteLengthInTicks(note, staff)
									- graceNotesDuration;
							if (noteDuration <= 0) // in case of too much grace
													// notes
								noteDuration = getNoteLengthInTicks(note, staff);
							if (fermata)
								noteDuration *= 2;
							playNote(note, i, currentKey, elapsedTime,
									noteDuration, track);
							elapsedTime += noteDuration;
						} else
						// ====================================================================
						...
						if ((voice.elementAt(i) instanceof abc.notation.MultiNote)) {
							MultiNote multiNote = (MultiNote) voice
									.elementAt(i);
							playMultiNote(multiNote, i, currentKey,
									elapsedTime, track, staff);
					...
					if (voice.elementAt(i) instanceof abc.notation.RepeatBarLine) {
						...
						if (repeatNumber < bar.getRepeatNumbers()[0]
							...
							repeatNumber++;
							i = lastRepeatOpen;
						} else if (repeatNumber > bar.getRepeatNumbers()[0])
							inWrongEnding = true;
						else
							inWrongEnding = false;
					} else
					...
					if (voice.elementAt(i) instanceof abc.notation.BarLine) {
						// currentKey = new
						// KeySignature(tuneKey.getAccidentals());
	 ...
	 * 
	 * @return The instrument currently used for sequence playback. Returns
	...
	/**
	 * Sets the instrument to be used for sequence playback. This implicitly
	 * loads the given instrument.
	 * 
	 * @param instr
	 *            The instrument to be used for sequence playback.
	 */
	 ...
	 * @param indexInScore
	 *            The index of the note in the score.
	 * @param currentKey
	 *            The current key this note is referring to.
	 * @param timeReference
	 *            The time reference expressed in ticks when the note should be
	 *            played.
	 * @param duration
	 *            The duration of the note expressed in ticks.
	 * @param track
	 *            The track where the note should be played.
	 * @throws InvalidMidiDataException
	 */
	protected void playNote(Note note, int indexInScore,
			KeySignature currentKey, long timeReference, long duration,
			Track track) throws InvalidMidiDataException {
		if (!note.isRest() && !note.isEndingTie()) {
			addNoteOnEventsFor(track, timeReference,
					getNoteOneMessageFor(note, currentKey));
			addNoteOffEventsFor(track, timeReference + duration,
					getNoteOffMessageFor(note, currentKey));
			// In case the note to be played had an accidental, the current key
			// needs
			// to be updated.
	...
	protected void playMultiNote(MultiNote multiNote, int indexInScore,
			KeySignature currentKey, long reference, Track track, Music staff)
			throws InvalidMidiDataException {
		Vector notesVector = multiNote.getNotesAsVector();
		for (int j = 0; j < notesVector.size(); j++) {
			Note note = (Note) (notesVector.elementAt(j));
			if (!note.isRest() && !note.isEndingTie())
				addNoteOnEventsFor(track, reference,
						getNoteOneMessageFor(note, currentKey));
		}
		for (int j = 0; j < notesVector.size(); j++) {
			Note note = (Note) (notesVector.elementAt(j));
			// TODO needs to be improved to take into account multi notes with
			// different notes length
			long noteDuration = getNoteLengthInTicks(multiNote, staff);
			if (!note.isRest() && !note.isEndingTie())
				addNoteOffEventsFor(track, reference + noteDuration,
						getNoteOffMessageFor(note, currentKey));
		}
		for (int j = 0; j < notesVector.size(); j++)
	...
	private static void addNoteOnEventsFor(Track track, long timeReference,
	...
	private void addTempoEventsFor(Track track, long timeReference,
			MidiMessage[] messages) {
		for (int i = 0; i < messages.length; i++) {
			MidiEvent me = new MidiEvent(messages[i], timeReference);
			// MidiEvent[] events = {me, new MidiEvent(new TempoMessageWA(),
			// timeReference)};
			addEventsToTrack(track, me);
		}
	}

	private void addNoteOffEventsFor(Track track, long timeReference,
			MidiMessage[] messages) {
		MidiMessage myNoteOn = messages[0];
		MidiEvent[] events = new MidiEvent[1];
		events[0] = new MidiEvent(myNoteOn, timeReference);
	...
	/**
	 * @return The length of the track in ticks, once events have been added to
	 *         it.
	 */
	...
	public abstract MidiMessage[] getNoteOneMessageFor(Note note,
			KeySignature key) throws InvalidMidiDataException;

	/** */
	public abstract MidiMessage[] getNoteOffMessageFor(Note note,
			KeySignature key) throws InvalidMidiDataException;

	/** Returns the corresponding midi events for a tempo change. */
	public abstract MidiMessage[] getMidiMessagesFor(Tempo tempo)
			throws InvalidMidiDataException;
	...
	protected static long getNoteLengthInTicks(Note note, Music staff) {
		...
		if (note.isBeginningTie() && note.getTieDefinition().getEnd() != null) {
			noteLength += ((Note) staff.getElementByReference(note
					.getTieDefinition().getEnd())).getDuration();
		}
		float numberOfQuarterNotesInThisNote = (float) noteLength
				/ Note.QUARTER;
		float lengthInTicks = (float) SEQUENCE_RESOLUTION
				* numberOfQuarterNotesInThisNote;
		return (long) lengthInTicks;
	}

	/**
	 * Returns the length of the multi note in ticks. This length is calculated
	 * from the resolution of the midi sequence manipulated internally.
	 * 
	 * @return The length of the multi note in ticks : this is equal to the
	 *         length of the longest note of the multi note.
	 */
	public static long getNoteLengthInTicks(MultiNote note, Music staff) {
		...
		/*
		 * float numberOfQuarterNotesInThisNote = (float)longestLength /
		 * Note.QUARTER; float lengthInTicks = (float)SEQUENCE_RESOLUTION *
		 * numberOfQuarterNotesInThisNote; return (long)lengthInTicks;
		 */
	}
	 ...
	 *            The note.
	 * @param key
	 *            The key this note should refer to for accidentals.
	...
	public static byte getMidiNoteNumber(Note note, KeySignature key) {
		...
		Accidental accidental = new Accidental(note.getAccidental(key)
		...
		midiNoteNumber += (byte) accidental.getValue(); // -2 dbl flat, -1 flat,
														// 0 natural...
