...
public class TunePlayer implements MetaEventListener {
	...
	private String m_tuneAsString = null;
	 ...
	 * 
	 * @param listener
	 *            The listener to be added to this tune player.
	 */
	 ...
	 * 
	 * @param listener
	 *            The listener to be removed from this tune player.
	 */
	 ...
	 * 
	 * @param tempo
	 *            the tempo used to play the tune.
	 */
	...
	/*
	 * public int getVolume() { try { MidiChannel[] channels
	 * =((Synthesizer)seq).getChannels(); for (int i=0; i<channels.length; i++)
	 * /
	 * /System.out.println(channels[i].getController(VOLUME_CONTROLLER_NUMBER));
	 * return
	 * ((Synthesizer)seq).getChannels()[0].getController(VOLUME_CONTROLLER_NUMBER
	 * ); } catch (Exception e) { e.printStackTrace(); return -1; } }
	 * 
	 * public void setVolume(int volume) { try { MidiChannel[] channels
	 * =((Synthesizer)seq).getChannels(); for (int i=0; i<channels.length; i++)
	 * channels[i].controlChange(VOLUME_CONTROLLER_NUMBER, volume); } catch
	 ...
	 *                Thrown if the player hasn't been started before.
	 */
	private Voice voice; //jAM
	public void play(Tune tune, String tuneAsString) throws IllegalStateException {
		if (m_isStarted) {
			try {
				m_tune = tune;
				m_tuneAsString=tuneAsString;
				
				//jAM : search for offset of first note in charstream:
//				NoteAbstract note=null;
//				for (int i = 0; i < 128; i++) {
//					note = (NoteAbstract)m_tune.getMusic().getElementAtStreamPosition(i);
//					if(note!=null) {
//						offset = i;
//						break;
//					}
//				}System.out.println();
				...
				//jAM
				Iterator it = m_tune.getMusic().getVoices().iterator();
				if(it.hasNext()) {
					voice = (Voice) it.next();
					voice=(Voice)voice.clone();
				}
				//just print ALL notes:
//				int i=0;
//				String s=("All Notes: "); 
//				while(i<voice.size()) {
//					if (voice.elementAt(i) instanceof abc.notation.Note) {
//						s+= (((Note)voice.elementAt(i)).getName() + ",");
//					}
//					i++;
//				}System.out.println(s);
	 ...
	 * 
	 * @return <TT>true</TT> if this player is currently playing a tune,
	...
	public void meta(MetaMessage meta) {
		...
		// Work around part with generic meta message work around
			...
			// System.out.println("detected tempo change : " +
			// expectdNotesNbPerMn +
		...
		} else if (meta.getType() == MidiMessageType.NOTE_INDEX_MARKER) {
			//FIXME notifyNotePlayedChange, use MusicReference? what is
			// NoteIndexMessage.getIndex
			...
			while(i<voice.size()) {
				if (voice.elementAt(i) instanceof abc.notation.Note) {
					//take first note, remove it and break
	                   note = (Note) voice.elementAt(i);
	                   voice.remove(i);
//	                   System.out.print(i + ":" + note.getName() + ",");
//	                   System.out.println(note.isBeginningTie());
//	                   System.out.println(note.isEndingTie());
	                   //buggy with lot of ties! TODO

	                   if(note.isBeginningTie()) { //no meta message for a note with endingTie so remove it!
//	                	   was ist an voice(i) ? erst wenn eine Note: loeschen !
	                	   while( !(voice.elementAt(i) instanceof abc.notation.Note) )
	                		   voice.remove(i);
		...
		} else if (meta.getType() == MidiMessageType.END_OF_TRACK) {
			notifyForPlayEnd();
		}
	}
	
	/**
	 * Starts the player so that it can play tunes. (Retrieves the default
	 * sequencer during this phasis).
	 */
	public void start() {
		try {
	...
	/*
	 * private ShortMessage createProgramMessage(int num) { ShortMessage
	 * programMessage = new ShortMessage();
	 * 
	 * try { Instrument myInstrument =
	 * MidiSystem.getSynthesizer().getDefaultSoundbank().getInstruments()[num];
	 * 
	 * programMessage.setMessage(ShortMessage.PROGRAM_CHANGE,
	 * myInstrument.getPatch().getProgram(),10); } catch
	 * (InvalidMidiDataException e) { e.printStackTrace(); }
	 * 
	...
	protected void notifyForTempoChange(int newTempoValue) {
		...
		for (int i = 0; i < m_listeners.size(); i++)
			((TunePlayerListenerInterface) m_listeners.elementAt(i))
					.tempoChanged(event);
	}
