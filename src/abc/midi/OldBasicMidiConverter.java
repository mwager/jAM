...
public class OldBasicMidiConverter extends OldMidiConverterAbstract {
	public MidiEvent[] getMidiEventsFor(Note note, KeySignature key,
			long elapsedTime) throws InvalidMidiDataException {
		...
		if (!note.isRest()) {
			...
			myNoteOn.setMessage(ShortMessage.NOTE_ON,
					getMidiNoteNumber(note, key), 50);
			events[0] = new MidiEvent(myNoteOn, elapsedTime);
			// events[1] = new MidiEvent(new
			// NotationMarkerMessage((PositionableNote)note), elapsedTime);
			ShortMessage myNoteOff = new ShortMessage();
			myNoteOff.setMessage(ShortMessage.NOTE_OFF,
					getMidiNoteNumber(note, key), 50);
	...
	public MidiEvent[] getMidiEventsFor(Tuplet tuplet, KeySignature key,
			long elapsedTime) throws InvalidMidiDataException {
		...
		for (int j = 0; j < tupletAsVector.size(); j++) {
			...
			// else
			// note =
			// (Note)((MultiNote)tupletAsVector.elementAt(j)).getNotesAsVector().elementAt(0);
			...
			if (!note.isRest()) {
				...
				events[3 * j + 1] = new MidiEvent(new NotationMarkerMessage(
						(PositionableNote) note), elapsedTime);
			...
			/*
			 * else if (tupletAsVector.elementAt(j) instanceof MultiNote) {
			 * events = getMidiEventsFor((MultiNote)tupletAsVector.elementAt(j),
			 * key, elapsedTime); long noteLength =
			 * events[1+(events.length-1)/2].getTick()-events[0].getTick();
			 * noteLength = (long)(noteLength * totalTupletLength /
			 * tuplet.countNotes()); for (int i=0; i<(events.length-1)/2-1; i++)
	...
	public MidiEvent[] getMidiEventsFor(Tempo tempo, long lastPosInTicks)
			throws InvalidMidiDataException {
		TempoMessage mt = new TempoMessage(tempo);
		MidiEvent me = new MidiEvent(mt, lastPosInTicks);
		MidiEvent[] events = null;// {me, new MidiEvent(new TempoMessageWA(),
									// lastPosInTicks)};
		return events;
	}

	/** Returns the corresponding midi events for a multi note. */
	public MidiEvent[] getMidiEventsFor(MultiNote notes, KeySignature key,
			long elapsedTime) throws InvalidMidiDataException {
		...
		events[notesVector.size()] = new MidiEvent(new NotationMarkerMessage(
				(PositionableMultiNote) notes), elapsedTime);
		...
		for (int j = 0; j < notesVector.size(); j++) {
			...
			if (!note.isRest()) {
				...
				myNoteOff.setMessage(ShortMessage.NOTE_OFF,
						getMidiNoteNumber(note, key), 50);
				events[notesVector.size() + j + 1] = new MidiEvent(myNoteOff,
						elapsedTime + noteLength);
			}
		}
		return events;
