 ...
 */
public class SonoraFont extends MusicalFontAbstract implements MusicalFont,
		Serializable {

	private static final long serialVersionUID = 659612792568835733L;

	/** Digits for time signature */
	private static final char[] DIGITS = { '\uF030', // 0
			'\uF031', // 1
			'\uF032', // 2
			'\uF033', // 3
			'\uF034', // 4
	...
	private static final char[] FINGERINGS_DIGITS = { '\uF102', // 0
			'\uF103', // 1
			'\uF104', // 2
			'\uF105', // 3
			'\uF106', // 4
	...
	private static final char[] ITALICS_DIGITS = { '\uF0BC', // 0
			'\uF0C1', // 1
			'\uF0AA', // 2
			'\uF0A3', // 3
			'\uF0A2', // 4
	...
	public Font getFont() throws Exception {
		if (font == null) {
			font = Font.createFont(Font.TRUETYPE_FONT, getClass()
					.getResourceAsStream("SONORA3.TTF"));
		}
		return font;
	}

	public char getAccidental(Accidental acc) throws MissingGlyphException,
		...
		if (acc.isDoubleFlat())
			return '\uF0BA';
		if (acc.isDoubleSharp())
			return '\uF099';// F0DC
		if (acc.equals(Accidental.HALF_FLAT))
			return '\uF0F5';
		if (acc.equals(Accidental.FLAT_AND_A_HALF))
			return '\uF0F6';
		if (acc.equals(Accidental.HALF_SHARP))
			return '\uF0F7';
		if (acc.equals(Accidental.SHARP_AND_A_HALF))
			return '\uF0F8';

		// case Accidental.COMMA_SHARP_ONE: return '\uF08E'; //Koma Diyezi
		// case Accidental.COMMA_SHARP_FIVE: return '\uF08F'; //Küçük Mücennep
		// Diyezi
		// case Accidental.COMMA_SHARP_EIGHT: return '\uF090'; //Buyuk Mücennep
		// Diyezi
		...
		throw new MissingGlyphException("Accidental type " + acc.toString(),
				this);
	}
	...
	public char getClef(Clef clef) throws MissingGlyphException {
		if (clef.isG()) {
			...
			return '\uF03A';
		else if (clef.equals(Clef.ottava_8va))
			return '\uF0C3';
		else if (clef.equals(Clef.ottava_8vb))
			return '\uF0D7';
		else if (clef.equals(Clef.ottava_15ma))
			return '\uF0DB';
		else if (clef.equals(Clef.ottava_15mb))
			return '\uF100';
		else if (clef.equals(Clef.NONE))
			return ' ';
	...
	public char getDecoration(int decoration, boolean inverted) {
		...
		case Decoration.STEM_COMBINE_UP_TRIPLE:
			return '\uF0BE';
		case Decoration.FINGERING_0:
			return getFingeringsDigits(0)[0];
		case Decoration.FINGERING_1:
			return getFingeringsDigits(1)[0];
		case Decoration.FINGERING_2:
			return getFingeringsDigits(2)[0];
		case Decoration.FINGERING_3:
			return getFingeringsDigits(3)[0];
		case Decoration.FINGERING_4:
			return getFingeringsDigits(4)[0];
		case Decoration.FINGERING_5:
			return getFingeringsDigits(5)[0];
		case Decoration.PLUS:
			return '\uF08E';
		case Decoration.OPEN:
			return '\uF08F';
		case Decoration.THUMB:
			return '\uF098';// thumb, snap
		...
		case Decoration.GENERAL_GRACING:
			return '\uF015';
		default:
			throw new MissingGlyphException("Decoration " + decoration, this);
		}
	}
	...
	public char getTimeSignatureAbbreviated(TimeSignature ts)
		...
		if (ts.equals(TimeSignature.SIGNATURE_4_4)) {
			...
			throw new IllegalArgumentException("There is no "
					+ "abbreviated symbol for time signature " + ts.toString());
	}
