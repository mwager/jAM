 ...
 * Texts:
 * <ul>
 * <li>At top center: title and subtitles
 ...
 * Main font family is Palatino Linotype, which is a bit wider, easier to read
 * (especially for lyrics). Arial and Dialog are used in case of lack of
 * Palatino.
 * 
...
public class DefaultScoreTemplate extends ScoreTemplate implements Cloneable {
	...
	private static final byte[] FOOTNOTES = new byte[] {
			...
			ScoreElements.TEXT_SOURCE, ScoreElements.TEXT_TRANSCRNOTES,
	...
	private void fonts() {
		...
		// order of preference
		setDefaultTextFontFamilyName(new String[] { "Palatino Linotype",
				"Arial", "Dialog" });
		// Part Labels
		setTextFontFamilyName(ScoreElements.PART_LABEL, new String[] {
				"Georgia", "Verdana", "Dialog" });
		// foot notes
		setTextFontFamilyName(FOOTNOTES, new String[] { "Arial", "Dialog" });

		setTextStyle(new byte[] { ScoreElements.TEXT_TITLE,
				ScoreElements.TEXT_SUBTITLE, ScoreElements.TEXT_COMPOSER,
				ScoreElements.TEXT_LYRICIST, ScoreElements.TEXT_RHYTHM,
				ScoreElements.TEXT_ORIGIN }, Font.BOLD);
		...
		// urlLink.put(TextAttribute.UNDERLINE,
	...
	private void positions() {
		setPosition(new byte[] { ScoreElements.TEXT_TITLE,
				ScoreElements.TEXT_SUBTITLE }, VerticalPosition.TOP,
				HorizontalPosition.CENTER);
		setPosition(new byte[] { ScoreElements.TEXT_COMPOSER,
				ScoreElements.TEXT_ORIGIN }, VerticalPosition.TOP,
				HorizontalPosition.RIGHT);
		setPosition(new byte[] { ScoreElements.TEXT_LYRICIST,
				ScoreElements.TEXT_RHYTHM, ScoreElements.TEXT_GROUP,
				ScoreElements.TEXT_PARTS_ORDER }, VerticalPosition.TOP,
				HorizontalPosition.LEFT);

		setPosition(new byte[] { ScoreElements.TEXT_CHORDS,
				ScoreElements.TEMPO, ScoreElements.PART_LABEL },
				VerticalPosition.ABOVE_STAFF, HorizontalPosition.LEFT);

		setPosition(FOOTNOTES, VerticalPosition.BOTTOM, HorizontalPosition.LEFT);

		setPosition(ScoreElements._DEFAULT, VerticalPosition.BOTTOM,
				HorizontalPosition.LEFT);
	}
