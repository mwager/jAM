 ...
 * <CENTER> <IMG src="../../../images/scoreEx.jpg"/> </CENTER> <BR/>
 * To render a tune score, just invoke the <TT>setTune(Tune)</TT> method with
 * your tune as parameter.<BR/>
 * Basically, a score if composed of {@link abc.ui.swing.JScoreElement score
 * elements}
 * 
 * @see Tune
 * @see JScoreElement
 */
public class JScoreComponent extends JComponent implements
		ScoreTemplateChangeListener {

	private static final long serialVersionUID = 7903517456075406436L;
	private static final Color SELECTED_ITEM_COLOR = Color.RED;
	/**
	 * The graphical representation of the tune currently set. <TT>null</TT> if
	 * no tune is set.
	 */
	...
	/**
	 * The buffer where the score image is put before rendition in the swing
	 * component.
	 */
	...
	/**
	 * A vector of selected item(s) in this score. <TT>null</TT> if no item is
	 * selected.
	 */
	...
	protected void initGfx() {
		m_bufferedImage = new BufferedImage((int) m_dimension.getWidth(),
				(int) m_dimension.getHeight(), BufferedImage.TYPE_INT_ARGB);
		m_bufferedImageGfx = (Graphics2D) m_bufferedImage.createGraphics();
		// staffLinesSpacing =
		// (int)(m_metrics.getStaffCharBounds().getHeight()*2.5);
	}
	...
	private Graphics2D getGraphics2D() {
		if ((m_bufferedImage == null) || (m_bufferedImageGfx == null))
			initGfx();
	...
	public void paint(Graphics g) {
		if (m_isBufferedImageOutdated) {
			...
			if (m_bufferedImage == null
					|| m_dimension.getWidth() > m_bufferedImage.getWidth()
					|| m_dimension.getHeight() > m_bufferedImage.getHeight()) {
				initGfx();
			}
			m_bufferedImageGfx.setColor(getBackground());
			m_bufferedImageGfx.fillRect(0, 0, (int) m_bufferedImage.getWidth(),
					(int) m_bufferedImage.getHeight());
	 ...
	 * 
	 * @param size
	 *            The size of the font used to display the music score expressed
	 *            in ?
	 * @deprecated use {@link #getScoreMetrics() getScoreMetrics()}.
	 *             {@link ScoreMetrics#setNotationSize(float)
	 *             setNotationSize(float)} and then {@link #refresh()} or
	 *             better, set attribute {@link ScoreAttribute#NOTATION_SIZE} in
	 *             template
	 */
	public void setSize(float size) {
		System.err.println("Warning! deprecated method setSize");
		getTemplate().setAttributeSize(ScoreAttribute.NOTATION_SIZE, size,
				SizeUnit.PT);
	}
	...
	/**
	 * Sets the note stem direction on a score. 0=auto 1=up 2=down
	 * 
	 * @deprecated use template attribute
	 *             {@link ScoreAttribute#NOTE_STEM_POLICY}
	 */
	public void setStemmingPolicy(byte policy) {
		getTemplate().setAttribute(ScoreAttribute.NOTE_STEM_POLICY,
				new Byte(policy));
	}

	/**
	 * @deprecated use getTemplate().getAttributeNumber
	 *             {@link ScoreAttribute#NOTE_STEM_POLICY}
	 */
	public byte getStemmingPolicy() {
		return (byte) getTemplate().getAttributeNumber(
				ScoreAttribute.NOTE_STEM_POLICY);
	}

	/**
	 * Refresh the score, blanks the component, compute and draw the score.<br>
	 * Call this method to refresh the component after changes on its
	 * {@link #getEngraver() Engraver} and {@link #getScoreMetrics()
	 * ScoreMetric}.
	 */
	public void refresh() {
		initGfx();
	 ...
	 * 
	 * @param os
	 *            The PNG output stream
	 * @throws IOException
	 *             Thrown if the given file cannot be accessed.
	 */
	public void writeScoreTo(OutputStream os) throws IOException {
		...
		BufferedImage bufferedImage = new BufferedImage(
				(int) m_dimension.getWidth(), (int) m_dimension.getHeight(),
				BufferedImage.TYPE_INT_ARGB);
		Graphics2D bufferedImageGfx = (Graphics2D) bufferedImage
				.createGraphics();
		bufferedImageGfx.setColor(getBackground());// Color.WHITE);
		bufferedImageGfx.fillRect(0, 0, (int) bufferedImage.getWidth(),
	...
	/**
	 * Sets tune and template in one shot, this is optimized version, it avoid 2
	 * calculation (first at setTemplate and second at setTune).
	 * 
	 * @param tune
	 * @param template
	 */
	...
	public void setTune(Tune tune) {
		m_jTune = new JTune(tune, new Point(0, 0), getTemplate());
		m_jTune.setColor(getForeground());

//		m_jTune.setColor(Color.RED);
	...
	/**
	 * Changes the justification of the rendition score. This will set the staff
	 * lines aligment to justify in order to have a more elegant display.
	 * 
	 * @param isJustified
	 *            <TT>true</TT> if the score rendition should be justified,
	...
	/**
	 * Return <TT>true</TT> if the rendition staff lines alignment is justified,
	 * <TT>false</TT> otherwise.
	 * 
	 ...
	 *         <TT>null</TT> is returned if no item is found at the given
	 *         location.
	 */
	...
	/**
	 * Highlights the given score element in the score. If an item was
	 * previously selected, this previous item is unselected.
	 * 
	 * @param elmnt
	 *            The music element to be highlighted in the score.
	 *            <TT>null</TT> can be specified to remove highlighting.
	...
	public void setSelectedItem(MusicElement elmnt) {
		...
		if (elmnt != null)
			// r =
			// (JScoreElementAbstract)m_jTune.getRenditionObjectsMapping().get(elmnt);
			r = m_jTune.getRenditionObjectFor(elmnt);
		// if (r!=null)
		// System.out.println("Selecting item " + elmnt + "->" + r + "@" +
		// r.getBase());
		setSelectedItem(r);
	}
	 ...
	 * 
	 * @param elmnt
	 *            The score rendition element to be highlighted in the score.
	 *            <TT>null</TT> can be specified to remove highlighting.
	...
	/**
	 * Highlights the given elements in the score. If item(s) was previously
	 * selected, it is unselected.
	 * 
	 * @param elments
	 *            A collection of score element to be highlighted in the score.
	 *            <TT>null</TT> or empty collection can be specified to remove
	 *            highlighting.
	 */
	 ...
	 * @param elmnt
	 *            A tune element.
	 * @return The graphical score element that corresponds to the given tune
	 *         element. <TT>null</TT> is returned if the given tune element does
	 *         not have any graphical representation.
	 */
	public JScoreElement getRenditionElementFor(MusicElement elmnt) {
		if (m_jTune != null)
			// return
			// (JScoreElement)m_jTune.getRenditionObjectsMapping().get(elmnt);
