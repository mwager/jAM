...
class JDecoration extends JScoreElementAbstract {
	...
	private boolean isInvertable() {
		...
		if ((pos != ABOVE_STAFF) && (pos != UNDER_STAFF) && (pos != ABOVE_NOTE)
				&& (pos != UNDER_NOTE)) {
			char charDeco = getMusicalFont().getDecoration(m_decoration, false);
			char charDeco2 = getMusicalFont().getDecoration(m_decoration, true);
	...
	public double render(Graphics2D g2) {
		try {
			char charDeco = getMusicalFont().getDecoration(m_decoration,
					m_isInverted);
			...
			if (m_scoreElement != null) {
				...
				for (int i = m_scoreElement.m_jDecorations.size() - 1; i >= 0; i--) {
					...
					JDecoration jdec = (JDecoration) m_scoreElement.m_jDecorations
							.elementAt(i);
					if (jdec == this)
						break;
			...
			g2.drawChars(new char[] { charDeco }, 0, 1, (int) x, (int) y);
