...
class JChord extends JNoteElementAbstract {
	...
	/**
	 * A flag that tells that the chord is part of a normalized chord then
	 * accidental and other positions computed for normalized chord are applied
	 * to "childs".
	 */
	 ...
	 * and to ease the rendition : the rendition is made on set of notes with
	 * the same strict duration. Such normalized chords are ordered with ascend
	 * strict durations in this array.
	 */
	...
	public JChord(MultiNote multiNote, Clef clef, ScoreMetrics metrics,
			Point2D base) {
		super(multiNote, clef, base, metrics);
		this.multiNote = multiNote;

		m_notes = multiNote.toArray();
		// create JNotePartOfGroup instances. Those instances stay the same when
		// the base is changed.
		...
		if (multiNote.hasUniqueStrictDuration()) {
			...
			for (int i = 0; i < m_notes.length; i++) {
				m_sNoteInstances[i] = new JChordNote(m_notes[i], getClef(),
						base, getMetrics());
				((JChordNote) m_sNoteInstances[i]).setIsLowest(i == 0);
				((JChordNote) m_sNoteInstances[i])
						.setIsHighest(i == m_notes.length - 1);
				if (i > 0) {
					Interval interval = new Interval(m_notes[i - 1],
							m_notes[i], null);
			...
			// m_sNoteInstances[0] = new JNote(multiNote.getHighestNote(), base,
			// m_metrics);
			m_sNoteInstances[0] = new JChordNote(multiNote.getHighestNote(),
					getClef(), base, getMetrics());
			...
			if (durations.length > 2)
				System.err
						.println("abc4j - warning : chords with more than 2 differents strict duration aren't supported : only 2 smaller durations are taken into account");
			...
			if (longest.getLowestNote().isLowerThan(shortest.getLowestNote())) {
				m_normalizedChords[1].setStemUp(false);
				m_normalizedChords[0].setStemUp(true);
			} else {
	...
	/**
	 * The anchor of the chord is the one that present the rhytm (the highest
	 * one in case of stem up, the lowest one in case of stem down)
	 * 
	 * @param note
	 * @param base
	 * @param metrics
	 * @return
	 */
	protected JNote createAnchorNote(Note note, Point2D base,
	 ...
	 * duration.
	 */
	protected JChord createNormalizedChord(MultiNote mNote, ScoreMetrics mtrx,
			Point2D base) {
	...
	/*
	 * JNotePartOfGroup[] getRenditionElements() { return m_sNoteInstances; }
	 */
	...
	protected JNote getHighestNote() {
		...
		if (m_normalizedChords != null) {
			for (int i = 0; i < m_normalizedChords.length; i++) {
				...
				if (highest == null)
					highest = current;
				else if (((Note) current.getMusicElement())
						.isHigherThan((Note) highest.getMusicElement()))
					highest = current;
			}
		} else {
			for (int i = 0; i < m_sNoteInstances.length; i++) {
			...
			for (int i = 0; i < m_normalizedChords.length; i++) {
				...
				if (lowest == null)
					lowest = current;
						...
						.isLowerThan((Note) lowest.getMusicElement()))
					lowest = current;
			}
		} else {
			for (int i = 0; i < m_sNoteInstances.length; i++) {
	...
	protected void onBaseChanged() {
		...
		if (m_normalizedChords == null) {
			...
			if (m_stemYEndForChord == -1) {
				...
				int stemLength = getMetrics().getStemLengthForContext(
						ScoreMetrics.NOTATION_CONTEXT_NOTE);
				int halfNoteHeight = (int) (getMetrics().getNoteHeight() / 2);
				int highestNoteY = (int) getHighestNote()
						.getStemUpBeginPosition().getY()
						- stemLength
						- halfNoteHeight;
				int lowestNoteY = (int) getLowestNote()
						.getStemDownBeginPosition().getY()
						+ stemLength
						+ halfNoteHeight;
			...
			for (int i = notes.length - 1; i >= 0; i--) {
				...
				if (notes[i].hasAccidental()) {
					...
					if (refAccidentedNote == -1) {
						...
						// if interval between reference and this note is
						// smaller than
						// a sixth, accidentals are moved on the left
						// else accidental is at normal position, set note as
						// referenec
						Interval interv = new Interval(
								notes[refAccidentedNote], notes[i], null);
					...
					// For stem down chord with inverted heads, add a .5 (a note
					// head)
					...
					double accWidth = jnote.getNotePosition().getX()
							- jnote.getBase().getX();
		...
		m_noteHeadX = getBase().getX()
				+ (graceNotesAfter ? 0 : graceNotesWidth);// + accidentalsWidth;
		...
		if (m_normalizedChords == null) {
			...
			for (int i = 0; i < m_sNoteInstances.length; i++) {
				...
				if (m_sNoteInstances[i].getStemBeginPosition().getX() > biggestStemX)
					biggestStemX = m_sNoteInstances[i].getStemBeginPosition()
							.getX();
			...
			for (int i = 0; i < m_sNoteInstances.length; i++) {
				...
				Point2D newStemBegin = new Point2D.Double(biggestStemX,
						stemBegin.getY());
			...
			for (int i = 0; i < m_normalizedChords.length; i++) {
				...
				if (accidentalsWidth > 0) {
					...
					newBase.setLocation(newBase.getX() + accidentalsWidth
							- tmpAccWidth, newBase.getY());
		...
		if (m_normalizedChords == null) {
			...
			slurUnderAnchorOutOfStem = stemUp ? slurUnderAnchor
					: new Point2D.Double(getBoundingBox().getMinX(),
							getBoundingBox().getMaxY()
									+ metrics.getSlurAnchorYOffset());
			slurAboveAnchor = m_sNoteInstances[m_sNoteInstances.length - 1]
					.getSlurAboveAnchor();
			slurAboveAnchorOutOfStem = stemUp ? new Point2D.Double(
					getBoundingBox().getMaxX(), getBoundingBox().getMinY()
							- metrics.getSlurAnchorYOffset()) : slurAboveAnchor;
		...
		if (m_jGracenotes != null) {
			...
			if (!graceNotesAfter)
				...
				gracesPosition = new Point2D.Double(m_noteHeadX + chordWidth
		...
		// m_width = /*c_width + */graceNotesWidth + accidentalsWidth +
		// chordWidth;
	...
	public Rectangle2D getBoundingBox() {
		...
		// other...
		bb.setRect(bb.getX(), bb.getY(), Math.max(bb.getWidth(), getWidth()),
				bb.getHeight());
		return bb;
	}
	...
	public double render(Graphics2D context) {
		...
		if (m_normalizedChords == null) {
			for (int i = 0; i < m_sNoteInstances.length; i++) {
				...
				// //System.out.println("\tstemYEnd="+((JNotePartOfGroup)
				// n).getStemYEnd());
				// }
	...
	public void setStemUp(boolean isUp) {
		...
		if (isUp) {
			// m_sNoteInstances[0] = new JChordNote(m_notes[0],
			// m_sNoteInstances[0].getBase(), getMetrics());
			...
			// anchor = createAnchorNote(highestNote, highestJNote.getBase(),
			// getMetrics());
			...
			// m_sNoteInstances[m_sNoteInstances.length-1] = new
			// JChordNote(m_notes[m_notes.length-1],
			// m_sNoteInstances[m_sNoteInstances.length-1].getBase(),
			// getMetrics());
			...
			// anchor = createAnchorNote(lowestNote, lowestJNote.getBase(),
			// getMetrics());
			// m_sNoteInstances[0] = anchor;
			// Why recreate new JChordNotes???
			anchor = m_sNoteInstances[0];
		}
		// Apply the stem direction to the rest of the notes composing the
		// chord.
		setAutoStem(false);
		for (int i = 0; i < m_sNoteInstances.length; i++) {
			...
			if (!(this instanceof JChordPartOfGroup)) {
				if (m_sNoteInstances[i] instanceof JChordNote) {
					((JChordNote) m_sNoteInstances[i])
							.setAnchor(m_sNoteInstances[i] == anchor);
				}
			}
		}
	}
