...
class JKeySignature extends JScoreElementAbstract {
	...
	protected void onBaseChanged() {
		...
		double FsharpY = baseY
				- (JClef.getOffset(new Note(Note.f), clef) - octaveOffset)
				* noteHeight;
		double CsharpY = baseY
				- (JClef.getOffset(new Note(Note.c), clef) - octaveOffset)
				* noteHeight;
		double GsharpY = baseY
				- (JClef.getOffset(new Note(Note.g), clef) - octaveOffset)
				* noteHeight;
		double DsharpY = baseY
				- (JClef.getOffset(new Note(Note.d), clef) - octaveOffset)
				* noteHeight;
		double AsharpY = baseY
				- (JClef.getOffset(new Note(Note.A), clef) - octaveOffset)
				* noteHeight;
		double EsharpY = baseY
				- (JClef.getOffset(new Note(Note.e), clef) - octaveOffset)
				* noteHeight;
		double BsharpY = baseY
				- (JClef.getOffset(new Note(Note.B), clef) - octaveOffset)
				* noteHeight;

		double BflatY = BsharpY;// baseY - JNote.getOffset(new Note(Note.B)) *
								// noteHeight;
		double EflatY = EsharpY;// baseY - JNote.getOffset(new Note(Note.e)) *
								// noteHeight;
		double AflatY = AsharpY;// baseY - JNote.getOffset(new Note(Note.A)) *
								// noteHeight;
		double DflatY = DsharpY;// baseY - JNote.getOffset(new Note(Note.d)) *
								// noteHeight;
		double GflatY = baseY
				- (JClef.getOffset(new Note(Note.G), clef) - octaveOffset)
				* noteHeight;
		double CflatY = CsharpY;// baseY - JNote.getOffset(new Note(Note.c)) *
								// noteHeight;
		double FflatY = baseY
				- (JClef.getOffset(new Note(Note.F), clef) - octaveOffset)
				* noteHeight;
		...
		double[] sharpYs = new double[] { FsharpY, CsharpY, GsharpY, DsharpY,
				AsharpY, EsharpY, BsharpY };
		double[] flatYs = new double[] { BflatY, EflatY, AflatY, DflatY,
				GflatY, CflatY, FflatY };
		...
		for (int twoPasses = 1; twoPasses <= 2; twoPasses++) {
			Accidental accidental = twoPasses == 1 ? firstAccidental
					: secondAccidental;
			...
			if ((twoPasses == 2) && key.hasSharpsAndFlats()) {
				...
				double litSpace = getMetrics().getBounds(
						getMusicalFont().getAccidental(Accidental.NATURAL))
						.getWidth();
				baseX += litSpace;
				m_width += litSpace;
			}
			for (int i = 0; i < order.length; i++) {
				if (accidentals[order[i]].getNearestOccidentalValue() == accidental
						.getNearestOccidentalValue()) {
					char glyph = getMusicalFont().getAccidental(
							accidentals[order[i]]);
					double glyphWidth = getMetrics().getBounds(glyph)
							.getWidth();
					chars.add(cpt, new char[] { glyph });
					positions.add(cpt, new Point2D.Double(baseX, Ys[i]));
					baseX += glyphWidth;
	...
	public double render(Graphics2D context) {
		...
		for (int i = 0, j = chars.size(); i < j; i++) {
			if (chars.get(i) != null) {
				...
				context.drawChars((char[]) chars.get(i), 0, 1, (int) p.getX(),
						(int) p.getY());
			}
		}
