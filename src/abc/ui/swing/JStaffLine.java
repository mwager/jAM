...
class JStaffLine extends JScoreElementAbstract {
	...
	/**
	 * Sets the top Y of the staff line, including the space above the staff,
	 * e.g. for chord line
	 */
	...
	/**
	 * Returns the top Y of the staff line, including the space above the staff,
	 * e.g. for chord line
	 */
	...
	/**
	 * Returns true if this staff line has note(s). Returns false if it only
	 * contains clef, key, time sig., tempo, barlines...
	 * 
	 * @return
	 */
	...
	public JScoreElementAbstract[] toArray() {
		JScoreElementAbstract[] r = new JScoreElementAbstract[m_staffElements
				.size()];
	...
	/**
	 * Returns the last added element, <TT>null</TT> if the staff line is empty
	 */
	...
	public JScoreElement getScoreElementAt(Point point) {
		...
		for (int i = 0; i < m_staffElements.size(); i++) {
			scoreEl = ((JScoreElement) m_staffElements.elementAt(i))
					.getScoreElementAt(point);
			if (scoreEl != null)
				return scoreEl;
		}
		return scoreEl;
	...
	public double render(Graphics2D g) {
		...
		/*
		 * Color previousColor = g.getColor(); g.setColor(Color.LIGHT_GRAY);
		 * g.fillRect((int)(getBase().getX()),
		 * (int)(getBase().getY()-m_metrics.getStaffCharBounds().getHeight()),
	...
	public void scaleToWidth(double newWidth) {
		...
		for (int i = 0; i < m_staffElements.size(); i++) {
			...
			if ((i == (m_staffElements.size() - 1)) && (elmt instanceof JBar)) {
			...
			if (elmt instanceof JGroupOfNotes)
				((JGroupOfNotes) elmt).setInternalSpacingRatio(newWidth
						/ getWidth);
			elmt.setBase(base);
		}
		// System.out.println("StaffLine, required Width : " + newWidth +
		// " | result width=" + getWidth());
	}

	public double getWidth() {
		JScoreElementAbstract lastElmt = (JScoreElementAbstract) m_staffElements
				.lastElement();
