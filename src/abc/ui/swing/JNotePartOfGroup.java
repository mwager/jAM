...
class JNotePartOfGroup extends JNote implements JGroupableNote {
	...
	/*
	 * protected int stemX = -1; protected int stemYBegin = -1;
	 */
	protected int stemYEnd = -1;

	private boolean anchor = false;

	public JNotePartOfGroup(Note noteValue, Clef clef, Point2D base,
			ScoreMetrics c) {
	...
	protected void onBaseChanged() {
		...
		int noteY = (int) (getBase().getY() - getOffset(note)
		...
		if (stemYEnd < 0) {
			...
			if (isStemUp()) {
				// stemYBegin = (int)(displayPosition.getY() -
				// glyphDimension.getHeight()/6);
				stemYEnd = (int) (displayPosition.getY()
						+ glyphDimension.getHeight() / 2 - metrics
		...
		setStemUpBeginPosition(new Point2D.Double(noteX
				+ Math.ceil(glyphDimension.getWidth()), stemYBegin));
		setStemDownBeginPosition(new Point2D.Double(noteX, stemYBegin));

		notePosition = new Point2D.Double(displayPosition.getX(),
	...
	/*
	 * public Point2D getStemBegin() { return new Point2D.Double(stemX,
	 * stemYBegin); }
	 */
	...
	public Rectangle2D getBoundingBox() {
		Dimension glyphDimension = getMetrics().getGlyphDimension(
				getNotationContext());
		if (isStemUp()) {
			return new Rectangle2D.Double((int) (getBase().getX()),
					(int) (stemYEnd), getWidth(), getStemBeginPosition().getY()
							- stemYEnd + glyphDimension.getHeight() / 2);
		} else {
			return new Rectangle2D.Double((int) (getBase().getX()),
					getStemBeginPosition().getY() - glyphDimension.getHeight()
							/ 2, getWidth(), stemYEnd
							- getStemBeginPosition().getY() + 1
	...
	public double render(Graphics2D context) {
		...
		// context.drawChars(noteChars, 0, 1, (int)displayPosition.getX(),
		...
		if (!note.isRest() && (note.getStrictDuration() < Note.WHOLE)) {
			...
			// context.drawLine((int)getStemBeginPosition().getX()+1,
			// (int)getStemBeginPosition().getY(),
			...
			context.fillRect((int) Math.ceil(getStemBeginPosition().getX()),
					Math.min((int) getStemBeginPosition().getY(), stemYEnd), 1,
		 ...
		 * context.drawLine((int)getStemBeginPosition().getX(),
		 * (int)getStemBeginPosition().getY(),
		 * (int)getStemBeginPosition().getX(),
		 * (int)getStemBeginPosition().getY()); //(int)getNotePosition().getX(),
		 * (int)getNotePosition().getY());
		 * context.setColor(java.awt.Color.GREEN); Point2D m_base = getBase();
