...
/**
 * This class is in charge of rendering a group of grace notes whose stems
 * should be linked.
 */
class JGroupOfGraceNotes extends JGroupOfNotes implements JGraceElement {
	...
	public JGroupOfGraceNotes(ScoreMetrics metrics, Point2D base,
			NoteAbstract[] notes, Clef clef, Engraver engraver) {
		super(metrics, base, notes, clef, engraver);

		JGraceNotePartOfGroup graceNote = null;
		for (int i = 0; i < notes.length; i++) {
			graceNote = new JGraceNotePartOfGroup((Note) m_notes[i], clef,
					base, metrics);
