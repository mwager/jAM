...
public class JText extends JScoreElementAbstract {
	...
	/**
	 * Override this method if you want to modify some properties depending on
	 * the attached score element
	 */
	...
	public byte getTextJustification() {
		if (m_textJustification == -1) {
			...
			case HorizontalPosition.CENTER:
				m_textJustification = TextJustification.CENTER;
				break;
			case HorizontalPosition.RIGHT:
				m_textJustification = TextJustification.RIGHT;
				break;
			default:
	...
	public double getWidth() {
		...
		for (int i = 0; i < lines.length; i++) {
			max = Math.max(
					max,
					(double) getMetrics().getTextFontWidth(m_textField,
							lines[i]));
		}
		return max;
	}
	...
	public double render(Graphics2D g2) {
		...
		LineMetrics lineMetrics = font.getLineMetrics(getText(),
				g2.getFontRenderContext());
		float leading = (float) Math.ceil(Math.max(lineMetrics.getLeading(),
		...
		char double_flat = getMusicalFont().getAccidental(
				Accidental.DOUBLE_FLAT);
		char double_sharp = getMusicalFont().getAccidental(
		...
		for (int line_count = 0; line_count < lines.length; line_count++) {
			...
			if (getTextJustification() == TextJustification.JUSTIFIED) {
				as.addAttribute(TextAttribute.JUSTIFICATION,
			...
			g2.drawString(as.getIterator(), (float) x,
