...
public class JTune extends JScoreElementAbstract {
	 ...
	 * {@link #render(Graphics2D)} recalculate the complete tune
	 */
	...
	/**
	 * Hashmap that associates <DEL>ScoreElement</DEL> <B>MusicElement</B>
	 * instances (key) and JScoreElement instances(value). It contains : JChord,
	 * JNote, JNotePartOfGroup, JChordNote, JGraceNote, JGraceNotePartOfGroup
	 * instances.
	 */
	...
	private int getTransposition() {
		return (int) getTemplate().getAttributeNumber(
				ScoreAttribute.TRANSPOSITION);
	}
	...
	/**
	 * Returns the rendition object (JNote, JChord...) for the given music
	 * element reference
	 * 
	 * @param note
	 * @return
	...
	public JScoreElement getRenditionObjectFor(MusicElementReference ref) {
		...
		if (jse == null) {
			...
			while (e.hasMoreElements()) {
				MusicElementReference mer = (MusicElementReference) e
						.nextElement();
				if (ref.equals(mer)) {
	...
	/**
	 * Returns the rendition object (JNote, JChord...) for the given notation
	 * (Note, MultiNote)
	 * 
	 * @param note
	 * @return
	...
	public JScoreElement getRenditionObjectFor(MusicElement musicElement) {
		...
		/*
		 * if ((ret == null) && (musicElement instanceof Note)) { Note note =
		 * (Note) musicElement; Enumeration e = m_scoreElements.keys(); while
		 * (e.hasMoreElements()) { MusicElement m = (MusicElement)
		 * e.nextElement(); if ((m instanceof MultiNote) && (((MultiNote)
		 * m).contains(note))) { JChord chord = (JChord) m_scoreElements.get(m);
		 * if (chord.m_normalizedChords == null) { JNote[] jnotes =
		 * chord.getScoreElements(); for (int i = 0; i < jnotes.length; i++) {
		 * if (jnotes[i].getMusicElement().equals(note)) return (jnotes[i]); } }
		 * } } }
		 */
		return ret;
	}
	 ...
	 * rendition objects.
	 * 
	 * @return Returns a vector of JGroupOfNotes instances appearing in this
	 *         score
	 */
	...
	/**
	 * Returns the part of the score (as a JScoreElement instance) located at a
	 * given point.
	 * 
	 * @param location
	 *            A location as a point.
	 * @return The part of the score (as a JScoreElement instance) located at a
	 *         given point. <TT>NULL</NULL> is returned if not JScoreElement is
	...
	public JScoreElement getScoreElementAt(Point location) {
		...
		for (int i = 0; i < m_staffLines.size(); i++) {
			scoreEl = ((JStaffLine) m_staffLines.elementAt(i))
					.getScoreElementAt(location);
			if (scoreEl != null)
				return scoreEl;
			scoreEl = null;
		}
	...
	private void computeTextFieldToJText(byte textField) {
		if ((textField == ScoreElements.TEXT_TITLE)
			...
			if (titles != null) {
				for (int i = 0; i < titles.length; i++) {
					...
					if ((i == 0) && (textField == ScoreElements.TEXT_TITLE)) {
						// title
						m_headerAndFooterTexts.add(new JText(getMetrics(),
								titles[i], ScoreElements.TEXT_TITLE));
					} else if ((i > 0)
							&& (textField == ScoreElements.TEXT_SUBTITLE)) {
						// subtitles
						m_headerAndFooterTexts.add(new JText(getMetrics(),
								titles[i], ScoreElements.TEXT_SUBTITLE));
					}
				}
			}
			...
			if (m_tune.getMultiPartsDefinition() != null) {
				...
				if (hasDouble) {
					m_headerAndFooterTexts.add(new JText(getMetrics(), txt,
			...
			case ScoreElements.TEXT_ORIGIN:// =AREA
				...
				if (text == null)
					text = "";
				if ((m_tune.getArea() != null)
						&& (m_tune.getArea().length() > 0))
					text += (text.length() > 0 ? ", " : "") + m_tune.getArea();
				break;
			...
			if ((text != null) && (text.length() > 0)) {
				m_headerAndFooterTexts.add(new JText(getMetrics(), text,
						textField));
			}
		}
	}
	...
	private void compute() {
		getBase().setLocation(
				getTemplate().getAttributeSize(ScoreAttribute.MARGIN_LEFT),
		...
		/* if (isShowTitles()) */{
			...
			while (itHeaders.hasNext()) {
				...
				// only headers
				if (oneText.getVerticalPosition() != VerticalPosition.TOP)
					continue;
		...
		while (itVoices.hasNext()) {
			...
			for (m_index = 0, size = voice.size(); m_index < size; m_index++) {
				...
				// s.getReference().toString());
				// ==== Notes>quarter, rests, notes without slur,tuplet ====
				if ((!(s instanceof Note || s instanceof MultiNote)
						|| (s instanceof Note && ((Note) s).isRest())
						// if we were in a tuplet and the current note isn't
						// part of tuplet anymore or part of another tuplet
						|| (s instanceof NoteAbstract
								&& tupletContainer != null && (!tupletContainer
								.equals(((NoteAbstract) s).getTuplet())))
						// if we weren't in a tuplet and the new note is part of
						// a tuplet.
						|| (s instanceof NoteAbstract
								&& tupletContainer == null && ((NoteAbstract) s)
								.isPartOfTuplet())
						|| (s instanceof Note && ((Note) s).getStrictDuration() >= Note.QUARTER) || (s instanceof MultiNote && (!((MultiNote) s)
						.hasUniqueStrictDuration() || ((MultiNote) s)
						.getLongestNote().getStrictDuration() >= Note.QUARTER)))

						&& lessThanQuarter.size() != 0) {
					// this is is the end of the group, append the current group
					// content to the score.
				...
				if (s instanceof KeySignature) {
					...
					if (currentStaffLineInitialized
						...
						appendToScore(new JKeySignature(currentKey,
								previousKey, cursor, getMetrics()));
						try {
							previousKey = (KeySignature) currentKey.clone();
						} catch (CloneNotSupportedException never) {
							never.printStackTrace();
				...
				if (s instanceof TimeSignature) {
					...
					if (currentStaffLineInitialized
						...
						appendToScore(new JTimeSignature(currentTime, cursor,
								getMetrics()));
				} else
				// ==== MultiNote ====
				if (s instanceof MultiNote) {
					...
					if (tiesStart != null)
						for (int j = 0; j < tiesStart.length; j++)
							m_beginningNotesLinkElements
									.addElement(tiesStart[j]);
					// checks if the shortest durations of the multi note is
					...
					if (((MultiNote) s).getStrictDurations()[0] < Note.QUARTER)
						...
						appendToScore(new JChord((MultiNote) s, currentClef,
								getMetrics(), cursor));
					}
				...
				if (s instanceof Note) {
					...
					if (strictDur < Note.QUARTER && !note.isRest()) {
						...
						// System.out.println("duration in group " +
						// durationInGroup);
						lessThanQuarter.add(note);
						/*
						 * if (durationInGroup>=maxDurationInGroup) {
						 * appendToScore(lessThanQuarter);
						 * lessThanQuarter.clear(); durationInGroup = 0; }
						 */
					} else {
						JNote noteR = new JNote(note, currentClef, cursor,
								getMetrics());
				...
				if (s instanceof RepeatBarLine) {
					appendToScore(new JRepeatBar((RepeatBarLine) s, cursor,
							getMetrics()));
				...
				if (s instanceof PartLabel) {
					appendToScore(new JPartLabel(getMetrics(), cursor,
							(PartLabel) s));
				} else
				// ==== Tempo ====
		...
		cursor.setLocation(cursor.getX(), cursor.getY()
		...
		if (m_staffLines != null) {
			double maxWidth = 0;
			for (int i = 0; i < m_staffLines.size(); i++) {
				JStaffLine currentStaffLine = (JStaffLine) m_staffLines
						.elementAt(i);
	...
	/**
	 * Append an element to the score, calculating its position
	 * 
	 * @param element
	 */
	private void appendToScore(JScoreElementAbstract element) {
		...
		if (lastElement != null) {
			if ((lastElement instanceof JPartLabel)
				...
				cursor.setLocation(cursor.getX()
						+ getMetrics().getNotesSpacing(), cursor.getY());
		...
		if (!(element instanceof JPartLabel) && !(element instanceof JTempo)) {
		...
		if ((element instanceof JPartLabel) && !currentStaffLine.hasNotes()) {
			((JPartLabel) element)
					.setTextJustification(TextJustification.RIGHT);
		}
		...
		if (element instanceof JNote)
			m_scoreElements.put(element.getMusicElement().getReference(),
					element);
			...
			for (int j = 0; j < g.getRenditionElements().length; j++) {
				if ((g.getRenditionElements()[j]) instanceof JNote)
					m_scoreElements.put(g.getMusicElements()[j].getReference(),
							g.getRenditionElements()[j]);
				else if (g.getRenditionElements()[j] instanceof JChord) {
					JNote[] jnotes = ((JChord) g.getRenditionElements()[j])
							.getScoreElements();
					// Vector ...=... getNotesAsVector() <-- CLONES!!!
					// Note[] notes =
					// ((MultiNote)((JChord)g.getRenditionElements()[j]).getMusicElement()).toArray();
					// adds all the notes of the chords into the hashtable
					// TODO the ordering of the get notes as vector and the
					// jnotes should be the same...
					...
					for (int i = 0; i < jnotes.length; i++)
						m_scoreElements.put(jnotes[i].getMusicElement()
								.getReference(), jnotes[i]);
					// adds also the chords itself
					m_scoreElements.put(g.getRenditionElements()[j]
							.getMusicElement().getReference(), g
							.getRenditionElements()[j]);
				}
			}
		} else if (element instanceof JChord) {
			JNote[] jnotes = ((JChord) element).getScoreElements();
			// Vector notes =
			// ((MultiNote)((JChord)element).getMusicElement()).getNotes();
			// adds all the notes of the chords into the hashtable
			// TODO the ordering of the get notes as vector and the jnotes
			// should be the same...
			// System.out.println("Warning - abc4j - current limitation prevents you from using chords with different notes lengths.");
			for (int i = 0; i < jnotes.length; i++)
				m_scoreElements.put(jnotes[i].getMusicElement().getReference(),
						jnotes[i]);
			// adds also the chords itself
			m_scoreElements.put(element.getMusicElement().getReference(),
					element);
	...
	private void appendToScore(ArrayList lessThanQuarterGroup) {
		if (lessThanQuarterGroup.size() > 0) {
			...
			JScoreElementAbstract[] renditionResultRootsElmts = new JScoreElementAbstract[lessThanQuarterGroup
					.size()];
			NoteAbstract[] notes = (NoteAbstract[]) lessThanQuarterGroup
					.toArray(new NoteAbstract[lessThanQuarterGroup.size()]);
			if (notes.length == 1) {
				if (notes[0] instanceof Note) {
					renditionResult = new JNote((Note) notes[0], currentClef,
							cursor, getMetrics());
					renditionResultRootsElmts[0] = renditionResult;
				} else {
					// This is a multi note
					renditionResult = new JChord((MultiNote) notes[0],
							currentClef, getMetrics(), cursor);
					renditionResultRootsElmts[0] = renditionResult;
				}
			} else {
				renditionResult = new JGroupOfNotes(getMetrics(), cursor,
						notes, currentClef, getEngraver());
				// if tuplet, add to m_beginningNotesLinkElements
				if (notes[0].getTuplet() != null)
					m_beginningNotesLinkElements.add(notes[0]);
				renditionResultRootsElmts = ((JGroupOfNotes) renditionResult)
						.getRenditionElements();
			}
	...
	private void applyStemmingPolicy(JScoreElementAbstract element) {
		if (element != null && element instanceof JStemmableElement) {
			...
			if (stemmable.isFollowingStemmingPolicy()) {
				byte noteStemPolicy = (byte) getTemplate().getAttributeNumber(
						ScoreAttribute.NOTE_STEM_POLICY);
			...
			if (stemmable.isStemUp() && (count >= 2)) {
				...
				if ((N1 instanceof JStemmableElement)
					...
					if (n.getHeight() == ((JNote) N1).getClef().getMiddleNote()
							.getHeight()) {
			...
			// grace notes
			byte gracenoteStemPolicy = (byte) getTemplate().getAttributeNumber(
					ScoreAttribute.GRACENOTE_STEM_POLICY);
			if (element instanceof JNoteElementAbstract) {
				...
				if ((jnea.m_jGracenotes != null)
					...
					if (gracenoteStemPolicy == STEMS_AUTO) {
						...
						boolean isup = (gracenoteStemPolicy == STEMS_UP) ? true
								: false;
	...
	public double render(Graphics2D g2) {
		...
		// staff line width
		// int staffCharNb =
		// (int)(getWidth()/getMetrics().getStaffCharBounds().getWidth());
			...
			if (align == HorizontalPosition.CENTER) {
				...
				text.setBase(new Point2D.Double(getBase().getX()
						+ getTemplate().getAttributeSize(
								ScoreAttribute.FIRST_STAFF_LEFT_MARGIN) + 30, y));
	...
	/**
	 * Triggers the re computation of all staff lines elements in order to get
	 * the alignment justified.
	 */
	private void justify() {
		if (m_staffLines.size() > 1) {
			double maxWidth = ((JStaffLine) m_staffLines.elementAt(0))
					.getWidth();
	...
	private void renderSlursAndTies(Graphics2D g2) {
		...
		for (int j = 0; j < m_beginningNotesLinkElements.size(); j++) {
			NoteAbstract n = (NoteAbstract) m_beginningNotesLinkElements
					.elementAt(j);
	...
	protected void drawLink(Graphics2D g2, TwoNotesLink slurDef) {
		...
		if (jSlurDef.isTuplet()) {
			...
			if (jSlurDef.getTupletControlPoint() == null) {
				jSlurDef.setTupletControlPoint(new Point2D.Double(points[START]
						.getX()
						+ (points[END].getX() - points[START].getX())
						/ 2, points[START].getY()
						- getMetrics().getSlurAnchorYOffset() * 5));
			}
			points[CONTROL].setLocation(jSlurDef.getTupletControlPoint());
		}
	...
	private JSlurOrTie getJSlurOrTie(TwoNotesLink slurDef) {
		JNoteElementAbstract elmtStart = (JNoteElementAbstract) getRenditionObjectFor(slurDef
				.getStart());
		return elmtStart.getJSlurDefinition();
	}

	// FIXME : well, slurs and ties shouldn't be drawn the same way :
	// slurs, should be drawn under/upper all the notes that are part of the
	// slur
	// ties, should consider notes between the 2 notes tied.
	/**
	 * Returns the 3 points of a slur <br>
	 * Method used by {@link #drawLink(Graphics2D, TwoNotesLink)}
	 * 
	 * @param slurDef
	...
	private Point2D[] getLinkPoints(TwoNotesLink slurDef) {
		...
		elmtStart = (JNoteElementAbstract) getRenditionObjectFor(slurDef
				.getStart());
		elmtEnd = (JNoteElementAbstract) getRenditionObjectFor(slurDef.getEnd());
		if (elmtStart == null || elmtStart == null) {
			System.err
					.println("getLinkPoints error: elmtStart or elmtEnd are null");
			return new Point2D[] {};
		}

		if ((elmtStart.getStaffLine() != null)
				&& (elmtEnd.getStaffLine() != null)) {
			if (!elmtStart.getStaffLine().equals(elmtEnd.getStaffLine())) {
				System.err
						.println("Warning - abc4j limitation : Slurs / ties cannot be drawn accross several lines for now.");
				return new Point2D[] {};
			}
		}
		...
		if (slurDef instanceof TieDefinition) {
			...
			if (position == JSlurOrTie.POSITION_AUTO
				...
				if (elmtStart.isStemUp()) {
					...
					position = ((JChordNote) elmtStart).isHighest() ? JSlurOrTie.POSITION_ABOVE
							: JSlurOrTie.POSITION_UNDER;
				} else {
					// in a chord stem down, all ties are above
					// the lowest note tie is under
					position = ((JChordNote) elmtStart).isLowest() ? JSlurOrTie.POSITION_UNDER
							: JSlurOrTie.POSITION_ABOVE;
				}
			}
			int factor;
			if ((position == JSlurOrTie.POSITION_ABOVE)
					|| (position == JSlurOrTie.POSITION_AUTO && (!elmtStart
							.isStemUp() || !elmtEnd.isStemUp()))) {
				// tie above
				ret = above;
				factor = -1;
			...
			ret[CONTROL] = new Point2D.Double(ret[START].getX()
					+ (ret[END].getX() - ret[START].getX()) / 2,
					ret[START].getY() + factor
							* getMetrics().getSlurAnchorYOffset());
			return ret;
		}
		...
		NoteAbstract startNote = // (NoteAbstract)
									// m_music.getElementByReference(slurDef.getStart());
		(NoteAbstract) elmtStart.getMusicElement();
		NoteAbstract endNote = // (NoteAbstract)
								// m_music.getElementByReference(slurDef.getEnd());
		(NoteAbstract) elmtEnd.getMusicElement();
		byte voiceNo = startNote.getReference().getVoice();
		// under in
		peakNote[UNDER_IN] = m_music.getVoice(voiceNo).getLowestNoteBewteen(
				startNote, endNote);
		peakNoteGlyph[UNDER_IN] = (JNoteElementAbstract) getRenditionObjectFor(peakNote[UNDER_IN]);

		// under out of stems
		peakNoteGlyph[UNDER_OUT] = getLowestNoteGlyphBetween(startNote,
				endNote, true);
		// enhance: if lowest glyph strictly between start/end is at same Y than
		// start or end
		if (peakNoteGlyph[UNDER_OUT] == null) { // no notes between start and
												// end
			peakNoteGlyph[UNDER_OUT] = p[UNDER_OUT][START].getY() > p[UNDER_OUT][END]
					.getY() ? elmtStart : elmtEnd; // Low
		} else {
			Point2D lowAnchor = peakNoteGlyph[UNDER_OUT]
		...
		peakNote[UNDER_OUT] = (NoteAbstract) peakNoteGlyph[UNDER_OUT]
				.getMusicElement();

		// above in
		peakNote[ABOVE_IN] = m_music.getVoice(voiceNo).getHighestNoteBewteen(
				startNote, endNote);
		peakNoteGlyph[ABOVE_IN] = (JNoteElementAbstract) getRenditionObjectFor(peakNote[ABOVE_IN]);

		// above out of stems (e.g. tuplet)
		peakNoteGlyph[ABOVE_OUT] = getHighestNoteGlyphBetween(startNote,
				endNote, true);
		// enhance: if lowest glyph strictly between start/end is at same Y than
		// start or end
		if (peakNoteGlyph[ABOVE_OUT] == null) { // no notes between start and
												// end
			peakNoteGlyph[ABOVE_OUT] = p[ABOVE_OUT][START].getY() < p[ABOVE_OUT][END]
					.getY() ? elmtStart : elmtEnd;// High
		} else {
			Point2D highAnchor = peakNoteGlyph[ABOVE_OUT]
		...
		peakNote[ABOVE_OUT] = (NoteAbstract) peakNoteGlyph[ABOVE_OUT]
				.getMusicElement();
		...
		for (short i = 0; i < 4; i++) {
			...
			if (peakNote[i].getReference().equals(slurDef.getStart())) {
				p[i][CONTROL] = new Point2D.Double(p[i][START].getX()
						+ (p[i][END].getX() - p[i][START].getX()) / 2,
						p[i][START].getY() + factor
								* getMetrics().getSlurAnchorYOffset());
				...
				switch (i) {
				case UNDER_IN:
					p[i][CONTROL] = peakNoteGlyph[UNDER_IN]
							.getSlurUnderAnchor();
					break;
				case UNDER_OUT:
					p[i][CONTROL] = peakNoteGlyph[UNDER_OUT]
							.getSlurUnderAnchorOutOfStem();
					break;
				case ABOVE_IN:
					p[i][CONTROL] = peakNoteGlyph[ABOVE_IN]
							.getSlurAboveAnchor();
					break;
				case ABOVE_OUT:
					p[i][CONTROL] = peakNoteGlyph[ABOVE_OUT]
							.getSlurAboveAnchorOutOfStem();
					break;
				}
			}

			// we don't like straight slurs, if Ystart=Ycontrol=Yend,
			// Ycontrol+=slurOffset
			double Ycontrol = p[i][CONTROL].getY(), Ystart = p[i][START].getY(), Yend = p[i][END]
					.getY();
			if ((Ycontrol == Ystart) || (Ycontrol == Yend)) {
				double Xoffset = 0;
				if (Ycontrol != Ystart)
					Xoffset = -getMetrics().getNoteWidth() / 2;
				else if (Ycontrol != Yend)
					Xoffset = getMetrics().getNoteWidth() / 2;
				p[i][CONTROL] = new Point2D.Double(p[i][CONTROL].getX()
						+ Xoffset, p[i][CONTROL].getY() + factor
						* getMetrics().getSlurAnchorYOffset() * 2);
			}
		}
		...
		if (!jSlurDef.isOutOfStems()) { // we can add all curves
			...
			if (!jSlurDef.isAbove()) { // down or auto
				...
				underCurves.add(new Point2D[] { p[UNDER_IN][START],
						p[UNDER_IN][CONTROL], p[UNDER_OUT][END] });
				underCurves.add(new Point2D[] { p[UNDER_IN][START],
						p[UNDER_OUT][CONTROL], p[UNDER_IN][END] });
				underCurves.add(new Point2D[] { p[UNDER_IN][START],
						p[UNDER_OUT][CONTROL], p[UNDER_OUT][END] });
				underCurves.add(new Point2D[] { p[UNDER_OUT][START],
						p[UNDER_IN][CONTROL], p[UNDER_IN][END] });
				underCurves.add(new Point2D[] { p[UNDER_OUT][START],
						p[UNDER_IN][CONTROL], p[UNDER_OUT][END] });
				underCurves.add(new Point2D[] { p[UNDER_OUT][START],
						p[UNDER_OUT][CONTROL], p[UNDER_IN][END] });
			...
			if (!jSlurDef.isUnder()) { // up or auto
				...
				aboveCurves.add(new Point2D[] { p[ABOVE_IN][START],
						p[ABOVE_IN][CONTROL], p[ABOVE_OUT][END] });
				aboveCurves.add(new Point2D[] { p[ABOVE_IN][START],
						p[ABOVE_OUT][CONTROL], p[ABOVE_IN][END] });
				aboveCurves.add(new Point2D[] { p[ABOVE_IN][START],
						p[ABOVE_OUT][CONTROL], p[ABOVE_OUT][END] });
				aboveCurves.add(new Point2D[] { p[ABOVE_OUT][START],
						p[ABOVE_IN][CONTROL], p[ABOVE_IN][END] });
				aboveCurves.add(new Point2D[] { p[ABOVE_OUT][START],
						p[ABOVE_IN][CONTROL], p[ABOVE_OUT][END] });
				aboveCurves.add(new Point2D[] { p[ABOVE_OUT][START],
						p[ABOVE_OUT][CONTROL], p[ABOVE_IN][END] });
		...
		for (Iterator itCurves = curves.iterator(); itCurves.hasNext();) {
			...
			try {
				SlurInfos slurInfos = new SlurInfos(p2d, getNoteGlyphesBetween(
						startNote, endNote), getMetrics());
				...
				if (slurInfos.intersect > 0) {
					...
					double distanceControlEnd = p2d[END].getX()
							- p2d[CONTROL].getX();
					double distanceControlStart = p2d[CONTROL].getX()
							- p2d[START].getX();
					int xFactor = 0;
					...
					for (float xO = 0f; xO <= 1; xO += .25f) {
						for (float yO = .5f; yO <= 2f; yO += .25f) {
							...
							newPtControl = new Point2D.Double(
									p2d[CONTROL].getX() + xO * xFactor
											* xOffset, p2d[CONTROL].getY() + yO
											* yFactor * yOffset);
							additionnalCurves.add(new Point2D[] { p2d[START],
									newPtControl, p2d[END] });
						}
					}
					// //move Y=1 note height
					// newPtControl = new Point2D.Double(p2d[CONTROL].getX(),
					// p2d[CONTROL].getY()+factor*yOffset);
					// additionnalCurves.add(new Point2D[]
					// {p2d[START],newPtControl,p2d[END]});
				...
				// TODO if flatness is high, away start/end position from
				// original
		...
		for (Iterator itAddCurves = additionnalCurves.iterator(); itAddCurves
				.hasNext();) {
		...
		return bestCurve != null ? bestCurve
				: (curves.size() == 0 ? new Point2D[] {} : (Point2D[]) curves
						.get(0));
	}
	 ...
	 * @return a JNote. May return <code>null</code> if exclude is true and no
	 *         notes between start and end!
	 */
	private JNoteElementAbstract getHighestNoteGlyphBetween(NoteAbstract start,
			NoteAbstract end, boolean excludeStartAndEnd) {
		...
		for (Iterator it = jnotes.iterator(); it.hasNext();) {
			...
			if (ret == null)
				...
				if (n.getBoundingBox().getMinY() < ret.getBoundingBox()
						.getMinY())
					ret = n;
			}
		}
	 ...
	 *         notes between start and end!
	 */
	private JNoteElementAbstract getLowestNoteGlyphBetween(NoteAbstract start,
			NoteAbstract end, boolean excludeStartAndEnd) {
		Collection jnotes = getNoteGlyphesBetween(start, end);
		JNoteElementAbstract ret = null;
		boolean first = true;
		for (Iterator it = jnotes.iterator(); it.hasNext();) {
			...
			if (ret == null)
				...
				if (n.getBoundingBox().getMaxY() > ret.getBoundingBox()
						.getMaxY())
					ret = n;
			}
		}
	...
	/**
	 * Returns a Collection of JNote representing the note glyphes between start
	 * and end NoteAbstract.
	 * 
	 * @param start
	 * @param end
	 * @return
	 */
	private Collection getNoteGlyphesBetween(NoteAbstract start,
			NoteAbstract end) {
		Collection jnotes = new Vector();
		try {
			Collection notes = m_music
					.getVoice(start.getReference().getVoice()).getNotesBetween(
							start, end);
	...
	private JStaffLine initNewStaffLine() {
		...
		if (m_staffLines.size() == 0) {
			...
			if (!(currentStaffLine.m_staffElements.lastElement() instanceof JBar)) {
				currentStaffLine.addElement(new JBar(new BarLine(), cursor,
						getMetrics()));
			}
		...
		if (m_music.getVoice(m_currentVoice).hasChordNames()
				|| m_music.hasPartLabel() || m_music.hasTempo()) {
			cursor.setLocation(
					cursor.getX(),
					cursor.getY()
							+ getTemplate().getAttributeSize(
		...
		if (m_staffLines.size() > 0) {
			...
			for (int i = m_index; i < voice.size(); i++) {
				...
				if (s instanceof KeySignature)
					...
					hasTimeChange = true;
				else if (s instanceof Clef)
					hasClefChange = true;
				else if ((s instanceof PartLabel) || (s instanceof Tempo))
					continue;
				else
					break;// barline, note...
			}
		...
		if ((currentKey != null) && !hasKeyChange) {
			JKeySignature sk = new JKeySignature(currentKey, previousKey,
					cursor, getMetrics());
			try {
				previousKey = (KeySignature) currentKey.clone();
			} catch (CloneNotSupportedException never) {
			...
			width = sk.getWidth();
			int cursorNewLocationX = (int) (cursor.getX() + width + getMetrics()
					.getNotesSpacing());
			cursor.setLocation(cursorNewLocationX, cursor.getY());
		}
		if ((currentTime != null) && (m_staffLines.size() == 0)
				&& !hasTimeChange) {
			try {
				JTimeSignature sig = new JTimeSignature(currentTime, cursor,
						getMetrics());
				sl.addElement(sig);
				...
				// This happens when the time signature is a bit "exotic", the
				// exception
...
/**
 * Calculates informations about a slur, and give it a mark telling if slur is
 * good or not.
 */
class SlurInfos {
	 ...
	 * 
	 * @param threePoints
	 *            The 3 points of the curve
	 * @param noteGlyphs
	 *            The glyphs ({@link JNote}) in the slur
	 * @param m_metrics
	 */
	protected SlurInfos(Point2D[] p2d, Collection noteGlyphs,
			ScoreMetrics m_metrics) throws MalFormedCurveException {
		...
		Point2D startSlurAnchorNearHead = p2d[START], endSlurAnchorNearHead = p2d[END];

		int divisor = 1;

		QuadCurve2D q = new QuadCurve2D.Float();
		q.setCurve(p2d[START],
				JTune.newControl(p2d[START], p2d[CONTROL], p2d[END]), p2d[END]);
		if ((q.getBounds2D().getMinX() < p2d[START].getX() || (q.getBounds2D()
				.getMaxX() > p2d[END].getX()))) {
			// redefine controlPoint at the center
			p2d[CONTROL] = new Point2D.Double(q.getBounds2D().getCenterX(),
					isAbove ? q.getBounds2D().getMinY() : q.getBounds2D()
							.getMaxY());
			q.setCurve(p2d[START],
		...
		// good example at
		// http://java.sun.com/products/java-media/2D/samples/suite/Arcs_Curves/Curves.java
		...
		while (!f.isDone()) {
			...
			if (segType == PathIterator.SEG_MOVETO
				...
				if (oldPts != null) {
					curveSegments.add(new Line2D.Float(oldPts[0], oldPts[1],
							pts[0], pts[1]));
				}
				oldPts = pts;
			}
		...
		for (Iterator itGlyphs = noteGlyphs.iterator(); itGlyphs.hasNext();) {
			...
			int duration = (note instanceof MultiNote) ? ((MultiNote) note)
					.getShortestNote().getDuration() : ((Note) note)
					.getDuration();
			...
			if (start) {
				startSlurAnchorNearHead = isAbove ? jnote.getSlurAboveAnchor()
						: jnote.getSlurUnderAnchor();
			} else if (!itGlyphs.hasNext()) {
				endSlurAnchorNearHead = isAbove ? jnote.getSlurAboveAnchor()
						: jnote.getSlurUnderAnchor();
			} else {
				// if (!start && itGlyphs.hasNext()) {
				// exclude first and last note from intersect checks
				// if (q.intersects(jnote.getBoundingBox()))
				// --not good : it's the whole shape between straight line
				// start-end and curve
				// that's why we check only segments of the curve
				Rectangle2D bb = jnote.getBoundingBox();
				// resize boundingBox for checking, curve must not be to close
				// than border of the box
				bb.add(bb.getCenterX(),
						bb.getMinY() - m_metrics.getSlurAnchorYOffset());
				bb.add(bb.getCenterX(),
						bb.getMaxY() + m_metrics.getSlurAnchorYOffset());
		...
		YfromStartHead = Math.abs(p2d[START].getY()
				- startSlurAnchorNearHead.getY())
