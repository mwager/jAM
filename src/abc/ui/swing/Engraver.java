...
public class Engraver implements Serializable {
	...
	/**
	 * Adapt the engraving to the tune, i.e. search for the shortest note in the
	 * tune. If shortest note is a quarter, we can reduce the space between
	 * quarter.
	 * 
	 * @param tune
	 */
	protected void adaptToTune(Tune tune, ScoreMetrics metrics) {
		...
		if (m_mode != NONE) {
			...
			short min = /*
						 * (shortestDuration >= Note.QUARTER) ? Note.SIXTEENTH :
						 */Note.SIXTEENTH;
			if (shortestDuration > min) {
				...
				if (offset != 0) {
					for (int i = durations.length - 1; i >= iShortest; i--) {
						// DEBUG
						/*
						 * String s = ""; for (Iterator itSet = set.iterator();
						 * itSet.hasNext();) { Integer i2 = (Integer)
						 * itSet.next(); System.out.print(i2+"\t"); s +=
						 * getSpaceAfter(i2.intValue())+"\t"; }
						 * System.out.println("\n"+s);
						 */
	...
	protected double getNoteSpacing(JScoreElement e) {
		...
		return (n1 == null) ? 0 : getSpaceAfter(n1.getDuration())/* * spaceFactor */;
	}
	...
	public double getSpaceAfter(int noteDuration) {
		...
		if (spacesAfter.containsKey(i))
			...
			for (Iterator itDur = spacesAfter.keySet().iterator(); itDur
					.hasNext();) {
		 ...
		 * System.out.println("getSpaceAfter("
		 * +noteDuration+") <= "+Note.SIXTY_FOURTH); return
		 * getSpaceAfter(Note.SIXTY_FOURTH); } else if (noteDuration >=
		 * Note.DOTTED_WHOLE) {
		 * System.out.println("getSpaceAfter("+noteDuration+
		 * ") >= "+Note.DOTTED_WHOLE); return getSpaceAfter(Note.DOTTED_WHOLE);
		 * }
		 */
		try {
			...
			float percent = (float) (noteDuration - bottomD)
					/ (float) (topD - bottomD);
	 ...
	 * <ul>
	 * <li>{@link #NONE} : equal space between each note
	 * <li>{@link #DEFAULT} : smaller space for short note, bigger space for
	 * long notes, something that look nice ;)
	 * </ul>
	 * 
	 * @param mode
	 */
	 ...
	 * 
	 * @param mode
	 *            {@link #DEFAULT} or {@link #NONE}
	 * @param variation
	 *            , in % negative to reduce, positive to improve space between
	 *            notes.
	 */
