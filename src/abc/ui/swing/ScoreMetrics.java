...
/**
 * This class encapsulates all requiered dimensions calculations needed to draw
 * correctly a score (notes spacing, glyph bounds...). All those values are
 * calculated from score template settings.
 */
public class ScoreMetrics {
	...
	protected Rectangle2D getBounds(char[] glyph, int notationContext) {
		String key = String.valueOf(notationContext) + "-"
				+ String.valueOf(glyph) + "-" + getMusicalFont().getName();
		if (bounds.get(key) == null) {
			FontRenderContext frc = g2.getFontRenderContext();
			bounds.put(key, new TextLayout(String.valueOf(glyph),// new
																	// Character(glyph[0]).toString(),
					getNotationFontForContext(notationContext), frc)
					.getBounds());
		}
	...
	protected Dimension getGlyphDimension(int notationContext) {
		Rectangle2D bounds = getBounds(new char[] { getMusicalFont()
				.getNoteWithoutStem(Note.QUARTER) }, notationContext);
	...
	public Font getNotationFontForContext(int notationContext) {
		try {
			Font baseNotationFont = ((MusicalFont) getTemplate()
					.getAttributeObject(ScoreAttribute.NOTATION_FONT))
					.getFont();
	...
	protected double getNotesSpacingForContext(int context) {
		...
		case NOTATION_CONTEXT_GRACENOTE:
			return getTemplate().getAttributeSize(
					ScoreAttribute.GRACENOTE_SPACING);
		case NOTATION_CONTEXT_TEMPO:
		case NOTATION_CONTEXT_NOTE:
		default:
	...
	protected BasicStroke getStemStroke() {
		return new BasicStroke(Math.max(1f,
				getTemplate().getAttributeSize(ScoreAttribute.STEM_STROKE)));
	}
	...
	/**
	 * Clear the stored bounds and other things that may have changed when
	 * template options are changed.<br>
	 * Call this method on
	 * {@link ScoreTemplateChangeListener#onTemplateChange()} implementation.
	 */
