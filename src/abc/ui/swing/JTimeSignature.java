...
class JTimeSignature extends JScoreElementAbstract {
	...
	public JTimeSignature(TimeSignature ts, Point2D base, ScoreMetrics c) {
		...
		if (!ts.isSumOfNumerators()) {
			m_numChars = getMusicalFont().getTimeSignatureDigits(
					ts.getNumerator());
		} else {
			String s_numChars = "";
			...
			for (int i = 0; i < sumOfNums.length; i++) {
				if (s_numChars.length() > 0)
					s_numChars += getMusicalFont().getDecoration(
							Decoration.PLUS);
				s_numChars += String.valueOf(getMusicalFont()
						.getTimeSignatureDigits(sumOfNums[i]));
			}
			m_numChars = s_numChars.toCharArray();
		}
		m_denomChars = getMusicalFont().getTimeSignatureDigits(
				ts.getDenominator());
		setBase(base);
	}
	...
	public double getWidth() {
		...
		return Math.max(getMetrics().getBounds(m_numChars).getWidth(),
	...
	protected void onBaseChanged() {
		// FIXME what if the signature numbers are not supported ? =>
		// arrayOutOfBounds ! :/
	...
	public double render(Graphics2D context) {
		...
		context.drawChars(
				...
				0,
				m_numChars.length,
				(int) (getBase().getX() + width / 2 - (getMetrics().getBounds(
						m_numChars).getWidth()) / 2), m_topNumY);
		context.drawChars(
				m_denomChars,
				0,
				m_denomChars.length,
				(int) (getBase().getX() + width / 2 - (getMetrics().getBounds(
						m_denomChars).getWidth()) / 2), m_bottomNumY);
