...
public class JAnnotation extends JText {
	...
	protected void onAttachmentChanged() {
		...
		box = box.createUnion(new Rectangle2D.Double(box.getMinX(), element
				.getStaffLine().get5thLineY(), box.getWidth(), element
				.getStaffLine().getHeight()));
		...
		case VerticalPosition.MIDDLE:
			...
			if (element instanceof JNoteElementAbstract)
				y = ((JNoteElementAbstract) element).getNotePosition().getY()
		...
		case HorizontalPosition.RIGHT:
			...
			if (tJustify == TextJustification.LEFT) {
				x = box.getMaxX() + getMetrics().getNoteWidth() / 2;
			}
			break;
