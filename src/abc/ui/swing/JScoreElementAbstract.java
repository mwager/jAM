...
/**
 * This class defines a score rendition element. Rendition scores elements are
 * graphical representations of tune score elements objects retrieved from a
 * tune object. A <TT>JScoreElement</TT> can itself contain
 * <TT>JScoreElement</TT> instances. You should figure out the graphical score
 * representation as a tree of <TT>JScoreElement</TT>. (Composite)
 * 
 * @see Tune#getMusic()
 */
abstract class JScoreElementAbstract implements JScoreElement {
	...
	/**
	 * Teh bounding box (more or less humm....) that encapsulates this graphical
	 * score element.
	 */
	...
	public Rectangle2D getBoundingBox() {
		Rectangle2D bb = new Rectangle2D.Double(m_base.getX(),
				m_base.getY() - 50, getWidth(), 50);
		return bb;
	}
	 ...
	 * @param location
	 *            A location.
	 * @return The score element whose bouding box contains the given location.
	 *         <TT>this</TT> can be returned or one of the sub
	 *         <TT>JScoreElement</TT> contained in this one. <TT>null</TT> is
	 *         returned if no matching element has been found.
	 */
	 ...
	 * 
	 * @return The base of this element. The base is the point that is used as a
	 *         reference to draw the element at this location.
	 * @see #setBase(Point2D)
	 */
	 ...
	 * 
	 * @param base
	 *            The new bas that should be used to draw this element.
	 * @see #getBase()
	 */
	...
	protected void calcDecorationPosition() {
		if (m_jDecorations != null && m_jDecorations.size() > 0) {
			...
			m_decorationAnchors[JDecoration.ABOVE_NOTE] = m_decorationAnchors[JDecoration.ABOVE_STAFF] = m_decorationAnchors[JDecoration.ABOVE_STAFF_AFTER_NOTE] = m_decorationAnchors[JDecoration.STEM_END] = m_decorationAnchors[JDecoration.STEM_END_OUT_OF_STAFF] = m_decorationAnchors[JDecoration.VERTICAL_NEAR_STEM] = m_decorationAnchors[JDecoration.VERTICAL_NEAR_STEM_OUT_STAFF] = new Point2D.Double(
					x, top);
			m_decorationAnchors[JDecoration.UNDER_NOTE] = m_decorationAnchors[JDecoration.UNDER_STAFF] = m_decorationAnchors[JDecoration.VERTICAL_AWAY_STEM] = m_decorationAnchors[JDecoration.VERTICAL_AWAY_STEM_OUT_STAFF] = new Point2D.Double(
					x, bottom);
			m_decorationAnchors[JDecoration.HORIZONTAL_AWAY_STEM] = m_decorationAnchors[JDecoration.HORIZONTAL_NEAR_STEM] = m_decorationAnchors[JDecoration.STEM_MIDDLE] = m_decorationAnchors[JDecoration.MIDDLE_STAFF] = new Point2D.Double(
					x, middle);
	 ...
	 * 
	 * @param color
	 *            The color used for the rendition of this score element.
	 */
	/*
	...
	/**
	 * Set the color for renderer, get color value in the score template, or
	 * apply the specified color for the current element.
	 * 
	 * @param g2
	 * @param scoreElement
	 */
	 ...
	 * 
	 * @param c
	 *            <code>null</code> to use the default color
	 */
	...
	protected void renderChordName(Graphics2D gfx) {
		...
		if (chord != null) {
			...
			if (this instanceof JNote)
				...
				displayPos = ((JChord) this).getHighestNote()
						.getDisplayPosition();
	...
	protected void renderDebugBoundingBoxOuter(Graphics2D context) {
		...
		bb.setRect(bb.getX() - 1, bb.getY() - 1, bb.getWidth() + 2,
				bb.getHeight() + 2);
		context.draw(bb);
		context.setColor(previousColor);
	}
		...
		for (int i = 0; i < m_decorationAnchors.length; i++) {
			if (m_decorationAnchors[i] != null) {
				context.drawOval((int) m_decorationAnchors[i].getX(),
						(int) m_decorationAnchors[i].getY(), 1, 1);
			}
		}
		context.setColor(previousColor);
	}
	...
	public String toString() {
		...
		if (getMusicElement() != null)
			ret += " <" + getMusicElement().toString() + " ref"
