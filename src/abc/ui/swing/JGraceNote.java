...
class JGraceNote extends JNote implements JGraceElement {
	...
	/**
	 * used to request glyph-specific metrics in a genric way that enables
	 * positioning, sizing, rendering to be done generically
	 * <p>
	...
	public double render(Graphics2D context) {
		...
		try {
			context.setFont(getMetrics().getNotationFontForContext(
					ScoreMetrics.NOTATION_CONTEXT_GRACENOTE));
			super.render(context);

			// visual testing of base/offset/note positions
			/*
			 * Color previousColor = context.getColor();
			 * context.setColor(Color.RED); double nh =
			 * m_metrics.getStaffLineHeight(); double offset = getOffset(note);
			 * context.drawRect((int)(m_base.getX()), (int)(m_base.getY()),1,
			 * 1); context.setColor(Color.GREEN);
			 * context.drawRect((int)(m_base.getX()),
			 * (int)(m_base.getY()+offset),1, 1);
		...
		if (renderSlash) {
			...
			context.drawLine((int) slashStart.getX(), (int) slashStart.getY(),
					(int) slashEnd.getX(), (int) slashEnd.getY());
