...
class JNote extends JNoteElementAbstract {
	...
	/**
	 * This position is independant from the note type (height or rhythm) and,
	 * as a way of consequence, can be used to render elements such as sharp,
	 * flats etc around the note. This is the base to compute the
	 * accidentalsPosition, the slurs positions etc.
	 */
	protected Point2D notePosition = null;
	/**
	 * This position is used to render the note character in the score. This
	 * position may (relatively) differ from one note to another and cannot be
	 * used as a stable reference position to draw elements around the note.
	 */
	protected Point2D displayPosition = null;
	/**
	 * Position to draw accidentals. <TT>null</TT> if no accidental for this
	 * note.
	 */
	protected Point2D accidentalsPosition = null;
	/**
	 * Position to draw the graces notes, if graces are before the note, this
	 * position will be getBase
	 */
	...
	public JNote(Note noteValue, Clef clef, Point2D base, ScoreMetrics c) {
		...
		if (!note.isRest()) {
			...
			if ((getClef() == null)
					|| noteValue.isLowerThan(getClef().getMiddleNote())) {
				setStemUp(true);
			} else {
				setStemUp(false);
			}
	...
	protected void valuateNoteChars() {
		...
		if (note.isRest()) {
			...
			if (isStemUp())
				noteChars = new char[] { getMusicalFont().getNoteStemUpChar(
						noteDuration) };
			else
				noteChars = new char[] { getMusicalFont().getNoteStemDownChar(
						noteDuration) };
		}
	}

	/**
	 * Sets the Unicode values of the stem [0] and the note head [1] for head
	 * inverted note
	 * 
	 * @return char[2][]
	 */
	protected char[][] valuateInvertedNoteChars() {
		...
		if (note.isRest()) {
			...
			if (noteDuration <= Note.HALF) {
				if (isStemUp())
					c1 = new char[] { getMusicalFont()
							.getStemWithoutNoteUpChar(noteDuration) };
				else
					c1 = new char[] { getMusicalFont()
							.getStemWithoutNoteDownChar(noteDuration) };
			}
	...
	/**
	 * Moves the note to a new position in order to set the new stem begin
	...
	public void setStemBeginPosition(Point2D newStemBeginPos) {
		...
		// System.out.println("translating " + this + " with " + xDelta + "/" +
		// yDelta);
		Point2D newBase = new Point2D.Double(getBase().getX() + xDelta,
				getBase().getY() + yDelta);
		setBase(newBase);
	}

	/**
	 * in a genric way that enables positioning, sizing, rendering to be done
	 * generically
	 * <p>
	...
	protected void onBaseChanged() {
		...
		Dimension glyphDimension = metrics
				.getGlyphDimension(getNotationContext());
		// note glyph is used for vertical position of graces notes
		Dimension noteGlyphDimension = metrics
				.getGlyphDimension(ScoreMetrics.NOTATION_CONTEXT_NOTE);
		...
		if (note.isRest())
			...
			noteY = (int) (base.getY() - getOffset(note)
					* noteGlyphDimension.getHeight()
					- noteGlyphDimension.getHeight() / 2 + glyphDimension
					.getHeight() / 2);
		...
		if (m_jGracenotes != null) {
			if (note.getGracingType() == GracingType.ACCIACCATURA)
				graceNotesAfter = getTemplate().getAttributeBoolean(
						ScoreAttribute.ACCIACCATURA_AFTER_NOTE);
		...
		try {
			if (note.hasAccidental()) {
				accidentalsChars = new char[] { getMusicalFont().getAccidental(
						note.getAccidental()) };
				accidentalsWidth = getMetrics().getBounds(accidentalsChars,
						getNotationContext()).getWidth()
						+ Math.max(
								3,
								getTemplate().getAttributeSize(
										ScoreAttribute.ACCIDENTAL_SPACE));
				accidentalsWidth *= Math.floor(getAccidentalPositionOffset());
				if (getAccidentalPositionOffset() != Math
						.floor(getAccidentalPositionOffset())) {
					accidentalsWidth += glyphDimension.getWidth();
				}
			}
		} catch (IllegalArgumentException iae) {
			throw new IllegalArgumentException("Incorrect accidental for "
					+ note + " : " + note.getAccidental());
		}

		// System.out.println("note chars " + noteChars[0]);
		// double noteY =(int)(base.getY()-getOffset(note)*c.getNoteHeigth());
		// double noteY
		// =(int)(base.getY()-getOffset(note)*glyphDimension.getHeigth());
		double noteX = base.getX() + (graceNotesAfter ? 0 : graceNotesWidth)
				+ accidentalsWidth;
		if (note.hasAccidental())
			accidentalsPosition = new Point2D.Double(base.getX()
					+ (graceNotesAfter ? 0 : graceNotesWidth), noteY
		...
		if (!isHeadInverted()) {
			stemUpBeginPosition = new Point2D.Double(notePosition.getX()
					+ glyphDimension.getWidth() * 0.93, notePosition.getY()
					- glyphDimension.getHeight() / 2);
		...
		// if (note.getHeight()>=Note.c && note.getStrictDuration() <
		// Note.EIGHTH) {
		if (note.isHigherThan(getClef().getMiddleNote())
				&& note.getStrictDuration() < Note.EIGHTH) {
			stemUpBeginPosition
					.setLocation(
							stemUpBeginPosition.getX(),
							stemUpBeginPosition.getY()
									- glyphDimension.getHeight() / 2);
			stemDownBeginPosition.setLocation(stemDownBeginPosition.getX(),
					stemDownBeginPosition.getY() - glyphDimension.getHeight()
							/ 2);
		}
		...
		if (((this instanceof JNote) || (this instanceof JChordNote))
				...
				&& !note.isRest()
				&& ((isStemUp() && ((note.getStrictDuration() <= Note.EIGHTH) || isHeadInverted())) || (!isStemUp() && isHeadInverted()))
				&& ((noteChars[0] != getMusicalFont().getNoteWithoutStem()) || isHeadInverted())) {
			extraWidth = glyphDimension.getWidth();
		}
		// calc dots needed extra space
		calcDotsPosition();
		if (dotsPosition != null) {
			double extraWidthDots = dotsPosition[dotsPosition.length - 1]
					.getX()
					- notePosition.getX()
					- glyphDimension.getWidth()
					+ metrics.getBounds(getMusicalFont().getDot(),
							getNotationContext()).getWidth();
		...
		if (m_jGracenotes != null) {
			if (!graceNotesAfter)
				...
				gracesPosition = new Point2D.Double(noteX
						+ glyphDimension.getWidth() + extraWidth
						+ getMetrics().getGraceNotesSpacing() * 2, getBase()
						.getY());
			}
		}

		m_width = (int) (graceNotesWidth + accidentalsWidth
				+ glyphDimension.getWidth() + extraWidth);
	...
	protected void calcDotsPosition() {
		if (note.countDots() != 0) {
			...
			Dimension glyphDimension = getMetrics().getGlyphDimension(
					getNotationContext());
	...
	protected void calcSlursAndTiesPosition() {
		...
		Dimension glyphDimension = metrics
				.getGlyphDimension(getNotationContext());
		slurUnderAnchor = new Point2D.Double(displayPosition.getX()
				+ glyphDimension.getWidth() / 2, notePosition.getY()
				+ metrics.getSlurAnchorYOffset());
		slurAboveAnchor = new Point2D.Double(displayPosition.getX()
				+ glyphDimension.getWidth() / 2, notePosition.getY()
				- glyphDimension.getHeight() - metrics.getSlurAnchorYOffset());
		// TODO improve position for rests
		slurUnderAnchorOutOfStem = (isStemUp() && !note.isRest()) ? slurUnderAnchor
				: new Point2D.Double(stemDownBeginPosition.getX(),// getBoundingBox().getMinX(),
						getBoundingBox().getMaxY()
								+ metrics.getSlurAnchorYOffset());
		slurAboveAnchorOutOfStem = (isStemUp() && !note.isRest()) ? new Point2D.Double(
				stemUpBeginPosition.getX(),// getBoundingBox().getMaxX(),
				getBoundingBox().getMinY() - metrics.getSlurAnchorYOffset())
				: slurAboveAnchor;
		Rectangle2D tieAnchors = new Rectangle2D.Double(notePosition.getX(),
				notePosition.getY() - glyphDimension.getHeight(),
				glyphDimension.getWidth(), glyphDimension.getHeight());
		// ties start and end in a corner of the glyph rectangle
		tieStartAboveAnchor = new Point2D.Double(tieAnchors.getMaxX(),
				tieAnchors.getMinY());
		tieStartUnderAnchor = new Point2D.Double(tieAnchors.getMaxX(),
				tieAnchors.getMaxY());
		tieEndAboveAnchor = new Point2D.Double(tieAnchors.getMinX(),
				tieAnchors.getMinY());
		tieEndUnderAnchor = new Point2D.Double(tieAnchors.getMinX(),
				tieAnchors.getMaxY());
	}
	...
	public Rectangle2D getBoundingBox() {
		...
		if (note.isRest()) {
			return new Rectangle2D.Double(getBase().getX(), getBase().getY()
					- getMetrics().getStaffCharBounds().getHeight(), m_width,
	...
	protected void renderNoteChars(Graphics2D gfx) {
		if (!(note.isRest() && note.isRestInvisible())) {
			if (note.isRest() || !isHeadInverted()) {
				gfx.drawChars(noteChars, 0, 1, (int) displayPosition.getX(),
						(int) displayPosition.getY());
			} else {
				char[][] chars = valuateInvertedNoteChars();
				if (chars[0] != null) // stem
					gfx.drawChars(chars[0], 0, 1, (int) displayPosition.getX(),
							(int) displayPosition.getY());
				if (chars[1] != null) // head
					gfx.drawChars(chars[1], 0, 1, (int) notePosition.getX(),
							(int) notePosition.getY());
			}
		}
	}
	...
	/**
	 * Returns true if the note is on staff line (or extended line), e.g. in G
	 * clef, C E G B d f a c'...
	 * 
	...
	private boolean isOnStaffLine() {
		...
		/*
		 * switch(note.getStrictHeight()) { case Note.C: case Note.E: case
		 * Note.G: case Note.B: return (note.getOctaveTransposition() % 2) == 0;
		 * default: //D F A return (note.getOctaveTransposition() % 2) == 1; }
		 * //}
		 */
	}

	protected void renderExtendedStaffLines(Graphics2D context,
			ScoreMetrics metrics, Point2D base) {
		...
		Dimension glyphDimension = metrics
				.getGlyphDimension(getNotationContext());
		// FIXME Breve and long are longer, so draws a longer extended staff
		// line
		if (note.getStrictDuration() == Note.BREVE
				|| note.getStrictDuration() == Note.LONG) {
			Rectangle2D bounds = metrics.getBounds(
					new char[] { getMusicalFont().getNoteWithoutStem(
							note.getStrictDuration()) }, getNotationContext());
		...
		if (note.getHeight() <= low.getHeight()) {
			...
			int currentPosition = (int) (base.getY() - 1 - currentOffset
					* noteGlyphDimension.getHeight() / 1.5);
			double offset = getOffset(note);
			Stroke dfs = context.getStroke();
			context.setStroke(metrics.getStemStroke());
			while (currentOffset >= offset) {
				context.drawLine((int) (displayPosition.getX() - extSize),
						currentPosition, (int) (displayPosition.getX()
								+ glyphDimension.getWidth() + extSize),
						currentPosition);
				currentOffset--;
				currentPosition = (int) (currentPosition + noteGlyphDimension
						.getHeight());
			...
			int currentPosition = (int) (base.getY() - 1 - currentOffset
					* noteGlyphDimension.getHeight() - noteGlyphDimension
					.getHeight() / 2);
			double offset = getOffset(note);
			Stroke dfs = context.getStroke();
			context.setStroke(metrics.getStemStroke());
	...
	protected void renderDots(Graphics2D context) {
		if (dotsPosition != null) {
			char[] glyph = new char[] { getMusicalFont().getDot() };
			for (int i = 0; i < dotsPosition.length; i++) {
				context.drawChars(glyph, 0, 1, (int) dotsPosition[i].getX(),
						(int) dotsPosition[i].getY());
			}
		}
	}
	...
	public Point2D getStemBeginPosition() {
		...
		/*
		 * new Point2D.Double(notePosition.getX()+
		 * m_metrics.getNoteWidth()*0.93,
	...
	/**
	 * @param stemDownBeginPosition
	 *            The stemDownBeginPosition to set.
	 */
	...
	/**
	 * @param stemUpBeginPosition
	 *            The stemUpBeginPosition to set.
	 */
	 ...
	 * 
	 * @param i
	 *            the offset expressed in accidental width (default 1)
	 */
