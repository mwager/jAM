 ...
 * <li>positions are denoted by a couple of byte from {@link VerticalPosition}
 * and {@link HorizontalPosition} constants.
 ...
 * </ul>
 * 
 * A score template defines also notation font size, margins, space between 2
 * staves...
 * 
...
public abstract class ScoreTemplate implements Cloneable, Serializable {
	...
	private class Position implements Cloneable, Serializable {
		...
		/*
		 * Position(String s) { this(); if ((s.indexOf('[') != -1) &&
		 * (s.indexOf(']') != -1)) { s = s.substring(s.indexOf('[')+1,
		 * s.indexOf(']')); } if (s.indexOf(';') != -1) { String[] s2 =
		 * s.split(";"); if (s2.length == 2) { m_vertical =
		 * VerticalPosition.toVerticalAlign(s2[0]); m_horizontal =
		 * HorizontalAlign.toHorizontalAlign(s2[1]); } } }
		 */
		public boolean equals(Object o) {
	 ...
	 *             if sa doesn't accept boolean value
	 */
	...
	/**
	 * <B>For the real size, see {@link #getAttributeSize(ScoreAttribute)}!</B> <br>
	...
	public float getAttributeSize(ScoreAttribute sa)
		...
		if ((u.equals(SizeUnit.PX)) || (u.equals(SizeUnit.PT))) {
			...
			return (float) (size * getMetrics().getStaffCharBounds().getWidth());
	...
	public float getDefaultTextSize() {
		ScoreAttribute.Size s = (ScoreAttribute.Size) getAttributeObject(ScoreAttribute.TEXT_DEFAULT_SIZE);
	 ...
	 * 
	 * @param scoreElement
	 *            One of the {@link ScoreElements} constant
	 * @return a Color object, or <TT>null</TT> if no default color has been
	 *         set.
	 */
	public Color getElementColor(byte scoreElement) {
		Object ret = m_attributes.get("COLOR_" + Byte.toString(scoreElement));
		if (ret == null) {
			ret = m_attributes.get("COLOR_"
					+ Byte.toString(ScoreElements._DEFAULT));
			if (ret == null)
	...
	private byte[] getSectionFields(byte verticalSection) {
		...
		byte[] left = getFieldsAtPosition(verticalSection,
	 ...
	 * @param field
	 *            one of {@link ScoreElements} constants
	 * @return Map of attributes, <TT>null</TT> possible
	...
	public Font getTextFont(byte field) {
		...
		Vector v = new Vector(m_defaultTextFontFamilyNames.length
				+ fi.m_fontFamilyNames.length);
	 ...
	 * @param field
	 *            one of {@link ScoreElements} constants
	 * @return style from Font class : {@link Font#PLAIN}, {@link Font#BOLD},
	 *         {@link Font#ITALIC} or Font.BOLD+Font.ITALIC
	 */
	...
	private boolean isFontAvailable(String fontName) {
		...
		if (m_fontAvailability.get(fontName) == null) {
			boolean b = false;
			String[] availableFamily = GraphicsEnvironment
	...
	/**
	 * Return <TT>true</TT> if the field is visible and should be displayed in
	 * score.
	 * 
	 * @param field
	 *            one of {@link ScoreElements} constants
	 ...
	 * @param sa
	 * @param value
	 *            <TT>null</TT> to remove this attribute
	 */
	...
	/**
	 * Sets the size associated to an attribute, using the attribute's default
	 * unit
	 * 
	 * @param sa
	 * @param size
	 ...
	 * @param size
	 * @param unit
	 *            {@link SizeUnit#PX}, {@link SizeUnit#STAFF_HEIGHT}...
	 * @throws RuntimeException
	 *             if attribute doesn't accept size value
	 */
	 ...
	 * 
	 * if text sizes are expressed in percent, all sizes will be calculated from
	 * default size.
	 * 
	 * @param f
	 */
	 ...
	 *            {@link #getTextSizeExpressedIn()} is {@link #PERCENT}, else in
	 *            pt
	 * @param unit
	...
	/**
	 * Sets the prefix of a field, e.g. "Source:" for source field.
	 * 
	 * @param field
	 *            one of {@link ScoreElements} constants
	 * @param s
	 ...
	 *            one of {@link ScoreElements} constants
	 * @param style
	 *            style from Font class : {@link Font#PLAIN}, {@link Font#BOLD},
	 *            {@link Font#ITALIC} or Font.BOLD+Font.ITALIC
	 */
	...
	public String toString() {
		...
		ret.append("\tdefault text font face: "
				+ Arrays.toString(m_defaultTextFontFamilyNames) + ";\n");
