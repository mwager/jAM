...
/**
 * This class is in charge of rendering a group of notes whose stems should be
 * linked.
 */
class JGroupOfNotes extends JScoreElementAbstract implements JStemmableElement {
	...
	/**
	 * All the notes rendition elements that are part of the group. chords and /
	 * or notes
	 */
	...
	public JGroupOfNotes(ScoreMetrics metrics, Point2D base,
			NoteAbstract[] notes, Clef clef, Engraver engrav) {
		super(metrics);
		m_clef = clef;
		if (notes.length <= 1)
			throw new IllegalArgumentException(notes
					+ " is not a group of notes, length = " + notes.length);
		m_engraver = engrav;
		m_notes = new NoteAbstract[notes.length];
		// create JNotePartOfGroup instances. Those instance should stay the
		// same
		...
		for (int i = 0; i < notes.length; i++)
			if (notes[i] instanceof Note) {
				...
				m_jNotes[i] = new JNotePartOfGroup((Note) m_notes[i], clef,
						base, getMetrics());
				// anchorNotes[i] = (Note)notes[i];
			} else {
				// This is a multiNote
				m_jNotes[i] = new JChordPartOfGroup((MultiNote) notes[i], clef,
						getMetrics(), new Point2D.Double());
	...
	/**
	 * A JChordPartOfGroup in this group of note can have multi-length chords,
	 * then only one is part of group (the shortest duration' one)
	 * 
	 * So, for auto stemming, stem direction... functions, if the
	 * JNoteElementAbstract is a JChord(PartOfGroup), returns the really needed
	 * chord, itself if one-length, the shortest if multi-length
	 * 
	 * @param chord
	 */
	...
	public boolean isAutoStem() {
		if (autoStemming) {
			for (int i = 0; i < m_jNotes.length; i++) {
				...
				// multi-length chord, there is only one part of a group (the
				// shortest)
	...
	/**
	 * When the score is justified, avoid big gaps between group of notes, by
	 * enlarging the JGroupOfNote by adding a space between each note of the
	 * group.
	 * 
	 * @param d
	 *            <= 0 to cancel justification
	 */
	...
	protected void onBaseChanged() {
		...
		JGroupableNote highestNote = m_jNotes[highIndex];// (m_notes); Notestsn
															// = new
															// JNotePartOfGroup(highestNote,
															// getBase(),
															// m_metrics);
		JGroupableNote lowestNote = m_jNotes[lowIndex];// (m_notes); Notestsn =
														// new
														// JNotePartOfGroup(lowestNote,
														// getBase(),
														// m_metrics);
		...
		if (isAutoStem()) {
			...
			if (m_notes[highIndex] instanceof MultiNote)
				h = ((MultiNote) m_notes[highIndex]).getHighestNote()
						.getHeight();
			else
			...
			if (h <= midHeight) {
				...
				if ((h - midHeight) < (midHeight - l)) {
					// lowest note is further away from center line than highest
					// note
					setStemUp(true);
				} else {
					setStemUp(false);
		...
		if (isStemUp) {
			// update the highest note to calculate when the stem Y end should
			// be after the base change.
			...
			m_stemYend = (int) (highestNote.getStemBeginPosition().getY()
					- getMetrics().getNoteHeight() / 2 - getMetrics()
					.getStemLengthForContext(getNotationContext()));
		} else {
			// update the lowest note to calculate when the stem Y end should be
		...
		setStemYEnd(m_stemYend); // apply the same stemYEnd to all notes in
									// group
		...
		for (int i = 0; i < m_jNotes.length; i++) {
			...
			// gracenote group have fixed spacing, so only use engraver spacing
			// if engraver is not null
			...
			double justifySpacingCorrection = engraverSpacing
					+ getMetrics().getNotesSpacingForContext(
							getNotationContext());
			if (m_internalSpacingRatio > 0)
				justifySpacingCorrection *= m_internalSpacingRatio;
			currentBase.setLocation(currentBase.getX() + m_jNotes[i].getWidth()
			// + getMetrics().getNotesSpacing()
			// + engraverSpacing,
					+ justifySpacingCorrection, getBase().getY());
			// }
		}
		if (lastNote == null)
			lastNote = firstNote;
		// double firstNoteAccidentalWidth =
		// (firstNote.getWidth()-getMetrics().getNoteWidth());

		/*
		 * if (isStemUp) { m_width =
		 * (int)(lastNote.getStemBeginPosition().getX()
		 * -(firstNote).getBase().getX() + firstNoteAccidentalWidth); } else {
		 * m_width =
		 * (int)(lastNote.getStemBeginPosition().getX()+getMetrics().getNoteWidth
	...
	public double render(Graphics2D context) {
		...
		for (int i = 0; i < m_jNotes.length; i++) {
			...
			BasicStroke notesLinkStroke = getMetrics()
					.getNotesLinkStrokeForContext(getNotationContext());
			context.setStroke(notesLinkStroke);
			short[] longerRhythms = null;
			short noteStrictDuration = (m_notes[i] instanceof MultiNote) ? ((MultiNote) m_notes[i])
					.getShortestNote().getStrictDuration()
					: ((Note) m_notes[i]).getStrictDuration();
			switch (noteStrictDuration) {
			case Note.EIGHTH:
				longerRhythms = new short[] { Note.EIGHTH };
				break;
			case Note.SIXTEENTH:
				longerRhythms = new short[] { Note.EIGHTH, Note.SIXTEENTH };
				break;
			...
			for (int j = 0; j < longerRhythms.length; j++) {
				...
				if (longerRhythms[j] == Note.EIGHTH)
					noteLinkY = (int) (m_stemYend + factor
							* notesLinkStroke.getLineWidth() * 0.5);
				else if (longerRhythms[j] == Note.SIXTEENTH)
					noteLinkY = (int) (m_stemYend + factor
							* notesLinkStroke.getLineWidth() * 2);
				else if (longerRhythms[j] == Note.THIRTY_SECOND)
					noteLinkY = (int) (m_stemYend + factor
							* notesLinkStroke.getLineWidth() * 3.5);
				else if (longerRhythms[j] == Note.SIXTY_FOURTH)
					noteLinkY = (int) (m_stemYend + factor
							* notesLinkStroke.getLineWidth() * 5);
				...
				if (hasNext) {
					nextNoteIsShorterOrEquals = ((m_notes[i + 1] instanceof MultiNote) ? ((MultiNote) m_notes[i + 1])
							.getShortestNote().getStrictDuration()
							: ((Note) m_notes[i + 1]).getStrictDuration()) <= longerRhythms[j];
				}
				if (hasPrevious) {
					previousNoteIsShorterOrEquals = ((m_notes[i - 1] instanceof MultiNote) ? ((MultiNote) m_notes[i - 1])
							.getShortestNote().getStrictDuration()
							: ((Note) m_notes[i - 1]).getStrictDuration()) <= longerRhythms[j];
				}
				if (hasPrevious) {
					if (previousNoteIsShorterOrEquals)
						...
						noteLinkEnd = (int) ((JGroupableNote) m_jNotes[i - 1])
								.getStemBeginPosition().getX();// getE
																// (int)(stemX-2*context.getNoteWidth());
					else if (!(hasNext && nextNoteIsShorterOrEquals))
						noteLinkEnd = (int) (m_jNotes[i].getStemBeginPosition()
								.getX() - getMetrics().getNoteWidth() / 1.2);
				} else if (!nextNoteIsShorterOrEquals)
					noteLinkEnd = (int) (m_jNotes[i].getStemBeginPosition()
							.getX() + getMetrics().getNoteWidth() / 1.2);
				if (noteLinkEnd != -1)
					context.drawLine((int) m_jNotes[i].getStemBeginPosition()
							.getX(), noteLinkY, noteLinkEnd, noteLinkY);
			}
		...
		if (nUpletSize != -1) {
			...
			float tupletNumberYOffset = getTemplate().getAttributeSize(
					ScoreAttribute.TUPLET_NUMBER_Y_OFFSET);
			Rectangle2D numberBounds = getMetrics().getBounds(chars);
			// TODO replace with commented line but needs to be improved because
			// of the get display position.
			// context.drawChars(chars, 0, 1,
			// (int)(((JNote)m_jNotes[0]).getDisplayPosition().getX()+m_width/2),
			// (int)(m_stemYend - m_metrics.getNoteHeigth()/4));
			Point2D ctrlP = null;
			if (isStemUp) {
				ctrlP = new Point2D.Double(m_jNotes[0].getBase().getX()
						+ getWidth() / 2, m_stemYend
						- getTemplate().getAttributeSize(
								ScoreAttribute.TUPLET_NUMBER_Y_OFFSET));
				context.drawChars(chars, 0, 1, (int) ctrlP.getX(),
						(int) ctrlP.getY());
			} else {
				...
				Line2D line = new Line2D.Double(
						((JNoteElementAbstract) m_jNotes[0])
								.getSlurAboveAnchor(),
						((JNoteElementAbstract) m_jNotes[m_jNotes.length - 1])
								.getSlurAboveAnchor());
				...
				if (intersects) { // control point above bounds of the group
					ctrlP = new Point2D.Double(m_jNotes[0]
							.getStemBeginPosition().getX()
							+ (getWidth() - getMetrics().getNoteWidth()) / 2,
			...
			ctrlP.setLocation(ctrlP.getX() + numberBounds.getWidth() / 2,
					ctrlP.getY() - numberBounds.getHeight()
							- tupletNumberYOffset);
