...
public class JTempo extends JText {
	...
	protected JTempo(ScoreMetrics mtrx, Point2D base, Tempo tempo) {
		...
		if (m_dotted) {
			...
			m_refNote = new char[] { getMusicalFont().getNoteStemUpChar(
					refLength) };
		}
		...
		m_noteHeadBounds = getMetrics().getBounds(
				...
				ScoreMetrics.NOTATION_CONTEXT_TEMPO);
		m_number = new String(getMusicalFont().getTimeSignatureDigits(
				m_tempo.getNotesNumberPerMinute()));
	...
	public Rectangle2D getBoundingBox() {
		return new Rectangle2D.Double(getBase().getX(), m_y, getTextWidth(),
				getHeight());
	}
	...
	public double getHeight() {
		...
		return m_noteHeadBounds.getHeight()
	...
	private double getTextWidth() {
		return m_refNoteBounds.getWidth() + m_numberBounds.getWidth()
	...
	public double render(Graphics2D g2) {
		...
		g2.drawString(String.valueOf(m_refNote), (int) (getBase().getX()),
				(int) (m_y + noteHeadHeight));
		g2.drawString(
				m_number,
				(int) (getBase().getX() + m_refNoteBounds.getWidth() + 3 * noteHeadWidth),
				(int) (m_y + noteHeadHeight - m_numberBounds.getHeight() / 2));
		// render the equals sign
		int xEquals = (int) (getBase().getX() + m_refNoteBounds.getWidth() + noteHeadWidth);
		int yEquals = (int) (m_y + noteHeadHeight / 2);
		g2.drawLine(xEquals, yEquals, (int) (xEquals + noteHeadWidth), yEquals);
