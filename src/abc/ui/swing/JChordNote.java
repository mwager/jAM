...
class JChordNote extends JNotePartOfGroup {
		...
		if (!isAnchor()) {
			...
			if (isStemUp())
				noteChars = new char[] { getMusicalFont().getNoteStemUpChar(
						((Note) getMusicElement()).getStrictDuration()) };
			else
	...
	public double render(Graphics2D context) {
		 ...
		 * (int)getStemBeginPosition().getY(), (int)getStemEndPosition().getX(),
		 * (int)getStemEndPosition().getY()); context.setColor(previousColor);
		 */
		return getWidth();
	}
	...
	/**
	 * Sets to <TT>true</TT> to tells that this is note is the highest in the
	 * chord
	 */
	...
	/**
	 * Sets to <TT>true</TT> to tells that this is note is the lowest in the
	 * chord
	 */
