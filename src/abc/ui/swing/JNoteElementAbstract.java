 ...
 */
abstract class JNoteElementAbstract extends JScoreElementAbstract implements
		JStemmableElement {
	...
	protected Point2D slurUnderAnchorOutOfStem = null,
			slurAboveAnchorOutOfStem = null;
	...
	/**
	 * Stem direction will be determined automatically based on note value.
	 * Notes B or higher will be stemed down, Notes A or lower will be stemed
	 * up. True by default.
	 */
	// private attribute so all classes are forced to accessor methods
	// this ensures autoStemming is done correctly
	...
	public JNoteElementAbstract(NoteAbstract noteValue, Clef clef,
			Point2D base, ScoreMetrics metrics) {
		super(base, metrics);
		m_clef = clef;
		...
		if (noteValue.hasGracingNotes()) {
			...
			if (graceNotes.length == 1) {
				m_jGracenotes = new JGraceNote(graceNotes[0], clef, base,
						getMetrics());
				base.setLocation(base.getX() + m_jGracenotes.getWidth(),
						base.getY());
			} else if (graceNotes.length > 1) {
				m_jGracenotes = new JGroupOfGraceNotes(getMetrics(), base,
						graceNotes, clef, null);
			...
			case GracingType.ACCIACCATURA:
				renderSlash = getTemplate().getAttributeBoolean(
						ScoreAttribute.ACCIACCATURA_SLASH);
				break;
			case GracingType.APPOGGIATURA:
			default:
				renderSlash = getTemplate().getAttributeBoolean(
						ScoreAttribute.APPOGGIATURA_SLASH);
				break;
			}
		...
		if (noteValue.hasDecorations()) {
			...
			for (int i = 0; i < decorations.length; i++) {
				if ((decorations[i].getType() == Decoration.ROLL)
					...
					addDecoration(new JDecoration(new Decoration(
							Decoration.GENERAL_GRACING), getMetrics()));
				} else {
	...
	public boolean isStemUp() {
		...
		if (autoStem && (note != null)) {
			...
			if ((getClef() == null)
					|| note.isLowerThan(getClef().getMiddleNote())) {
				isup = true;
			} else {
				isup = false;
			}
	...
	/**
	 * Sets if the note head is inverted (i.e. right to stem if stem is up, left
	 * to stem if stem is down)
	 * 
	 * @param b
	 */
	...
	/**
	 * Returns if the note head is inverted (i.e. right to stem if stem is up,
	 * left to stem if stem is down)
	 */
	...
	protected void calcDecorationPosition() {
		...
		if (m_jDecorations != null && m_jDecorations.size() > 0) {
			...
			double noteHeadX = lowest.getNotePosition().getX() + noteWidth
					* 0.5;
			double aboveNoteHeadY = highest.getNotePosition().getY()
					- noteHeight * 1.5;
			double underNoteHeadY = lowest.getNotePosition().getY()
					+ noteHeight * 0.5;
			double stemX = lowest.getStemBeginPosition().getX();
			double endStemY = isStemUp ? (bb.getMinY() - noteHeight * 0.5)
					: (bb.getMaxY() + noteHeight * 0.5);
			double middleStemY = isStemUp ? (endStemY + noteHeight * 1.5)
					: (endStemY - noteHeight * 1.5);
			double aboveStaffY = (getStaffLine() != null ? getStaffLine()
					.get5thLineY() : aboveNoteHeadY) - noteHeight;
			double aboveStaffAfterNoteY = aboveStaffY - noteHeight;
			double aboveStaffAfterNoteX = noteHeadX + noteWidth * 2;
			double underStaffY = (getStaffLine() != null ? getStaffLine()
					.get1stLineY() : underNoteHeadY) + noteHeight;
			double middleStaffY = aboveStaffY
					+ ((underStaffY - aboveStaffY) / 2);
			double noteHeadY = lowest.getNotePosition().getY() - noteHeight
					* 0.5;
			double leftNoteHeadX = (lowest.accidentalsPosition != null ? lowest.accidentalsPosition
					.getX() : lowest.notePosition.getX())
					- noteWidth * 0.5;
			double rightNoteHeadX = (lowest.dotsPosition != null ? lowest.dotsPosition[lowest.dotsPosition.length - 1]
					.getX() : (lowest.notePosition.getX() + noteWidth))
					+ noteWidth * 0.5;
			...
			double endStemOutOfStaffY = isStemUp ? Math.min(endStemY,
					aboveStaffY) : Math.max(endStemY, underStaffY);

			m_decorationAnchors[JDecoration.ABOVE_STAFF] = new Point2D.Double(
					noteHeadX, aboveStaffY);
			m_decorationAnchors[JDecoration.UNDER_STAFF] = new Point2D.Double(
					noteHeadX, underStaffY);
			m_decorationAnchors[JDecoration.ABOVE_NOTE] = new Point2D.Double(
					noteHeadX, aboveNoteHeadY);
			m_decorationAnchors[JDecoration.UNDER_NOTE] = new Point2D.Double(
					noteHeadX, underNoteHeadY);
			m_decorationAnchors[JDecoration.VERTICAL_NEAR_STEM] = new Point2D.Double(
					noteHeadX, isStemUp ? aboveNoteHeadY : underNoteHeadY);
			m_decorationAnchors[JDecoration.VERTICAL_NEAR_STEM_OUT_STAFF] = new Point2D.Double(
					noteHeadX, isStemUp ? aboveNoteOutOfStaffY
							: underNoteOutOfStaffY);
			m_decorationAnchors[JDecoration.VERTICAL_AWAY_STEM] = new Point2D.Double(
					noteHeadX, isStemUp ? underNoteHeadY : aboveNoteHeadY);
			m_decorationAnchors[JDecoration.VERTICAL_AWAY_STEM_OUT_STAFF] = new Point2D.Double(
					noteHeadX, isStemUp ? underNoteOutOfStaffY
							: aboveNoteOutOfStaffY);
			m_decorationAnchors[JDecoration.HORIZONTAL_NEAR_STEM] = new Point2D.Double(
					isStemUp() ? rightNoteHeadX : leftNoteHeadX, noteHeadY);
			m_decorationAnchors[JDecoration.HORIZONTAL_AWAY_STEM] = new Point2D.Double(
					isStemUp() ? leftNoteHeadX : rightNoteHeadX, noteHeadY);
			m_decorationAnchors[JDecoration.STEM_END] = new Point2D.Double(
					stemX, endStemY);
			m_decorationAnchors[JDecoration.STEM_END_OUT_OF_STAFF] = new Point2D.Double(
					stemX, endStemOutOfStaffY);
			m_decorationAnchors[JDecoration.STEM_MIDDLE] = new Point2D.Double(
					stemX, middleStemY);
			m_decorationAnchors[JDecoration.ABOVE_STAFF_AFTER_NOTE] = new Point2D.Double(
					aboveStaffAfterNoteX, aboveStaffAfterNoteY);
			m_decorationAnchors[JDecoration.LEFT_NOTE] = new Point2D.Double(
					leftNoteHeadX, noteHeadY);
			m_decorationAnchors[JDecoration.RIGHT_NOTE] = new Point2D.Double(
					rightNoteHeadX, noteHeadY);
			m_decorationAnchors[JDecoration.MIDDLE_STAFF] = new Point2D.Double(
					noteHeadX, middleStaffY);
		}
	}
	...
	protected void renderGraceNotes(Graphics2D context) {
		if (m_jGracenotes != null) {
			...
			if (getMusicElement() instanceof NoteAbstract) {
				...
				addSlur = getTemplate()
						.getAttributeBoolean(
								graceType == GracingType.ACCIACCATURA ? ScoreAttribute.ACCIACCATURA_SLUR
			...
			if (addSlur) {
				...
				if (m_jGracenotes.getBase().getX() > getBase().getX())
					graceAfterNote = true;
				boolean gracesUp = ((JStemmableElement) m_jGracenotes)
						.isStemUp();
				Point2D p2dStart, p2dControl, p2dEnd;
				JNoteElementAbstract start, end;
				if (graceAfterNote) {
					if (this instanceof JChord) {
						start = gracesUp ? ((JChord) this).getLowestNote()
					...
					if (m_jGracenotes instanceof JGraceNote)
						...
						// JGroupOfGrace
						end = (JGraceNotePartOfGroup) ((JGroupOfGraceNotes) m_jGracenotes).m_jNotes[((JGroupOfGraceNotes) m_jGracenotes).m_jNotes.length - 1];
				} else {
					if (m_jGracenotes instanceof JGraceNote)
						start = (JGraceNote) m_jGracenotes;
					else
						// JGroupOfGrace
						start = (JGraceNotePartOfGroup) ((JGroupOfGraceNotes) m_jGracenotes).m_jNotes[0];
					if (this instanceof JChord) {
						end = gracesUp ? ((JChord) this).getLowestNote()
								: ((JChord) this).getHighestNote();
					} else
						end = this;
				}
				p2dStart = gracesUp ? start.getTieStartUnderAnchor() : start
						.getTieStartAboveAnchor();
				p2dEnd = gracesUp ? end.getTieEndUnderAnchor() : end
						.getTieEndAboveAnchor();
				double yControl = gracesUp ? Math.max(p2dStart.getY(),
						p2dEnd.getY()) : Math.min(p2dStart.getY(),
						p2dEnd.getY());
				...
				p2dControl = new Point2D.Double(p2dStart.getX()
						+ (p2dEnd.getX() - p2dStart.getX()) / 2, yControl
						+ ((gracesUp ? .5 : -.5) * getMetrics()
								.getSlurAnchorYOffset()));
				...
				q.setCurve(p2dStart,
						JTune.newControl(p2dStart, p2dControl, p2dEnd), p2dEnd);
				path.append(q, true);
				p2dControl.setLocation(p2dControl.getX(), p2dControl.getY()
						+ (gracesUp ? 1 : -1));
				q.setCurve(p2dEnd,
