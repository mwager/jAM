...
public class TuneEditorPane extends JTextPane// implements ActionListener
{
	...
	private static Color SELECTION_FOREGROUND_COLOR = Color.white;// new
																	// Color(10,36,106);
	...
	/** */
	private Style m_barStyle, m_textStyle, m_errorStyle, m_fieldStyle,
			m_rhythmStyle, m_defaultStyle, m_baseNoteStyle, m_commentStyle,
			m_gracingStyle, m_symbolStyle = null;
	private static final int IDLE_TIME_BEFORE_REFRESH = 200;
	/**
	 * The thread in charge of refreshing the tune representation of this editor
	 * pane.
	 */
	 ...
	 * 
	 * @param parser
	 *            The parser to be used to parse the abc notation written in the
	 *            TuneEditorPane and to give the resulting
	 *            {@link abc.notation.Tune tune}
	 */
	 ...
	 * 
	 * @param idleTimeBeforeRefresh
	 *            The idle time after any text update before triggering the
	 *            parsing of the abc text written in this pane. This is used to
	 *            avoid perpetual parsing of the text at each text update
	 */
	...
	/**
	 * Creates a new TuneEditorPane with the given parser and idle time before
	 * refresh.
	 * 
	 * @param parser
	 *            The parser to be used to parse the abc notation written in the
	...
	public TuneEditorPane(TuneParser parser, int idleTimeBeforeRefresh) {
		...
		// m_tuneParser = parser;
		m_refresher = new ParsingRefresh((DefaultStyledDocument) getDocument(),
				parser);
		...
		registerKeyboardAction(myAL, PASTE_ACTION, paste,
	...
	/**
	 * Returns the parser used to build up the tune from the abc text written in
	 * this pane.
	 * 
	 * @return the parser used to build up the tune from the abc text written in
	 *         this pane.
	 */
	 ...
	 *         otherwise. Text coloring enables user to distinguish abc tokens
	 *         detected in the text of this pane.
	 ...
	 * 
	 * @param coloring
	 *            <TT>true</TT> if text coloring should be enabled,
	...
	public void setColoringEnable(boolean coloring) {
		...
		if (m_enableColoring)
			...
			((DefaultStyledDocument) getDocument()).setCharacterAttributes(0,
					getDocument().getEndPosition().getOffset(), m_defaultStyle,
					true);
			// setSelectedTextColor(SELECTION_FOREGROUND_COLOR);
			// setSelectionColor(SELECTION_BACKGROUND_COLOR);
		}
	 ...
	 * 
	 * @param elmnt
	 *            The element to be highlighted in the abc tune notation.
	 */
	 ...
	 * //m_forceRefresh = true; super.setDocument(doc); //if (m_refresher!=null)
	 * //{ // setCharacterAttributes(m_defaultStyle, true); //
	...
	public void setText(String text) {
		 ...
		 * m_refresher.setDocument(doc); this.setDocument(doc); } catch
			...
			if (e.getActionCommand().compareTo(PASTE_ACTION) == 0) {
				...
				try {
					...
					if (selectedText != null)
						getDocument().remove(getCaretPosition(),
								selectedText.length());
					getDocument().insertString(getCaretPosition(), trstring,
							m_defaultStyle);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
	...
	private class ParsingRefresh extends Thread implements DocumentListener,
			...
			while (true) {
				try {
					synchronized (m_mutex) {
						...
						if (!m_forceRefresh) {
							do {
								// System.out.println("Area - compting before parsing : "
								// + m_idleTime);
								m_mutex.wait(10);
								m_idleTime += 10;
						...
						try {
							...
							String tuneNotation = TuneEditorPane.this
									.getDocument().getText(
											0,
											TuneEditorPane.this.getDocument()
													.getLength());
		...
		public void tuneEnd(Tune tune) {
			if (m_enableColoring) {
				// ===========================================================THOSE
				// CALLBACK CREATE A DEADLOCK WHEN SETTING TEXT ATTRIBUTES.
				try {
				...
				if (event instanceof TokenEvent) {
					...
					if (token != null) {
						try {
							...
							AbcTokenType tokenType = (AbcTokenType) token
									.getType();
							if (tokenType.isField())
								att = m_fieldStyle;
							else if (tokenType.equals(AbcTokenType.TUPLET_SPEC)
									|| tokenType
											.equals(AbcTokenType.BROKEN_RHYTHM))
								att = m_rhythmStyle;
							else if (tokenType.equals(AbcTokenType.BARLINE)
									|| tokenType
											.equals(AbcTokenType.REPEAT_OPEN)
									|| tokenType
											.equals(AbcTokenType.REPEAT_CLOSE)
									|| tokenType
											.equals(AbcTokenType.NTH_REPEAT))
								att = m_barStyle;
							else if (token.getType()
									.equals(AbcTokenType.SYMBOL))
								att = m_symbolStyle;
								...
								if (m_contextForText
										.equals(AbcTokenType.COMMENT))
									att = m_commentStyle;
								else
									att = m_textStyle;
							} else if (token.getType().equals(
									AbcTokenType.BASE_NOTE)) {
								if (AbcTokenType.GRACING_BEGIN
										.equals(m_contextForNote))
									att = m_gracingStyle;
								else
									att = m_baseNoteStyle;
							} else if (token.getType().equals(
