...
public class Abc2xml {
	...
	/** C major */
	protected static final Accidental[] KEY_NO_ACCIDENTAL = {
			Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL,
			Accidental.NATURAL };
	/** G major */
	protected static final Accidental[] KEY_SHARP_1ST = { Accidental.NATURAL,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.SHARP,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL };
	/** D major */
	protected static final Accidental[] KEY_SHARP_2ND = { Accidental.SHARP,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.SHARP,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL };
	/** A major */
	protected static final Accidental[] KEY_SHARP_3RD = { Accidental.SHARP,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.SHARP,
			Accidental.SHARP, Accidental.NATURAL, Accidental.NATURAL };
	/** E major */
	protected static final Accidental[] KEY_SHARP_4TH = { Accidental.SHARP,
			Accidental.SHARP, Accidental.NATURAL, Accidental.SHARP,
			Accidental.SHARP, Accidental.NATURAL, Accidental.NATURAL };
	/** B major */
	protected static final Accidental[] KEY_SHARP_5TH = { Accidental.SHARP,
			Accidental.SHARP, Accidental.NATURAL, Accidental.SHARP,
			Accidental.SHARP, Accidental.SHARP, Accidental.NATURAL };
	/** F# major */
	protected static final Accidental[] KEY_SHARP_6TH = { Accidental.SHARP,
			Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
			Accidental.SHARP, Accidental.SHARP, Accidental.NATURAL };
	/** C# major */
	protected static final Accidental[] KEY_SHARP_7TH = { Accidental.SHARP,
			Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
			Accidental.SHARP, Accidental.SHARP, Accidental.SHARP };

	/** F major */
	protected static final Accidental[] KEY_FLAT_1ST = { Accidental.NATURAL,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT };
	/** Bb major */
	protected static final Accidental[] KEY_FLAT_2ND = { Accidental.NATURAL,
			Accidental.NATURAL, Accidental.FLAT, Accidental.NATURAL,
			Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT };
	/** Eb major */
	protected static final Accidental[] KEY_FLAT_3RD = { Accidental.NATURAL,
			Accidental.NATURAL, Accidental.FLAT, Accidental.NATURAL,
			Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT };
	/** Ab major */
	protected static final Accidental[] KEY_FLAT_4TH = { Accidental.NATURAL,
			Accidental.FLAT, Accidental.FLAT, Accidental.NATURAL,
			Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT };
	/** Db major */
	protected static final Accidental[] KEY_FLAT_5TH = { Accidental.NATURAL,
			Accidental.FLAT, Accidental.FLAT, Accidental.NATURAL,
			Accidental.FLAT, Accidental.FLAT, Accidental.FLAT };
	/** Gb major */
	protected static final Accidental[] KEY_FLAT_6TH = { Accidental.FLAT,
			Accidental.FLAT, Accidental.FLAT, Accidental.NATURAL,
			Accidental.FLAT, Accidental.FLAT, Accidental.FLAT };
	/** Cb major */
	protected static final Accidental[] KEY_FLAT_7TH = { Accidental.FLAT,
	...
	public Document createMusicXmlDOM(Tune tune) {
		...
		try {
			...
			if (tune.getTitles().length > 0)
				movementTitleEl
						.appendChild(doc.createTextNode(tune.getTitles()[0]));
	...
	protected void appendTo(Element measureElement, TimeSignature time,
			Document context) {
		Element attributeEl = (Element) measureElement.getElementsByTagName(
				ATTRIBUTES_TAG).item(0);
		if (attributeEl == null) {
			/*
	...
	protected void appendTo(Element measureElement, KeySignature key,
			Document doc) {
		Element attributeEl = (Element) measureElement.getElementsByTagName(
				ATTRIBUTES_TAG).item(0);
		if (attributeEl == null) {
	...
	protected void convert(Document doc, Music music, Element musicElement) {
		...
		int voltaRunning = 0;
		currentMeasureEl.setAttribute(NUMBER_ATTRIBUTE,
				new Integer(measureNb).toString());
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < voice.size(); i++) {
				...
				if (voice.elementAt(i) instanceof Note) {
					...
					appendTo(currentMeasureEl,
							(TimeSignature) voice.elementAt(i), doc);
				} else if (voice.elementAt(i) instanceof KeySignature) {
					// key = (KeySignature)music.elementAt(i);
					appendTo(currentMeasureEl,
							(KeySignature) voice.elementAt(i), doc);
					...
					if (voltaRunning > 1) { // end of volta > 2 on the first bar
						...
						endingEl.setAttribute(NUMBER_ATTRIBUTE,
								Integer.toString(voltaRunning));
					...
					if (barline instanceof RepeatBarLine) { // start of volta
						...
						Element endingEl = doc.createElement("ending");
						endingEl.setAttribute(NUMBER_ATTRIBUTE, Integer
								.toString(repeatBarLine.getRepeatNumbers()[0]));
					...
					if (barline.getType() == BarLine.REPEAT_CLOSE) { // Close
						...
						if (barline.toString().equals(":|")) {
							...
							barLineNode.setAttribute(LOCATION_ATTRIBUTE,
									"right");
							Element repeatNode = doc.createElement(REPEAT_TAG);
							repeatNode.setAttribute(DIRECTION_ATTRIBUTE,
									"backward");
					...
					if (addedMusicElement == 0) {
						if (barline.getType() == BarLine.REPEAT_OPEN) {
							...
							barLineNode
									.setAttribute(LOCATION_ATTRIBUTE, "left");
							Element repeatNode = doc.createElement(REPEAT_TAG);
							repeatNode.setAttribute(DIRECTION_ATTRIBUTE,
									"forward");
							barLineNode.appendChild(repeatNode);
				...
				if (voice.elementAt(i) instanceof NotesSeparator) {
					...
					if (lastShorterThanQuarterNote != null) {
						Element beamNode;
						if (lastShorterThanQuarterNote.getElementsByTagName(
								BEAM_TAG).getLength() == 0)
							beamNode = doc.createElement(BEAM_TAG);
						else
							beamNode = (Element) lastShorterThanQuarterNote
									.getElementsByTagName(BEAM_TAG).item(0);
	...
	protected Node convert(Document doc, TimeSignature signature) {
		...
		beatsEl.appendChild(doc.createTextNode(Integer.toString(signature
				.getNumerator())));
		beatTypeEl.appendChild(doc.createTextNode(Integer.toString(signature
				.getDenominator())));
	...
	protected Node[] convert(Document doc, MultiNote chord) {
		...
		for (int i = 0; i < notes.size(); i++) {
			...
			if (i != 0)
				nodes[i].insertBefore(doc.createElement(CHORD_TAG),
						nodes[i].getFirstChild());
		}
		return nodes;
	...
	protected Element convert(Document doc, Note note) {
		...
		if (note.isRest()) {
			...
			if (keySignature != null) {
				...
				if (accidental.isInTheKey()) {
					accidental = keySignature.getAccidentalFor(note
							.getStrictHeight());
				}
		...
		int relDuration = note.getDuration() * DIVISIONS_PER_QUARTER_NOTE
				/ Note.QUARTER;
		...
		if (note.isTied()) {
			...
			if (type != null) {
				...
				Element notationsEl = (Element) noteEl.getElementsByTagName(
						NOTATIONS_TAG).item(0);
				if (notationsEl == null) {
		...
		if (note.isPartOfTuplet()) {
			...
			Element timeModificationEl = doc
					.createElement(TIMEMODIFICATION_TAG);

			Element actualNotesEl = doc.createElement(ACTUALNOTES_TAG);
			actualNotesEl.appendChild(doc.createTextNode(Integer.toString(d
					/ gcd)));
			timeModificationEl.appendChild(actualNotesEl);

			Element normalNotesEl = doc.createElement(NORMALNOTES_TAG);
			normalNotesEl.appendChild(doc.createTextNode(Integer
					.toString(tuplet.getTotalDuration() / gcd)));
