...
/**
 * In music theory, the term interval describes the relationship between the
 * pitches of two notes.
 */
public class Interval implements Cloneable, Serializable {
	...
	// quality
	/**
	 * Quadruple diminished quality, quite improbable but mathematically
	 * possible
	 */
	public static final byte QUADRUPLE_DIMINISHED = -5;
	/**
	 * Triple diminished quality, quite improbable but mathematically possible
	 */
	...
	/**
	 * Perfect quality, note that only unison, fourth, fifth and octave can be
	 * perfect
	 */
	...
	/**
	 * Triple augmented quality, quite improbable but mathematically possible
	 */
	// very rare, but mathematically possible? Eb - B##, or Ebb - B#
	public static final byte TRIPLE_AUGMENTED = 4;
	/**
	 * Quadruple augmented quality, quite improbable but mathematically possible
	 */
	...
	public static final Interval DIMINISHED_UNISON = new Interval(UNISON,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_UNISON = new Interval(UNISON,
			AUGMENTED);
	/** Diminished second, e.g. C-Dbb, similar to {@link #PERFECT_UNISON} */
	public static final Interval DIMINISHED_SECOND = new Interval(SECOND,
			DIMINISHED); // equals unison
	...
	public static final Interval AUGMENTED_SECOND = new Interval(SECOND,
			AUGMENTED);
	/** Diminished second, e.g. C-Ebb, similar to {@link #MAJOR_SECOND} */
	public static final Interval DIMINISHED_THIRD = new Interval(THIRD,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_THIRD = new Interval(THIRD,
			AUGMENTED);
	/** Diminished fourth, e.g. C-Fb, similar to {@link #MAJOR_THIRD} */
	public static final Interval DIMINISHED_FOURTH = new Interval(FOURTH,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_FOURTH = new Interval(FOURTH,
			AUGMENTED);
	/** Diminished fifth, e.g. C-Gb, similar to {@link #AUGMENTED_FOURTH} */
	public static final Interval DIMINISHED_FIFTH = new Interval(FIFTH,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_FIFTH = new Interval(FIFTH,
			AUGMENTED);
	/** Diminished sixth, e.g. C-Abb, similar to {@link #PERFECT_FIFTH} */
	public static final Interval DIMINISHED_SIXTH = new Interval(SIXTH,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_SIXTH = new Interval(SIXTH,
			AUGMENTED);
	/** Diminished seventh, e.g. C-Bbb, similar to {@link #MAJOR_SIXTH} */
	public static final Interval DIMINISHED_SEVENTH = new Interval(SEVENTH,
			DIMINISHED);
	...
	public static final Interval AUGMENTED_SEVENTH = new Interval(SEVENTH,
			AUGMENTED);
	/** Diminished octave, e.g. C-cb */
	public static final Interval DIMINISHED_OCTAVE = new Interval(OCTAVE,
			DIMINISHED);
	/** Perfect octave, e.g. C-c */
	public static final Interval PERFECT_OCTAVE = new Interval(OCTAVE, PERFECT);
	/** Augmented octave, e.g. C-c# */
	public static final Interval AUGMENTED_OCTAVE = new Interval(OCTAVE,
			AUGMENTED);

	/**
	 * Reverse the order of an interval. if i.getOrder() is upward, return a
	 * downward interval, and vice-versa.
	 * 
	...
	public static Interval reverseOrder(Interval i) {
		return new Interval(i.getLabel(), i.getQuality(),
				(byte) -i.getDirection());
	}
	 ...
	 */
	public Interval(byte label, byte quality) throws IllegalArgumentException {
	 ...
	 * 
	 * The parameter key, if specified, helps to determinate the accidentals of
	 * the notes if n1.getAccidental() equals Accidental.NONE (e.g. in D major,
	 * F with accidental NONE is understood as F sharp).
	 * 
	 * If n1 is lower than n2, interval order is upward, if n1 is higher than
	 * n2, interval order is downward.
	 * 
	 * @param n1
	 * @param n2
	 * @param key
	...
	public Interval(Note n1, Note n2, KeySignature key) {
		...
		deltaSemitones = Math.abs(high.getMidiLikeHeight(key)
				- low.getMidiLikeHeight(key));
		...
		for (int i = 0; i < notes.length; i++) {
			if (notes[i] == low.getStrictHeight())
				height1 = i;
			if (notes[i] == high.getStrictHeight())
				height2 = i;
		}
		if (height2 < height1)
			height2 += 7;
		...
		if (height1 == height2)
			deltaOctave = high.getOctaveTransposition()
					- low.getOctaveTransposition();
		...
		default:
			System.err
					.println("Could not compute the qualification of the interval between "
							+ low
							+ " and "
							+ high
							+ " : label="
	...
	public int getSemitones() {
		...
		case PERFECT:
		case MAJOR:
			break; // do not change pitch for major and perfect
		case AUGMENTED:
	...
	/**
	 * Two Interval are equals if they have the same label, quality and
	 * direction
	 */
	 ...
	 * 
	 * <pre>
	 * <code>if (myInterval.getLabel() <= Interval.FIFTH) {
	 ...
	 *         {@link #FOURTH}, {@link #FIFTH}, {@link #SIXTH}, {@link #SEVENTH}
	 *         + <I>n</i> {@link #OCTAVE}
	 */
	...
	public boolean isAugmented() {
		return (m_quality == AUGMENTED) || (m_quality == DOUBLE_AUGMENTED)
				|| (m_quality == TRIPLE_AUGMENTED)
				|| (m_quality == QUADRUPLE_AUGMENTED);
	}

	public boolean isDiminished() {
		return (m_quality == DIMINISHED) || (m_quality == DOUBLE_DIMINISHED)
	...
	/**
	 * Compute the second note of this interval based on first note n1.
	 * 
	 * This version is more accurate, if n1 has no accidental, the key signature
	 * tells which accidental it has. The returned note will have no accidental
	 * if its accidental is in the key signature.
	 * 
	 * @param n1
	 * @param key
	 *            can be <TT>null</TT>
	...
	public Note calculateSecondNote(Note n1, KeySignature key) {
		...
		if (key != null) {
			if (n2.getAccidental().equals(
					key.getAccidentalFor(n2.getStrictHeight())))
	...
	private Accidental[] calculateSecondNoteAccidental(Note n1, KeySignature key) {
		...
		if (isPerfectUnisonOrOctave()) {
			...
			if (n1AccValue.isDoubleFlat())
				...
				return new Accidental[] { Accidental.DOUBLE_SHARP,
						Accidental.SHARP };
			...
			if (n1AccValue.isDoubleFlat())
				...
				return new Accidental[] { Accidental.DOUBLE_FLAT,
						Accidental.FLAT, Accidental.NATURAL };
			else if ((n1AccValue.isInTheKey()) || (n1AccValue.isNatural()))
				return new Accidental[] { Accidental.FLAT,
			...
			if (n1AccValue.isDoubleFlat())
				...
				return new Accidental[] {
						Accidental.NATURAL,
						(m_quality == PERFECT && (m_label % OCTAVE) == FOURTH) ? Accidental.SHARP
								: Accidental.FLAT };
			else if (n1AccValue.isSharp()) {
			...
			if (n1AccValue.isDoubleFlat())
				...
				return new Accidental[] {
						Accidental.NATURAL,
						(m_quality == PERFECT && (m_label % OCTAVE) == FOURTH) ? Accidental.FLAT
								: Accidental.SHARP };
			else if (n1AccValue.isSharp())
				return new Accidental[] {
						Accidental.SHARP,
						(m_quality == PERFECT && (m_label % OCTAVE) == FOURTH) ? Accidental.NATURAL
								: Accidental.DOUBLE_SHARP };
			else if (n1AccValue.isDoubleSharp())
				return new Accidental[] { Accidental.DOUBLE_SHARP,
						Accidental.SHARP };
		}
		System.err.println("calculateSecondNoteAccidental(" + n1
				+ ") for interval " + toString() + " gave no good result");
		return new Accidental[] { Accidental.NATURAL };
	}
