...
/**
 * This class defines key signatures using modes definition like E major, G
 * minor etc etc...
 * 
 * <PRE>
 *                           1   2   3   4   5   6   7
 ...
 * </PRE>
 * 
 * If we consider the key namned "Ab aeolian", "A" is the note of this key, "b"
 * is the key accidental and "aeolian" is the mode.
 */
public class KeySignature extends MusicElement implements Cloneable {
	...
	private static final Accidental[][] accidentalsRules = {
			...
			// C D E F G A B
			/* C */{ Accidental.NATURAL, Accidental.NATURAL,
					Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL,
					Accidental.NATURAL, Accidental.NATURAL }, // C
			/* Db */
			{ Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT,
					Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT,
					Accidental.FLAT },
			/* D */{ Accidental.SHARP, Accidental.NATURAL, Accidental.NATURAL,
			...
			{ Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT,
					Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT,
					Accidental.FLAT },
			/* E */{ Accidental.SHARP, Accidental.SHARP, Accidental.NATURAL,
			...
			{ Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT,
					Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT,
					Accidental.FLAT },
			/* A */{ Accidental.SHARP, Accidental.NATURAL, Accidental.NATURAL,
					Accidental.SHARP, Accidental.SHARP, Accidental.NATURAL,
			...
			{ Accidental.NATURAL, Accidental.NATURAL, Accidental.FLAT,
					Accidental.NATURAL, Accidental.NATURAL, Accidental.NATURAL,
					Accidental.FLAT },
			/* B */{ Accidental.SHARP, Accidental.SHARP, Accidental.NATURAL,
	...
	private static final Accidental[][] accidentalRulesFlat = {
			...
			/* E */{},
			/* F */{},
			/* Gb */{ Accidental.FLAT, Accidental.FLAT, Accidental.FLAT,
					Accidental.NATURAL, Accidental.FLAT, Accidental.FLAT,
					Accidental.FLAT },
			/* G */{},
			/* */{},
			/* A */{},
			/* */{},
			/* Cb */{ Accidental.FLAT, Accidental.FLAT, Accidental.FLAT,
					Accidental.FLAT, Accidental.FLAT, Accidental.FLAT,
					Accidental.FLAT } };

	private static final Accidental[][] accidentalRulesSharp = {
			/* C */{},
			/* C# */{ Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
					Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
					Accidental.SHARP },
			/* D */{},
			/* */{},
			/* E */{},
			/* F */{},
			/* F# */{ Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
					Accidental.SHARP, Accidental.SHARP, Accidental.SHARP,
					Accidental.NATURAL },
			/* G */{},
			/* */{},
	...
	/**
	 * Highland Bagpipe notation is also catered for : K:HP puts no key
	 * signature on the music but implies the bagpipe scale, while K:Hp puts F
	 * sharp, C sharp and G natural.
	 */
	 ...
	 *            <TT>LYDIAN</TT> <TT>MAJOR</TT>, <TT>MINOR</TT>,
	 *            <TT>MIXOLYDIAN</TT>, <TT>PHRYGIAN</TT> or <TT>OTHER</TT>.
	 */
	...
	public KeySignature(byte keyNoteType, float keyAccidentalValue,
			byte modeType) {
	 ...
	 *            <TT>Accidental.NONE</TT>, or <TT>Accidental.FLAT</TT>.
	 * @param modeType
	 *            The type of the mode. Possible values are <TT>AEOLIAN</TT>,
	 *            <TT>DORIAN</TT>, <TT>IONIAN</TT>, <TT>LOCRIAN</TT>,
	 *            <TT>LYDIAN</TT>, <TT>MAJOR</TT>, <TT>MINOR</TT>,
	...
	public KeySignature(byte keyNoteType, Accidental keyAccidental,
		...
		if (m_keyNote == Note.D)
			keyIndex = 2;
		else if (m_keyNote == Note.E)
			keyIndex = 4;
		else if (m_keyNote == Note.F)
			keyIndex = 5;
		else if (m_keyNote == Note.G)
			keyIndex = 7;
		else if (m_keyNote == Note.A)
			keyIndex = 9;
		else if (m_keyNote == Note.B)
			keyIndex = 11;
		if (m_keyAccidental.isSharp())
			keyIndex += 1;
		else if (m_keyAccidental.isFlat())
			keyIndex -= 1;
		if (modeType == MINOR)
			keyIndex += 3;
		...
		else if (modeType == OTHER)
			keyIndex = 0;// all natural
		...
		else if (keyIndex == 6 && keyAccidental.isSharp()) // F# (The E is
															// sharp)
	 ...
	 *            accidentals are : <TT>Accidental.SHARP</TT>,
	 *            <TT>Accidental.NATURAL</TT> or <TT>Accidental.FLAT</TT>.
	 */
	 ...
	 * 
	 * @return The note of the mode. Possible values are <TT>Note.A</TT>,
	 *         <TT>Note.B</TT>, <TT>Note.C</TT>, <TT>Note.D</TT>,
	 *         <TT>Note.E</TT>, <TT>Note.F</TT> or <TT>Note.G</TT>.
	 */
	public byte getNote() {
	 ...
	 *            {@link #getNote()}.
	 * @return Possible values are <TT>Note.A</TT>, <TT>Note.B</TT>,
	 *         <TT>Note.C</TT>, <TT>Note.D</TT>, <TT>Note.E</TT>,
	 *         <TT>Note.F</TT> or <TT>Note.G</TT>.
	 ...
	 * <TT>accidentals = accidentalRules...[...]</TT> keeps the reference to
	 * accidentalRules... so if you set an accidental with
	 * {@link #setAccidental(byte, byte)} the static accidentalRule...[] is
	 * altered.
	 * 
	 * @param accidentalsDefinition
	 */
	 ...
	 * 
	 * @param noteHeigth
	 *            The note heigth. Possible values are, <TT>Note.A</TT>,
	 *            <TT>Note.B</TT>, <TT>Note.C</TT>, <TT>Note.D</TT>,
	 *            <TT>Note.E</TT>, <TT>Note.F</TT> or <TT>Note.G</TT>.
	 * @param accidental
	 *            The accidental to be set to the note. Possible values are :
	 *            <TT>Accidental.SHARP</TT>, <TT>Accidental.NATURAL</TT> or
	 *            <TT>Accidental.FLAT</TT>.
	 * @exception IllegalArgumentException
	 *                Thrown if an invalid note heigth or accidental has been
	 *                given.
	 */
	public void setAccidental(byte noteHeigth, Accidental accidental)
			throws IllegalArgumentException {
		int index = 0;
		noteHeigth = (byte) (noteHeigth % 12);
		if (noteHeigth == Note.C)
			index = 0;
		else if (noteHeigth == Note.D)
			index = 1;
		else if (noteHeigth == Note.E)
			index = 2;
		else if (noteHeigth == Note.F)
			index = 3;
		else if (noteHeigth == Note.G)
			index = 4;
		else if (noteHeigth == Note.A)
			index = 5;
		else if (noteHeigth == Note.B)
			index = 6;
		else
			throw new IllegalArgumentException("Invalid note heigth : "
					+ noteHeigth);
		...
		// throw new
		// IllegalArgumentException("Accidental can't be DOUBLE_SHARP or DOUBLE_FLAT");
		else
	 ...
	 * 
	 * @param noteHeigth
	 *            A note heigth among <TT>Note.C</TT>, <TT>Note.D</TT>,
	 *            <TT>Note.E</TT>, <TT>Note.F</TT>, <TT>Note.G</TT>,
	 *            <TT>Note.A</TT>, <TT>Note.B</TT>.
	 * @return Accidental value for the specified note heigth. This value can be
	 *         <TT>NATURAL</TT>, <TT>FLAT</TT> or <TT>SHARP</TT>.
	 * @exception IllegalArgumentException
	 *                Thrown if the specified note heigth is invalid.
	 */
	...
	/**
	 * Returns <TT>true</TT> if the key has only flats or naturals. Takes into
	 * account the additionnals accidentals.
	 * 
	 * e.g.:
	 * <ul>
	...
	/**
	 * Returns <TT>true</TT> if the key has only sharps or naturals. Takes into
	 * account the additionnals accidentals.
	 * 
	 * e.g.:
	 * <ul>
	...
	/**
	 * Returns <TT>true</TT> if the key contains sharps <B>and</B> flats (e.g.:
	 * in ABC format oriental scale <I>nawa atar</i> <TT>K:Cm ^f =b</TT> which
	 * scale is C D Eb F# G Ab B).
	 * 
	...
	public boolean isSharpDominant() {
		return ((keyIndex == 1 && m_keyAccidental.isSharp()) || keyIndex == 2
				|| keyIndex == 4
				|| (keyIndex == 6 && m_keyAccidental.isSharp())
				|| keyIndex == 7 || keyIndex == 9 || (keyIndex == 11 && m_keyAccidental
				.isNatural()));
	}
	...
	public boolean isFlatDominant() {
		return ((keyIndex == 1 && m_keyAccidental.isFlat()) || keyIndex == 3
				|| keyIndex == 5 || (keyIndex == 6 && m_keyAccidental.isFlat())
				|| keyIndex == 8 || keyIndex == 10 || (keyIndex == 11 && m_keyAccidental
				.isFlat()));
	}
	...
	public String toLitteralNotation() {
		...
		else if (m_keyNote == Note.C)
			notation = notation.concat("C");
		else if (m_keyNote == Note.D)
		...
		if (m_keyAccidental.isFlat())
			notation = notation.concat("b");
		else if (m_keyAccidental.isSharp())
	...
	public static byte convertToModeType(String mode) {
		...
		if ("AEO".equals(mode))
			return KeySignature.AEOLIAN;
		else if ("DOR".equals(mode))
			return KeySignature.DORIAN;
		else if ("ION".equals(mode))
			return KeySignature.IONIAN;
		else if ("LOC".equals(mode))
			return KeySignature.LOCRIAN;
		else if ("LYD".equals(mode))
			return KeySignature.LYDIAN;
		else if ("MAJ".equals(mode) || ("".equals(mode)))
			return KeySignature.MAJOR;
		else if ("MIN".equals(mode) || ("M".equals(mode)))
			return KeySignature.MINOR;
		else if ("MIX".equals(mode))
			return KeySignature.MIXOLYDIAN;
		else if ("PHR".equals(mode))
			return KeySignature.PHRYGIAN;
		// "EXP" = explicit -> return other
		else
			return KeySignature.OTHER;
	}
	...
	public String toString() {
		String string2Return = "KeySignature: " + toLitteralNotation() + " (";
		for (int i = 0; i < 7; i++) {
	...
	static public KeySignature transpose(KeySignature key, int semitones) {
		...
		KeySignature reference = new KeySignature(keyNote.getStrictHeight(),
				keyNote.getAccidental(), key.getMode());
		// addedAcc[degree] array of degree => accidental
		float[] addedAcc = new float[7];
		for (int i = Degree.I; i <= Degree.VII; i++) {
			float refAcc = reference.getAccidentalFor(reference.getDegree(i))
					.getValue();
		...
		KeySignature ret = new KeySignature(
				...
				transposedKeyNote.getAccidental(), key.getMode());
		for (int i = Degree.I; i <= Degree.VII; i++) {
			if (addedAcc[i - 1] != 0) {
				Accidental acc = new Accidental(ret.getAccidentalFor(
						ret.getDegree(i)).getValue()
						+ addedAcc[i - 1]);
	...
	/*
	 * public void display () { System.out.println (key + " " + m_keyAccidental
	 * + " " + mode); }
	 */
}
