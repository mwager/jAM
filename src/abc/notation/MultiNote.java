...
public class MultiNote extends NoteAbstract implements Cloneable {
	 ...
	 * 
	 * @param notes
	 *            A Vector containing the <TT>NoteAbstract</TT> of this
	 *            <TT>MultiNote</TT>.
	 */
	...
	public Note getLongestNote() {
		/*
		 * float length = 0; float currentNoteLength = 0; Note maxNote = null;
		 * for (int i=0; i<m_notes.size() && maxNote==null; i++) {
		 * currentNoteLength = ((Note)(m_notes.elementAt(i))).getDuration(); if
		 * (currentNoteLength > length) maxNote = (Note)m_notes.elementAt(i); }
		 * return maxNote;
	 ...
	 * @param notes
	 *            An arry of notes.
	 * @param aStrictDuration
	 *            A strict duration (use constants defined class Note)
	 * @return All notes strictly shorter than the given strict duration.
	 *         <TT>null</TT> is returned is no note if shorter than the given
	 *         strict duration.
	...
	public Note getLowestNote() {
		/*
		 * Note lowestNote = (Note)m_notes.elementAt(0); for (int i=1;
		 * i<m_notes.size(); i++) if
		 * (((Note)(m_notes.elementAt(i))).isLowerThan(lowestNote)) lowestNote =
		 * (Note)m_notes.elementAt(i); return lowestNote;
		 */
	...
	public boolean hasUniqueStrictDuration() {
		 ...
		 * ((Note)m_notes.elementAt(0)).getStrictDuration(); short
		 * currentDuration = 0; for (int i=1; i<m_notes.size(); i++) {
		 * currentDuration = ((Note)(m_notes.elementAt(i))).getStrictDuration();
		 * if (currentDuration!=strictDuration) return false; } return true;
		 */
	 ...
	 * 
	 * @return The strict durations composing this multi notes. The array is
	 *         sorted from shortest durations to longest ones.
	 */
	public short[] getStrictDurations() {
		Vector durations = new Vector();
		short currentDuration = 0;
		for (int i = 0; i < m_notes.size(); i++) {
			currentDuration = ((Note) (m_notes.elementAt(i)))
					.getStrictDuration();
		...
		if (durations.size() == 0)
			...
			for (int i = 0; i < durations.size(); i++) {
				...
				while (j < sortedDurations.size()
						&& ((Short) sortedDurations.elementAt(j)).shortValue() < (((Short) durations
	...
	/**
	 * Normalizes this multi note by decomposing it into multinotes with same
	 * strict duration.
	 * 
	...
	public MultiNote[] normalize() {
		...
		for (int i = 0; i < strictDurations.length; i++) {
			normalizedChords[i] = new MultiNote(
					(Vector) splitter.get(new Short(strictDurations[i])));
		}
	...
	private static Vector fromLowestToHighest(Vector original) {
		...
		for (int i = 0; i < original.size(); i++) {
			...
			while (j < orderedNotes.size()
					&& ((Note) orderedNotes.elementAt(j))
							.isLowerThan((Note) original.elementAt(i)))
				j++;
	...
	/**
	 * Returns <TT>true</TT> if this multi note some accidentals (for at least
	 * one of its note).
	 * 
	 * @return <TT>true</TT> if this multi note some accidentals (for at least
	 *         one of its note), <TT>false</TT> otherwise.
	...
	/**
	 * Returns a new vector containing <B>clones</B> of all <TT>Note</TT>
	...
	/**
	 * Returns notes composing this multinote as an array of notes, sorted from
	 * the lowest note to the highest one.
	 * 
	 * @return Notes composing this multinote as an array of notes, sorted from
	 *         the lowest note to the highest one.
	 */
	public Note[] toArray() {
		if (m_notes.size() > 0) {
	...
	public String toString() {
		...
		for (int i = 0; i < m_notes.size(); i++) {
			...
			if (n.getStrictHeight() == Note.REST)
				string2Return = string2Return.concat("z");
			else if (n.getStrictHeight() == Note.C)
				string2Return = string2Return.concat("C");
			else if (n.getStrictHeight() == Note.D)
				string2Return = string2Return.concat("D");
			else if (n.getStrictHeight() == Note.E)
				string2Return = string2Return.concat("E");
			else if (n.getStrictHeight() == Note.F)
				string2Return = string2Return.concat("F");
			else if (n.getStrictHeight() == Note.G)
				string2Return = string2Return.concat("G");
			else if (n.getStrictHeight() == Note.A)
				string2Return = string2Return.concat("A");
			else if (n.getStrictHeight() == Note.B)
				string2Return = string2Return.concat("B"); /*
															 ...
															 * .concat("b");
															 */
			if (n.getOctaveTransposition() == 1)
				string2Return = string2Return.concat("'");
			else if (n.getOctaveTransposition() == -1)
				string2Return = string2Return.concat(",");
			if (n.getAccidental().isFlat())
				string2Return = string2Return.concat("b");
			if (n.getAccidental().isSharp())
				string2Return = string2Return.concat("#");
