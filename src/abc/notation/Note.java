 ...
 * with a strict duration equals to <TT>Note.SIXTEENTH</TT>. <B>Dots</B><BR/>
 * <IMG src="../../images/multiDots.jpg"/><BR/>
 * When a note is dotted, its strict duration remains unchanged. The only
 * difference between a non-dotted note and a dotted one can be retrieved from
 * the method <TT>countDots()</TT> that returns the number of dots for a given
 * <TT>Note</TT> instance. So in the example above :
 * <UL>
...
public class Note extends NoteAbstract implements Cloneable {
	 ...
	 */
	public static final short SIXTEENTH = LENGTH_RESOLUTION * 4; // double
																	// croche
	...
	public static final short SIXTY_FOURTH = LENGTH_RESOLUTION; // quadruple
																// croche
	 ...
	 * major, C minor).
	 * 
	 * <B>Be careful!</B> note without accidental ({@link Accidental#NONE}) is
	 * considered as NATURAL. In fact it depends on the key! If height = Note.E
	 * and accidental = NONE, it could be E flat if key is Bb major, C minor...!
	 * 
	 * @param midiHeight
	 * @param key
	 */
	 ...
	 *            The octave transposition for this note : 1, 2 or 3 means
	 *            "1, 2 or 3 octave(s) higher than the reference octave" and -1,
	 *            -2 or -3 means
	 *            "1, 2 or 3 octave(s) less than the reference octave".
	 * @see #setAccidental(byte)
	 ...
	 * <B>takes into account</B> octave transposition. This height is not the
	 * height of the note itself (like how it would be played using midi for
	 * instance) but the height of its representation on a score. For instance 2
	 * notes written C and C# would have the same value returned by getHeight().
	 * They would only differ with their accidental value returned by
	 * {@link #getAccidental()}.
	 * 
	 ...
	 *            constants such as <TT>Note.WHOLE</TT>, <TT>Note.HALF</TT> etc
	 *            etc ...
	 ...
	 *            {@link #LONG}, {@link #BREVE}, {@link #WHOLE}, {@link #HALF},
	 *            {@link #QUARTER}, {@link #EIGHTH}, {@link #SIXTEENTH},
	 ...
	 *         <TT>Note.SIXTEENTH</TT>, <TT>Note.THIRTY_SECOND</TT>,
	 *         <TT>Note.SIXTY_FOURTH</TT> or -1 if this note duration is not
	 *         expressed using (strict duration + dots + tuplet) but with an
	 *         exotic duration (that can be retrieved using getDuration() in
	 *         that case).
	 * @see #getDuration()
	 */
	 ...
	 *         constants such as <TT>Note.WHOLE</TT>, <TT>Note.HALF</TT> etc etc
	 *         ...
	 * @see #setLength(short)
	 * @see #getStrictDuration()
	 */
	public short getDuration() {
		if (m_duration != -1)
			// The duration has been set in an absolute manner (not recommanded,
			// but
			// can be usefull
			return m_duration;
		else {
	 ...
	 *         <TT>Accidental.NATURAL</TT>, <TT>Accidental.FLAT</TT>,
	 *         <TT>Accidental.SHARP</TT> or <TT>Accidental.NONE</TT>.
	 * @see #setAccidental(byte)
	 */
	...
	/**
	 * A convenient method that returns <TT>true</TT> if this note is a rest. A
	 * note is a rest if its height returned by {@link #getHeight()} or
	...
	/**
	 * Returns <TT>true</TT> if the duration of the note is one of the following
	 * : {@link #LONG}, {@link #BREVE}, {@link #WHOLE}, {@link #HALF},
	 * {@link #QUARTER}, {@link #EIGHTH}, {@link #SIXTEENTH},
	 * {@link #THIRTY_SECOND}, {@link #SIXTY_FOURTH}.
	 * 
	...
	public static int getLowestNoteIndex(NoteAbstract[] notes) {
		...
		int lowestNoteHeight = (lowestNote instanceof MultiNote) ? ((MultiNote) lowestNote)
				.getLowestNote().getHeight() : ((Note) lowestNote).getHeight();
		int index = 0;
		for (int i = 1; i < notes.length; i++) {
			int currentNoteHeight = (notes[i] instanceof MultiNote) ? ((MultiNote) notes[i])
					.getLowestNote().getHeight() : ((Note) notes[i])
					.getHeight();
	...
	public static int getHighestNoteIndex(NoteAbstract[] notes) {
		...
		int highestNoteHeight = (highestNote instanceof MultiNote) ? ((MultiNote) highestNote)
				.getHighestNote().getHeight() : ((Note) highestNote)
				.getHeight();
		int index = 0;
		for (int i = 1; i < notes.length; i++) {
			int currentNoteHeight = (notes[i] instanceof MultiNote) ? ((MultiNote) notes[i])
					.getHighestNote().getHeight() : ((Note) notes[i])
					.getHeight();
