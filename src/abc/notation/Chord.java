...
/**
 * Chord class handle chord names and probably later some useful functions for
 * chord. <B>Do not confuse</B> with {@link abc.notation.MultiNote} which is a
 * collection of {@link abc.notation.Note}s played at the same time, what we
 * call a "chord" in music! <BR>
 * Chord names are texts added to a {@link abc.notation.NoteAbstract} object,
 * which are represented on the top of the staff. <BR>
 * In ABC, chord names are between quotes e.g.: "C"EEE2 EEE2 | "G"EGC>DE4 |
 * "F"FFF>F "C"EEE>E... (Jingle bells) <BR>
 * Complex chords can be entered, and sometimes it's not a chord! So before
 * gettings chord properties, check {@link #isChord()}.
 */
public class Chord extends MusicPresentationElement implements Cloneable {
	...
	// groupe 11 :) | groupe 11 :)
	private static final String REGEXP = "^([\\s ]*[\\(\\/]{0,2})" // spaces,
																	// opening
																	// parenthesis,
																	// /
	...
	public Chord(String chordName) {
		...
		if (m_isChord) {
			try {
				...
				boolean hasNote = (!group[1].contains("/"))
						&& (group[3].length() > 0);
				boolean hasBass = ((group[1].contains("/") && (group[3]
						.length() > 0)) || ((group[7] != null)
						&& group[7].contains("/") && (group[9] != null) && (group[9]
						.length() > 0)));
				if (hasNote || hasBass) {
					byte height = Note.convertToNoteType(group[3]);
					Accidental accidental = Accidental
							.convertToAccidental(group[4]);
					if (hasNote)
						m_note = new Note(height, accidental);
					else
						// this is a bass without note
						m_bass = new Note(height, accidental);
					if (hasNote && hasBass) {
						height = Note.convertToNoteType(group[9]);
						accidental = Accidental.convertToAccidental(group[10]);
						m_bass = new Note(height, accidental);
					}
					m_isOptional = (group[1].contains("(") && group[11]
							.contains(")"));// ==
					if (hasNote)
						m_quality = group[5];
				}
			} catch (Exception e) {
				System.err.println("Error analysing chord " + m_chordName
						+ " : " + e.getMessage());
			}
		}
	}
	...
	/**
	 * Returns the note of the chord, <TT>null</TT> if no note (e.g. only one
	 * bass in the sequence <TT>E7, /G#</TT>)
	 */
	public Note getNote() {
		return m_note;
	}
	 ...
	 * 
	 * @param unicode
	 *            If any, accidentals are not # and b, but \u266F (&#9839;) =
	 *            sharp and \u266D (&#9837;) = flat
	 */
	...
	/**
	 * Returns <TT>true</TT> if the chord text is understood has a real chord,
	 * <TT>false</TT> elsewhere.
	 */
