...
public class Clef extends MusicElement implements Cloneable {
	...
	public static Clef parseClef(String clefText, Clef previousClef) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i] == null)
				continue;
			if (args[i].startsWith("s=") || args[i].startsWith("stafflines=")) {
				try {
		...
		if (octaveTransp == -1)
			octaveTransp = (byte) (previousClef != null ? previousClef
					.getOctaveTransposition() : 0);
		if (semitonesTransp == -1)
			semitonesTransp = (byte) (previousClef != null ? previousClef
					.getSemitoneTransposition() : 0);
		if (clefName == null) {
			clefName = previousClef != null ? previousClef.getName() : "G";
			lineNumber = (byte) (previousClef != null ? previousClef
					.getLineNumber() : 2);
		}
		if (lineNumber == -1) {
			if (previousClef != null)
				lineNumber = (byte) previousClef.getLineNumber();
			else if (clefName.equals(G.m_clefName))
				lineNumber = G.m_lineNumber;
			else if (clefName.equals(F.m_clefName))
				lineNumber = C.m_lineNumber;
			else if (clefName.equals(C.m_clefName))
				lineNumber = F.m_lineNumber;
			else if (clefName.equals(PERC.m_clefName))
				lineNumber = PERC.m_lineNumber;
			else {
				System.err
						.println("Can't determinate clef's line number, set to 2 (treble default)");
				lineNumber = G.m_lineNumber;
			}
		}
		if (staffLines == -1)
			staffLines = (byte) (previousClef != null ? previousClef
					.getStaffLines() : 5);

		return new Clef(clefName, lineNumber, octaveTransp, semitonesTransp,
				staffLines);
	}
	...
	/**
	 * Number of the line staff where clef is printed. 2 for treble, 3 for alto,
	 * 4 for tenor, 4 for bass...
	 */
	private byte m_lineNumber = -1;
	/** Transposition in octave, +1 for *va, -1 for *vb... */
	private byte m_octaveTransposition = 0;
	/**
	 * Transposition in semitones, doesn't affect printed output, only audio
	 * rendition.
	 */
	 ...
	 * @param octaveTranspo
	 *            Transposition in octave, +1 for *va, -1 for *vb...
	 * @param semitoneTranspo
	 *            Transposition in semitones, doesn't affect printed output,
	 *            only audio rendition.
	 * @param staffLines
	 *            Number of lines in the staff (1 for perc, 5 for all others)
	 */
	...
	/**
	 * Returns the reference note, <TT>G</TT> for G clef, <TT>G'</TT> for G + 1
	 * octave, <TT>F,</TT> for bass clef.
	 * 
	 * @return
	 */
	public Note getReferenceNote() {
		if (m_referenceNote == null) {
			if (getName() != null) {
				...
				m_referenceNote
						.setOctaveTransposition((byte) (octav + getOctaveTransposition()));
			}
		}
	...
	/**
	 * Return the high first note which needs an extended staff line. <BR>
	 * For a 5 lines staff, it's a seventh over the middle note. For standard
	 * treble G clef, returns <TT>a</TT>.
	 */
	public Note getNoteFirstExtendedLineHigh() {
		if (m_highNote == null) {
			...
			Interval interv = new Interval(label, label == 5 ? Interval.PERFECT
	...
	/**
	 * Return the low first note which needs an extended staff line. <BR>
	 * For a 5 lines staff, it's a seventh under the middle note. For standard
	 * treble G clef, returns <TT>C</TT>.
	 */
	...
	/**
	 * Returns the note at the middle of the staff. <BR>
	 * Generally, notes lower or equal will have stems up, and notes higher will
	 * have stems down. <BR>
		...
		if (m_middleNote == null) {
			...
			if ((ref != null) && (getLineNumber() != -1)) {
				if (m_lineNumber == 3 || isPerc()) {
					...
					case 5:
						interval = Interval
								.reverseOrder(Interval.PERFECT_FIFTH);
						break;
					}
	...
	public boolean equals(Clef otherClef) {
		return ((m_clefName != null && m_clefName.equals(otherClef.getName())) || ((m_clefName == null) && (otherClef.m_clefName == null)))
	...
	/**
	 * Returns the transposition in semitones, doesn't affect printed output,
	 * only audio rendition.
	 */
	...
	/**
	 * Sets the transposition in semitones, doesn't affect printed output, only
	 * audio rendition.
	 */
	...
	/**
	 * Returns the number of the line staff where clef is printed. 2 for treble,
	 * 3 for alto, 4 for tenor, 4 for bass...
	 */
	...
	/**
	 * Sets the number of the line staff where clef is printed. 2 for treble, 3
	 * for alto, 4 for tenor, 4 for bass...
	 */
	...
	/**
	 * Returns the number of lines in the staff (1 for perc, 5 for all others)
	 */
