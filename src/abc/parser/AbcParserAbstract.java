...
/**
 * Abstract class from which all abc notation parsers inherit. Known
 * limitations: ELEMSKIP is not supported. Missing support for tex command and
 * file fields.
 */
//
...
public class AbcParserAbstract {
	protected static final Set FIRST_END_OF_LINE = new Set(
			AbcTokenType.LINE_FEED).union(AbcTokenType.SPACE).union(
			AbcTokenType.COMMENT);
	protected static final Set FIRST_PART_SPEC = new Set(AbcTokenType.PART)
			.union(AbcTokenType.PARENTHESIS_OPEN);
	protected static final Set FIRST_PARTS = FIRST_PART_SPEC;

	protected static final Set FIRST_NOTE_LENGTH_STRICT = new Set(
			AbcTokenType.NUMBER);

	protected static final Set FIRST_TEMPO = new Set(AbcTokenType.NUMBER)
			.union(AbcTokenType.C_TEMPO).union(FIRST_NOTE_LENGTH_STRICT);

	protected static final Set FIRST_METER_FRACTION = new Set(
			AbcTokenType.NUMBER);
	protected static final Set FIRST_METER = new Set(FIRST_METER_FRACTION)
			.union(AbcTokenType.C_METER);

	protected static final Set FIRST_GLOBAL_ACCIDENTAL = new Set(
			AbcTokenType.ACCIDENTAL);
	...
	protected static final Set FIRST_KEY_ACCIDENTAL = new Set(
			AbcTokenType.KEY_ACCIDENTAL);
	protected static final Set FIRST_KEYNOTE = new Set(AbcTokenType.BASE_NOTE);
	protected static final Set FIRST_KEYSPEC = new Set(FIRST_KEYNOTE).union(
			FIRST_MODE).union(AbcTokenType.CLEF);
	protected static final Set FIRST_KEY = new Set(FIRST_KEYSPEC)
			.union(AbcTokenType.KEY_HP);

	protected static final Set FIRST_FIELD_KEY = new Set(AbcTokenType.FIELD_KEY);
	protected static final Set FIRST_FIELD_TRANSCRNOTES = new Set(
			AbcTokenType.FIELD_TRANSCRNOTES);
	protected static final Set FIRST_FIELD_SOURCE = new Set(
			AbcTokenType.FIELD_SOURCE);
	protected static final Set FIRST_FIELD_RHYTHM = new Set(
			AbcTokenType.FIELD_RHYTHM);
	protected static final Set FIRST_FIELD_TEMPO = new Set(
			AbcTokenType.FIELD_TEMPO);
	protected static final Set FIRST_FIELD_PARTS = new Set(
			AbcTokenType.FIELD_PARTS);
	protected static final Set FIRST_FIELD_ORIGIN = new Set(
			AbcTokenType.FIELD_ORIGIN);
	protected static final Set FIRST_FIELD_NOTES = new Set(
			AbcTokenType.FIELD_NOTES);
	protected static final Set FIRST_FIELD_METER = new Set(
			AbcTokenType.FIELD_METER);
	protected static final Set FIRST_FIELD_DEFAULT_LENGTH = new Set(
			AbcTokenType.FIELD_DEFAULT_LENGTH);
	protected static final Set FIRST_FIELD_INFORMATION = new Set(
			AbcTokenType.FIELD_INFORMATION);
	protected static final Set FIRST_FIELD_HISTORY = new Set(
			AbcTokenType.FIELD_HISTORY);
	protected static final Set FIRST_FIELD_GROUP = new Set(
			AbcTokenType.FIELD_GROUP);
	protected static final Set FIRST_FIELD_FILE = new Set(
			AbcTokenType.FIELD_FILEURL);
	protected static final Set FIRST_FIELD_ELEMSKIP = new Set(
			AbcTokenType.FIELD_ELEMSKIP);
	protected static final Set FIRST_FIELD_DISCOGRAPHY = new Set(
			AbcTokenType.FIELD_DISCOGRAPHY);
	protected static final Set FIRST_FIELD_COMPOSER = new Set(
			AbcTokenType.FIELD_COMPOSER);
	protected static final Set FIRST_FIELD_BOOK = new Set(
			AbcTokenType.FIELD_BOOK);
	protected static final Set FIRST_FIELD_AREA = new Set(
			AbcTokenType.FIELD_AREA);
	protected static Set FIRST_COMMENT = new Set(AbcTokenType.COMMENT);
	protected static final Set FIRST_OTHER_FIELDS = new Set(FIRST_FIELD_AREA)
			.union(FIRST_FIELD_BOOK).union(FIRST_FIELD_COMPOSER)
			.union(FIRST_FIELD_DISCOGRAPHY).union(FIRST_FIELD_ELEMSKIP)
			.union(FIRST_FIELD_FILE).union(FIRST_FIELD_GROUP)
			.union(FIRST_FIELD_HISTORY).union(FIRST_FIELD_INFORMATION)
			.union(FIRST_FIELD_DEFAULT_LENGTH).union(FIRST_FIELD_METER)
			.union(FIRST_FIELD_NOTES).union(FIRST_FIELD_ORIGIN)
			.union(FIRST_FIELD_PARTS).union(FIRST_FIELD_RHYTHM)
			.union(FIRST_FIELD_SOURCE).union(FIRST_FIELD_TEMPO)
			.union(FIRST_FIELD_TRANSCRNOTES).union(FIRST_COMMENT);
	protected static final Set FIRST_FIELD_TITLE = new Set(
			AbcTokenType.FIELD_TITLE);
	protected static final Set FIRST_FIELD_NUMBER = new Set(
			AbcTokenType.FIELD_NUMBER);
	...
	protected static final Set FIRST_NO_LINE_BREAK = new Set(
			AbcTokenType.NO_LINE_BREAK);
	protected static final Set FIRST_LINE_BREAK = new Set(
			AbcTokenType.LINE_BREAK);

	protected static final Set FIRST_SPACE = new Set(AbcTokenType.SPACE);
	// protected static Set FIRST_TEX_COMMAND = new
	// Set(AbcTokenType.TEX_COMMAND);

	protected static final Set FIRST_USER_DEFINED = new Set(
			AbcTokenType.USER_DEFINED);

	protected static final Set FIRST_FIELD_WORDS = new Set(
			AbcTokenType.FIELD_WORDS);
	protected static final Set FIRST_FIELD_PART = new Set(
			AbcTokenType.FIELD_PARTS);
	protected static final Set FIRST_TUNE_FIELD = new Set(/*
														 * FIRST_FIELD_ELEMSKIP).
														 * union(
														 */FIRST_FIELD_KEY)
			.union(FIRST_FIELD_DEFAULT_LENGTH).union(FIRST_FIELD_METER)
			.union(FIRST_FIELD_PART).union(FIRST_FIELD_TEMPO)
			.union(FIRST_FIELD_TITLE).union(FIRST_FIELD_WORDS);
	...
	protected static final Set FIRST_BEGIN_SLUR = new Set(
			AbcTokenType.BEGIN_SLUR);
	protected static final Set FIRST_NTH_REPEAT = new Set(
			AbcTokenType.NTH_REPEAT);
	protected static final Set FIRST_BARLINE = new Set(AbcTokenType.BARLINE);

	protected static final Set FIRST_CHORD_TYPE = new Set(
			AbcTokenType.CHORD_TYPE);
	protected Set FIRST_FORMAL_CHORD = new Set(FIRST_BASE_NOTE);
	protected static final Set FIRST_GUITAR_CHORD = new Set(
			AbcTokenType.GUITAR_CHORD);

	protected static final Set FIRST_ACCIACCATURA = new Set(
			AbcTokenType.ACCIACCATURA);
	protected static final Set FIRST_GRACE_NOTES = new Set(
			AbcTokenType.GRACING_BEGIN);
	protected static final Set FIRST_GRACINGS = new Set(AbcTokenType.GRACING);
	protected static final Set FIRST_SYMBOL_BEGIN = new Set(
			AbcTokenType.SYMBOL_BEGIN);
	protected static final Set FIRST_SYMBOL = new Set(AbcTokenType.SYMBOL);
	protected static final Set FIRST_ANNOTATION_BEGIN = new Set(
			AbcTokenType.ANNOTATION_BEGIN);
	protected static final Set FIRST_ANNOTATION = new Set(
			AbcTokenType.ANNOTATION);
	protected static final Set FIRST_TIE = new Set(AbcTokenType.TIE);
	protected static final Set FIRST_BROKEN_RHYTHM = new Set(
			AbcTokenType.BROKEN_RHYTHM);
	...
	protected static final Set FIRST_ACCIDENTAL = new Set(
			AbcTokenType.ACCIDENTAL);
	protected static final Set FIRST_NOTE_LENGTH = new Set(AbcTokenType.NUMBER)
			.union(AbcTokenType.FRACTION);
	protected static final Set FIRST_OCTAVE = new Set(AbcTokenType.OCTAVE);
	protected static final Set FIRST_PITCH = new Set(FIRST_ACCIDENTAL)
			.union(FIRST_BASE_NOTE);
	protected static final Set FIRST_NOTE_OR_REST = new Set(FIRST_PITCH)
			.union(FIRST_REST);
	protected static final Set FIRST_NOTE = new Set(FIRST_NOTE_OR_REST);
	protected static final Set FIRST_MULTI_NOTE = new Set(
			AbcTokenType.MULTI_NOTE_BEGIN);
	protected static final Set FIRST_NOTE_STEM = new Set()
			// .union(FIRST_SYMBOL_BEGIN)
			// .union(FIRST_GUITAR_CHORD)
			.union(FIRST_GRACE_NOTES).union(FIRST_GRACINGS).union(FIRST_NOTE)
			.union(FIRST_MULTI_NOTE);
	...
	protected static final Set FIRST_TUPLET_SPEC = new Set(
			AbcTokenType.TUPLET_SPEC);
	protected static final Set FIRST_TUPLET_ELEMENT = new Set(FIRST_TUPLET_SPEC);

	protected static final Set FIRST_LINE_ENDER = new Set(AbcTokenType.COMMENT)
			.union(AbcTokenType.LINE_FEED).union(AbcTokenType.LINE_BREAK)
			.union(AbcTokenType.NO_LINE_BREAK);
	protected static final Set FIRST_ELEMENT = new Set(FIRST_SYMBOL_BEGIN)
			.union(FIRST_ANNOTATION_BEGIN).union(FIRST_SPACER)
			.union(FIRST_NOTE_ELEMENT).union(FIRST_TUPLET_ELEMENT)
			.union(AbcTokenType.BARLINE).union(AbcTokenType.NTH_REPEAT)
			.union(AbcTokenType.BEGIN_SLUR).union(AbcTokenType.END_SLUR)
			.union(AbcTokenType.SPACE)
	/* .union(AbcTokenType.USER_DEFINED) */;
	protected static Set FIRST_ABC_LINE = new Set(FIRST_ELEMENT)
			.union(FIRST_MID_TUNE_FIELD)/*
										 * .union(FIRST_COMMENT).union(
										 * FIRST_TEX_COMMAND)
										 */;
	...
	private KeySignature lastParsedKey = new KeySignature(Note.C,
			KeySignature.MAJOR);
	 ...
	 * 
	 * @param listener
	 *            Object that implements the TuneParserListenerInterface.
	 */
	...
	protected void parseAbcFile(Set follow) {
		Set current = FIRST_ABCTUNE.createUnion(FIRST_COMMENT).createUnion(
				FIRST_LINE_FEED);
		// are missing TEX COMMAND and FILE FIELDS
		// .createUnion(FIRST_TEX_COMMAND);//.createUnion(FIRST_FILE_FIELDS);
		m_scanner.setFinaleStateAutomata(AutomataFactory.getAutomata(
				current.getTypes(), m_abcVersion));
		...
		while (m_token != null) {
			if (FIRST_ABCTUNE.contains(m_token.getType())) {
				...
				accept(AbcTokenType.LINE_FEED, current,
						(current.createUnion(follow)));
			// else if (Syntax.FIRST_TEX_COMMAND.contains(token.getType()))
			// parseAbcTune(current.createUnion(follow));
			// else if (Syntax.FIRST_FILE_FIELDS.contains(token.getType()))
			// parseAbcTune(current.createUnion(follow));
		}
	}

	/**
	 * Inits the parsing : sets the starting state of the current Set + sets the
	 * finale state automata of the scanner + retrieves the first token and its
	 * type.
	 * 
	 * @return The starting state of the current Set as it should be at the
	 *         begining of the parsing.
	 */
	/*
	 * protected Set initParsing() { Set startingSet =
	 * FIRST_ABCTUNE.createUnion(FIRST_COMMENT).createUnion(FIRST_LINE_FEED); //
	 * are missing TEX COMMAND and FILE FIELDS
	 * //.createUnion(FIRST_TEX_COMMAND);//.createUnion(FIRST_FILE_FIELDS);
	 * m_scanner
	 * .setFinaleStateAutomata(AutomataFactory.getAutomata(startingSet.getTypes
	 * ())); m_token = m_scanner.nextToken(); m_tokenType = m_token.getType();
	 * return startingSet; }
	 */
	...
	protected Tune parseAbcTune(Set follow) {
		Set current = new Set()
				./* union(FIRST_ABCHEADER). */union(FIRST_ABC_MUSIC);
	...
	/**
	 * other-fields ::= field-area / field-book / field-composer /
	 * field-discography / field-elemskip / field-group / field-history /
	 * field-information / field-default-length / field-meter / field-notes /
	 * field-origin / field-parts / field-tempo / field-rhythm / field-source /
	...
	private void parseOtherFields(Set follow) // throws TuneNotationException
	{
		...
		// as well as the set of all the FIRSTs for all other rules invoked by
		// S.
		// Object returnedObject = null;
		/*
		 * Set current =
		 * FIRST_FIELD_AREA.union(FIRST_FIELD_BOOK).union(FIRST_FIELD_COMPOSER)
		 * .
		 * createUnion(FIRST_FIELD_DISCOGRAPHY).createUnion(FIRST_FIELD_ELEMSKIP
		 * ).createUnion(FIRST_FIELD_GROUP)
		 * .createUnion(FIRST_FIELD_HISTORY).createUnion
		 * (FIRST_FIELD_INFORMATION).createUnion(FIRST_FIELD_NOTES)
		 * .createUnion(
		 * FIRST_FIELD_ORIGIN).createUnion(FIRST_FIELD_RHYTHM).createUnion
		 * (FIRST_FIELD_SOURCE)
		 * .createUnion(FIRST_FIELD_TRANSCRNOTES).createUnion(FIRST_COMMENT);
		 */
		// Set current = new Set();
		/*
		 * if (m_tokenType.equals(AbcTokenType.FIELD_AREA))
		 * current.remove(FIRST_FIELD_AREA); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_BOOK))
		 * current.remove(FIRST_FIELD_BOOK); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_COMPOSER))
		 * current.remove(FIRST_FIELD_COMPOSER); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_DISCOGRAPHY))
		 * current.remove(FIRST_FIELD_DISCOGRAPHY); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_ELEMSKIP))
		 * current.remove(FIRST_FIELD_ELEMSKIP); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_GROUP))
		 * current.remove(FIRST_FIELD_GROUP); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_HISTORY))
		 * current.remove(FIRST_FIELD_HISTORY); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_INFORMATION))
		 * current.remove(FIRST_FIELD_INFORMATION); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_DEFAULT_LENGTH))
		 * current.remove(FIRST_FIELD_DEFAULT_LENGTH); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_METER))
		 * current.remove(FIRST_FIELD_METER); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_NOTES))
		 * current.remove(FIRST_FIELD_NOTES); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_ORIGIN))
		 * current.remove(FIRST_FIELD_ORIGIN); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_RHYTHM))
		 * current.remove(FIRST_FIELD_RHYTHM); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_SOURCE))
		 * current.remove(FIRST_FIELD_SOURCE); else if
		 * (m_tokenType.equals(AbcTokenType.FIELD_TRANSCRNOTES))
		 * current.remove(FIRST_FIELD_TRANSCRNOTES);
		 */

		if (m_tokenType.equals(AbcTokenType.FIELD_HISTORY))
			parseFieldHistory(follow);
		else if (m_tokenType.equals(AbcTokenType.FIELD_DEFAULT_LENGTH)) {
			short defaultNoteLength = parseFieldDefaultLength(follow);
			if (defaultNoteLength != -1)
				m_defaultNoteLength = defaultNoteLength;
		} else if (m_tokenType.equals(AbcTokenType.FIELD_METER)) {
		...
		} else if (m_tokenType.equals(AbcTokenType.FIELD_PARTS)) {
			MultiPartsDefinition parts = parseFieldParts(follow);
			if (parts != null)
				m_tune.setMultiPartsDefinition(parts);
		} else if (m_tokenType.equals(AbcTokenType.FIELD_TEMPO)) {
			Tempo tempo = parseFieldTempo(follow);
			if (tempo != null)
				m_music.addElement(currentVoice, tempo);
		} else if (m_tokenType.equals(AbcTokenType.COMMENT)) {
			// current.remove(AbcTokenType.COMMENT);
			parseComment(follow);
		} else {
			AbcTextField field = parseField(m_tokenType, follow);
			if (field != null)
				if (field.getType() == AbcTextField.AREA)
					m_tune.setArea(field.getText());
				else if (field.getType() == AbcTextField.BOOK)
					m_tune.addBook(field.getText());
				else if (field.getType() == AbcTextField.COMPOSER)
					m_tune.addComposer(field.getText());
				else if (field.getType() == AbcTextField.DISCOGRAPHY)
					m_tune.addDiscography(field.getText());
				else if (field.getType() == AbcTextField.FILEURL)
					m_tune.setFileURL(field.getText());
				else if (field.getType() == AbcTextField.GROUP)
					m_tune.addGroup(field.getText());
				else if (field.getType() == AbcTextField.HISTORY)
					m_tune.addHistory(field.getText());
				else if (field.getType() == AbcTextField.INFORMATION)
					m_tune.addInformation(field.getText());
				else if (field.getType() == AbcTextField.NOTES)
					m_tune.addNotes(field.getText());
				else if (field.getType() == AbcTextField.ORIGIN)
					m_tune.setOrigin(field.getText());
				else if (field.getType() == AbcTextField.RHYTHM)
					m_tune.setRhythm(field.getText());
				else if (field.getType() == AbcTextField.SOURCE)
					m_tune.addSource(field.getText());
				else if (field.getType() == AbcTextField.TRANSCRNOTES)
					m_tune.addTranscriptionNotes(field.getText());
		}
	}

	private AbcTextField parseField(TokenType tokenType, Set follow) {
		// System.out.println("TuneParser - parseFieldArea(" + token + ")");
		Set current = new Set().union(FIRST_END_OF_LINE).union(
				AbcTokenType.TEXT);
		...
		if (ret != null && text != null) {
			if (tokenType.equals(AbcTokenType.FIELD_AREA))
				return new AbcTextField(AbcTextField.AREA, text);
			else if (tokenType.equals(AbcTokenType.FIELD_BOOK))
				return new AbcTextField(AbcTextField.BOOK, text);
			else if (tokenType.equals(AbcTokenType.FIELD_COMPOSER))
				return new AbcTextField(AbcTextField.COMPOSER, text);
			else if (tokenType.equals(AbcTokenType.FIELD_DISCOGRAPHY))
				return new AbcTextField(AbcTextField.DISCOGRAPHY, text);
			// else if (tokenType.equals(AbcTokenType.FIELD_ELEMSKIP)) return
			// new AbcTextField(AbcTextField.ELEMSKIP, text);
			else if (tokenType.equals(AbcTokenType.FIELD_FILEURL))
				return new AbcTextField(AbcTextField.FILEURL, text);
			else if (tokenType.equals(AbcTokenType.FIELD_GROUP))
				return new AbcTextField(AbcTextField.GROUP, text);
			else if (tokenType.equals(AbcTokenType.FIELD_INFORMATION))
				return new AbcTextField(AbcTextField.INFORMATION, text);
			else if (tokenType.equals(AbcTokenType.FIELD_NOTES))
				return new AbcTextField(AbcTextField.NOTES, text);
			else if (tokenType.equals(AbcTokenType.FIELD_ORIGIN))
				return new AbcTextField(AbcTextField.ORIGIN, text);
			else if (tokenType.equals(AbcTokenType.FIELD_RHYTHM))
				return new AbcTextField(AbcTextField.RHYTHM, text);
			else if (tokenType.equals(AbcTokenType.FIELD_SOURCE))
				return new AbcTextField(AbcTextField.SOURCE, text);
			else if (tokenType.equals(AbcTokenType.FIELD_TITLE))
				return new AbcTextField(AbcTextField.TITLE, text);
			else if (tokenType.equals(AbcTokenType.FIELD_TRANSCRNOTES))
				return new AbcTextField(AbcTextField.TRANSCRNOTES, text);
			else if (tokenType.equals(AbcTokenType.FIELD_WORDS))
				return new AbcTextField(AbcTextField.WORDS, text);
			return null;
		} else
			return null;
	}
	...
	private short parseFieldDefaultLength(Set follow) {
		// System.out.println("TuneParser - parseFieldArea(" + token + ")");
		Set current = new Set().union(FIRST_NOTE_LENGTH_STRICT).union(
				FIRST_END_OF_LINE);
	...
	private MultiPartsDefinition parseParts(Set follow) {
		...
		do {
			RepeatedPartAbstract partSpec = parsePartSpec(current
					.createUnion(follow));
			if (partSpec != null)
	...
	private RepeatedPartAbstract parsePartSpec(Set follow) {
		Set current = new Set().union(FIRST_PART_SPEC).union(AbcTokenType.PART)
				.union(AbcTokenType.PARENTHESIS_OPEN)
				.union(AbcTokenType.PARENTHESIS_CLOSE)
				.union(AbcTokenType.DIGIT);
		...
		{
			current.remove(AbcTokenType.PART);
			String partNameAsString = accept(AbcTokenType.PART, current,
					follow, true);
			current.remove(AbcTokenType.PARENTHESIS_OPEN);
			current.remove(AbcTokenType.PARENTHESIS_CLOSE);
	...
	private Tempo parseTempo(Set follow) {
		...
		if (m_tokenType.equals(AbcTokenType.C_TEMPO)) {
			current = new Set().union(FIRST_NOTE_LENGTH)
					.union(AbcTokenType.EQUALS).union(AbcTokenType.NUMBER);
			short refLength = -1;
			...
			if (FIRST_NOTE_LENGTH.contains(m_tokenType)) {
				...
				if (length != null)
					refLength = (short) (m_defaultNoteLength * length
							.floatValue());
			}
			...
			if (m_tokenType.equals(AbcTokenType.FRACTION)) {
				...
				current = new Set(AbcTokenType.NUMBER)
						.union(AbcTokenType.EQUALS);
				accept(AbcTokenType.FRACTION, current, follow);
				...
				if (num != null && denom != null && tempoString != null)
					try {
						tempo = new Tempo(Note.convertToNoteLengthStrict(
								new Integer(num).intValue(),
								new Integer(denom).intValue()), new Short(
								tempoString).shortValue());
			...
			if (tempoString != null)
				tempo = new Tempo(m_defaultNoteLength,
						new Short(tempoString).shortValue());
		}
		return tempo;
	}
	 ...
	 * 
	 * clef ::= ( ("clef=" (clef-note / clef-name)) / clef-name) clef-line ["+8"
	 * / "-8"] [1*WSP clef-middle]<br>
	 * clef-note ::= "G" / "C" / "F" / "P"<br>
	 * clef-name ::= "treble" / "alto" / "tenor" / "baritone" / "bass" / "mezzo"
	 * / "soprano" / "perc" / "none" ; Maybe also Doh1-4, Fa1-4<br>
	...
	private KeySignature parseKeySpec(Set follow) {
		Set current = new Set().union(AbcTokenType.SPACE)
				.union(FIRST_MODE_SPEC).union(AbcTokenType.SPACE)
				.union(FIRST_GLOBAL_ACCIDENTAL).union(AbcTokenType.CLEF);
		KeySignature key = null;
		Note note = null;
		...
		while (m_tokenType.equals(AbcTokenType.SPACE) || foundaspace) {
			...
			if (FIRST_GLOBAL_ACCIDENTAL.contains(m_tokenType)) {
				...
				if (ga != null && key != null) {
					key.setAccidental(((Byte) ga[0]).byteValue(),
							(Accidental) ga[1]);
		...
		if (m_tokenType.equals(AbcTokenType.CLEF)) {
			...
			while (m_tokenType.equals(AbcTokenType.CLEF))
				clefText += accept(AbcTokenType.CLEF, current, follow, true);
			current.remove(AbcTokenType.CLEF);
			// don't understand why, but "Cphr bass" token CLEF starts at
			// "phr..."
			// so mode has not been set (no token MODE SPEC)
			// Em "m" is in clef token
			if (!modeSpecFound
					&& ((clefText.length() == 1) || (clefText.length() >= 3))) {
				String maybeMode = clefText.length() == 1 ? clefText // maybe
																		// it's
																		// "m"
																		// of
																		// minor
				...
				if (modeSpec2 != KeySignature.OTHER) {
					...
					key = new KeySignature(key.getNote(), key.getAccidental(),
							modeSpec2);
	...
	private void parseEndOfLine(Set follow) {
		Set current = new Set(AbcTokenType.SPACE).union(AbcTokenType.COMMENT)
				.union(AbcTokenType.TEXT).union(AbcTokenType.LINE_FEED);
	...
	/**
	 * abc-header ::= field-number *comment 1*field-title *other-fields
	 * field-key In practice, many tunes are e-mailed without field-number, so
	 * those wishing to implement an abc parser should treat this ; field as
	 * optional.
	 */
	protected Tune parseAbcHeader(Set follow) {
		...
		;
		Set current = new Set().union(FIRST_COMMENT).union(FIRST_FIELD_TITLE)
				.union(FIRST_OTHER_FIELDS).union(FIRST_FIELD_KEY);
		...
		// COMMENT is not considered as part as the follow when parsing the
		// title
		// => false because a comment can occur after a title : it's part of
		// other
		// fields.
		current = new Set().union(FIRST_FIELD_TITLE).union(FIRST_OTHER_FIELDS)
				.union(FIRST_FIELD_KEY);

		do {
			AbcTextField title = parseField(AbcTokenType.FIELD_TITLE,
					current.createUnion(follow));
			if (title != null)
				m_tune.addTitle(title.getText());
		...
		if (key != null)
			m_music.addElement(currentVoice, key);
		return m_tune;
	}
	...
	private Object[] parseGlobalAccidental(Set follow) {
		...
		String noteHeigthString = accept(AbcTokenType.BASE_NOTE, current,
				follow);
		// byte index = 0;
	...
	protected void parseComment(Set follow) {
		...
		// String[] endOfLine =
		// {AbcTokenType.LINE_FEED,AbcTokenType.LINE_BREAK,AbcTokenType.NO_LINE_BREAK};
		if (m_tokenType.equals(AbcTokenType.LINE_BREAK))
			accept(AbcTokenType.LINE_BREAK, current, follow);
		else if (m_tokenType.equals(AbcTokenType.NO_LINE_BREAK))
	...
	protected void parseAbcMusic(Set follow) {
		Set current = new Set().union(FIRST_ABC_LINE).union(
				AbcTokenType.LINE_FEED);
		do
	...
	protected void parseAbcLine(Set follow) {
		...
		if (FIRST_ELEMENT.contains(m_tokenType)) {
			Set current = new Set().union(FIRST_ELEMENT)
					.union(FIRST_LINE_ENDER);
			...
			MusicPresentationElement lineEnder = parseLineEnder(current
					.createUnion(follow));
			if (lineEnder != null)
				m_music.addElement(currentVoice, lineEnder);
		} else if (FIRST_MID_TUNE_FIELD.contains(m_tokenType))
	 ...
	 * field-meter / field-part / field-tempo / field-title / field-words
	...
	private void parseTuneField(Set follow) {
		 ...
		 * parseField(AbcTokenType.FIELD_ELEMSKIP, follow); else
		 */
		if (FIRST_FIELD_KEY.contains(m_tokenType)) {
			...
			if (key != null)
				m_music.addElement(currentVoice, key);
			lastParsedKey = key;
		} else if (FIRST_FIELD_DEFAULT_LENGTH.contains(m_tokenType)) {
			short defaultNoteLength = parseFieldDefaultLength(follow);
			if (defaultNoteLength != -1)
				m_defaultNoteLength = defaultNoteLength;
		} else if (FIRST_FIELD_METER.contains(m_tokenType)) {
			TimeSignature meter = parseFieldMeter(follow);
			if (meter != null) {
				m_music.addElement(currentVoice, meter);
				m_defaultNoteLength = meter.getDefaultNoteLength();
		...
		} else if (FIRST_FIELD_PART.contains(m_tokenType)) {
			char partLabel = parseFieldPart(follow);
			if (m_tune.getPart(partLabel) != null)
				m_music = m_tune.getPart(partLabel).getMusic();
			else
				// this part hasn't been used in the multi part definition but
				// it's defined.
				m_music = m_tune.createPart(partLabel).getMusic();
		} else if (FIRST_FIELD_TEMPO.contains(m_tokenType)) {
			Tempo tempo = parseFieldTempo(follow);
			if (tempo != null)
				m_music.addElement(currentVoice, tempo);
		} else if (FIRST_FIELD_WORDS.contains(m_tokenType)) {
	...
	private void parseElement(Set follow) {
		...
		if (FIRST_SPACER.contains(m_tokenType)) {
			...
			if (decorations.size() > 0)
				spacer.setDecorations((Decoration[]) decorations
						.toArray(new Decoration[1]));
		...
		if (FIRST_NOTE_ELEMENT.contains(m_tokenType)) {
			// NoteAbstract note =
			// parseNoteElement(current.createUnion(follow));
			// Experimentation
			NoteAbstract note = parseNoteElement(follow);
			/*
			 * if (!slursDefinitionStack.isEmpty()){ note.setPartOfSlur(true);
			 * SlurDefinition currentSlurDef =
			 * (SlurDefinition)slursDefinitionStack
			 * .elementAt(slursDefinitionStack.size()-1); if
			 * (currentSlurDef.getStart()==null) currentSlurDef.setStart(note);
			 * }
			 */
			if (note != null) {
				...
				if (decorations.size() > 0)
					note.setDecorations((Decoration[]) decorations
							.toArray(new Decoration[1]));
				if (annotations.size() > 0)
		...
		} else if (FIRST_TUPLET_ELEMENT.contains(m_tokenType)) {
			// Experimentation
			// Tuplet tuplet = parseTupletElement(current.createUnion(follow));
			Tuplet tuplet = parseTupletElement(follow);
			// tuplet is not put in the score itself, but notes composing the
			// tuplet
		...
		} else if (FIRST_BARLINE.contains(m_tokenType)) {
			byte[] barLineTypes = BarLine.convertToBarLine(accept(
					AbcTokenType.BARLINE, null, follow));
			for (int i = 0; i < barLineTypes.length; i++) {
				BarLine barline = new BarLine(barLineTypes[i]);
				if (i == 0) {
					barline.setDynamic(dynamic);
					if (decorations.size() > 0)
						barline.setDecorations((Decoration[]) decorations
								.toArray(new Decoration[1]));
					if (annotations.size() > 0)
		...
		} else if (FIRST_NTH_REPEAT.contains(m_tokenType)) {
			byte[] repeatNumbers = convertToRepeatBarLine(accept(
					AbcTokenType.NTH_REPEAT, null, follow));
			if (repeatNumbers.length > 0)
				m_music.addElement(currentVoice, new RepeatBarLine(
						repeatNumbers));
			else
				m_music.addElement(currentVoice, new BarLine());
		} else if (m_tokenType.equals(AbcTokenType.BEGIN_SLUR)) {
			accept(AbcTokenType.BEGIN_SLUR, null, follow);
			SlurDefinition def = new SlurDefinition();
			slursDefinitionStack.addElement(def);
			// m_isPartOfSlur = true;
		} else if (m_tokenType.equals(AbcTokenType.END_SLUR)) {
			...
			if (!slursDefinitionStack.isEmpty()) {
				...
				while (i >= 0) {
					SlurDefinition slurDef = (SlurDefinition) slursDefinitionStack
							.elementAt(i);
		...
		} else if (m_tokenType.equals(AbcTokenType.SPACE)) {
			accept(AbcTokenType.SPACE, null, follow);
			NoteAbstract lastScoreNote = m_music.getVoice(currentVoice)
					.getLastNote();
			if (lastScoreNote != null
					&& !lastScoreNote.equals(lastNoteFlaggedAsEndOfGroup))
	...
	private Tuplet parseTupletElement(Set follow) {
		...
		if (tupletSpec[1] == -1) {
			/*
			 * if (m_timeSignature!=null) if (m_timeSignature.isCoumpound())
			 * tupletSpec[1]=3; else tupletSpec[1]=2; else
			 */
			/*
			 * The values of the particular tuplets are (to quote the abc
			 * specification) (2 2 notes in the time of 3 (3 3 notes in the time
			 * of 2 (4 4 notes in the time of 3 (5 5 notes in the time of n (6 6
			 * notes in the time of 2 (7 7 notes in the time of n (8 8 notes in
			 * the time of 3 (9 9 notes in the time of n n is 3 in compound time
			 * signatures (3/4, 3/8, 9/8 etc), and 2 in simple time signatures
			 * (C, 4/4, 2/4 etc.)
			 */
			if (tupletSpec[0] == 2 || tupletSpec[0] == 4 || tupletSpec[0] == 8)
				...
				tupletSpec[1] = 2;
			else if (tupletSpec[0] == 5 || tupletSpec[0] == 7
					|| tupletSpec[0] == 9)
				...
				System.err
						.println("Cannot evaluate tuplet : no time signature");
		}
		if (tupletSpec[2] == -1)
	...
	/**
	 * tuplet-spec ::= "(" DIGIT [":" [DIGIT] [":" [DIGIT]]] ([0]:[1]:[2] ==
	 * (p:q:r p = the number of notes to be put into time q q = the time that p
	 * notes will be played in r = the number of notes to continue to do this
	 * action for. If q is not specified, it defaults to 3 in compound time
	 * signatures and 2 in simple time signatures. If r is not specified, it is
	 * taken to be the same as p.
	 */
	...
	private NoteAbstract parseNoteElement(Set follow) {
		...
		if (note != null) {
			...
			if (brknRthmDotsCorrection != 0) {
				...
				else if (brknRthmDotsCorrection < 0) {
					short correctedDuration = (short) (((Note) note)
							.getStrictDuration() / (Math.pow(2,
							-brknRthmDotsCorrection)));
					try {
		...
		if (m_tokenType.equals(AbcTokenType.BROKEN_RHYTHM)) {
			...
			String brokenRhythmString = accept(AbcTokenType.BROKEN_RHYTHM,
					current, follow);
			...
			if (note != null) {
				if (brokenRhythm > 0) {
					...
					short correctedDuration = (short) (((Note) note)
							.getStrictDuration() / (Math.pow(2, -brokenRhythm)));
					try {
						((Note) note).setStrictDuration(correctedDuration);
					} catch (IllegalArgumentException e) {
						((Note) note).setDuration(correctedDuration);
		...
		/*
		 * if (note!=null) { int length =
		 * m_scanner.getPosition().getCharactersOffset
		 * ()-beginPosition.getCharactersOffset(); try {
		 * ((PositionableNote)note).setBeginPosition(beginPosition);
		 * ((PositionableNote)note).setLength(length); }
		 * catch(ClassCastException e) //this was a multi note instance not a
		 * single note. {
	...
	/**
	 * Parse annotations "^top", "_bottom", "<left", ">right",
	...
	private Vector parseAnnotationsAndChord(Set follow) {
		...
		while (FIRST_ANNOTATION_BEGIN.contains(m_tokenType)
				|| FIRST_GUITAR_CHORD.contains(m_tokenType)) {
			Set current = new Set().union(AbcTokenType.ANNOTATION_BEGIN).union(
					AbcTokenType.ANNOTATION);// .union(AbcTokenType.SPACE);
	 ...
	 * 
	 * @param follow
	 * @return a vector of {@link abc.notation.SymbolElement} (Decoration or
	 *         Dynamic)
	 */
	private Vector parseSymbols(Set follow) {
		...
		while (FIRST_SYMBOL_BEGIN.contains(m_tokenType)) {
			Set current = new Set().union(AbcTokenType.SYMBOL_BEGIN).union(
	...
	private NoteAbstract parseNoteStem(Set follow) {
		Set current = new Set()
				// .union(FIRST_SYMBOL_BEGIN)
				// .union(FIRST_GUITAR_CHORD)
				.union(FIRST_GRACE_NOTES).union(FIRST_ACCIACCATURA)
				.union(FIRST_GRACINGS).union(FIRST_NOTE)
				.union(FIRST_MULTI_NOTE);
		...
		if (note != null) {
			...
			if (!slursDefinitionStack.isEmpty()) {
				...
				int i = slursDefinitionStack.size() - 1;
				while (i >= 0) {
					SlurDefinition currentSlurDef = (SlurDefinition) slursDefinitionStack
							.elementAt(i);
	...
	private Vector parseMultiNote(Set follow) {
		Set current = new Set().union(FIRST_NOTE).union(
				AbcTokenType.MULTI_NOTE_END);
		Vector notes = new Vector();
	...
	private Note[] parseGraceNotes(Set follow) {
		Set current = new Set().union(FIRST_PITCH).union(
				AbcTokenType.GRACING_END);
		...
		if (gracingNotes.isEmpty())
			...
			// System.arraycopy(gracingNotes.toArray(), 0, gracings,0,
			// gracingNotes.size());
			return gracings;
		}
	}
...
/** guitar-chord ::= <"> (text <"> / formal-chord */
	private String parseGuitarChord(Set follow) {
		Set current = new Set().union(AbcTokenType.GUITAR_CHORD).union(
				AbcTokenType.CHORD_NAME);
		String chordName = null;
	...
	private Note parseNote(Set follow) {
		...
		// TODO The start position is based on token info but the end pos is
		// based on scanner info : check why.
		...
		if (FIRST_NOTE_LENGTH.contains(m_tokenType)) {
			...
			if (note != null) {
				try {
					...
					// duration
					DurationDescription d = AbcToolkit.getAbsoluteDurationFor(
							length, m_defaultNoteLength);
					...
					note.setDuration((short) (m_defaultNoteLength * length
							.floatValue()));
				}
			}
		} else if (note != null)
		...
		if (note != null) {
			...
			if (isTied) {
				...
				// if (tieStartingNote!=null && note!=tieStartingNote &&
				// ((Note)note).getHeight()==tieStartingNote.getHeight()) {
				Note startTieNote = getNoteStartingTieFor(note);
				if (startTieNote != null) {
					startTieNote.getTieDefinition().setEnd(note.getReference());
					((Note) note).setTieDefinition(startTieNote
							.getTieDefinition());
	...
	private Note parseNoteOrRest(Set follow) {
		...
		if (m_tokenType.equals(AbcTokenType.REST)) {
			...
			PositionableNote note = new PositionableNote(
					Note.convertToNoteType(sNote), Accidental.NONE);
			if (sNote.equals("x"))
	...
	private Note parsePitch(Set follow) {
		...
		if (m_tokenType.equals(AbcTokenType.ACCIDENTAL))
			accidental = Accidental.convertToAccidental(accept(
					AbcTokenType.ACCIDENTAL, current, follow));
		...
		if (heigth != null)
			note = new PositionableNote(noteHeigth, accidental,
					octaveTransposition);
		// if (note!=null)
		// note.setPartOfSlur(!slursDefinitionStack.isEmpty());
		return note;
	...
	private MusicPresentationElement parseLineEnder(Set follow) {
		...
		if (FIRST_COMMENT.contains(m_tokenType))
			parseComment(follow);
		else if (m_tokenType.equals(AbcTokenType.LINE_BREAK))
			lineEnder = (accept(AbcTokenType.LINE_BREAK, null, follow) == null) ? null
					: new EndOfStaffLine();
		else if (m_tokenType.equals(AbcTokenType.NO_LINE_BREAK))
			accept(AbcTokenType.NO_LINE_BREAK, null, follow);
		else
			// if (m_tokenType.equals(AbcTokenType.LINE_FEED))
			lineEnder = (accept(AbcTokenType.LINE_FEED, null, follow) == null) ? null
					: new EndOfStaffLine();
	...
	/*
	 * private void parseTexCommand(Set follow) { Set current = new
	 * Set(AbcTokenType.TEXT).createUnion(AbcTokenType.LINE_FEED);
	 * accept(AbcTokenType.TEX_COMMAND, current, follow);
	 * current.remove(AbcTokenType.TEXT); accept(AbcTokenType.TEXT, current,
	 * follow); current.remove(AbcTokenType.LINE_FEED);
	 * accept(AbcTokenType.LINE_FEED, current, follow); }
	 */
	// HELP METHODS
	// ==================================================================================
	protected Note getNoteStartingTieFor(Note aNote) {
		for (int i = 0; i < notesStartingTies.size(); i++) {
			...
			if (((Note) notesStartingTies.elementAt(i)).getHeight() == aNote
					.getHeight())
	...
	protected String accept(TokenType tokenType, Set current, Set follow,
		...
		// System.out.println("AbcHeaderParser - Try to accept " + token +
		// " with " + set);
		String value2return = null;
		// should be .equals() in theory but as there's only one instance per
		// token
		...
		if (m_tokenType == tokenType) {
			...
			// System.out.println("AbcHeaderParser - ACCEPTED: " +
			// token.getValue()+ "(" +
			...
			if (isCurrentOptional) {
				 ...
				 * if (alreadyCreatedSetIndex!=-1) { union=
				 * (Set)m_setsForAccept.elementAt(alreadyCreatedSetIndex);
				 * //System.out.println("A set has already been created " +
				 * m_recreationNb++ + " times for " + union ); } else
				 * m_setsForAccept.addElement(union);
				 */
				m_automata.setDefinition(DefinitionFactory.getDefinition(
						union.getTypes(), m_abcVersion));
				m_scanner.setFinaleStateAutomata(m_automata);
			...
			// System.out.println(ParserTools.convertToTypeName(tokenType) +
			// " EXPECTED HERE !!!!!!!! @" + scanner.getLineNumber() + ", " +
			// m_scanner.getColumnNumber());
			if (m_token != null)
				notifyListenersForInvalidToken(m_token, m_token.getPosition(),
						tokenType);
			else
				notifyListenersForInvalidToken(null, m_scanner.getPosition(),
						tokenType);
	...
	private Set getSetResultingUnionFrom(Set current, Set follow) {
		/*
		 * Set foundSet = null; for (int i=0; i<m_setsForAccept.size() &&
		 * foundSet==null; i++) { Set currentSet =
		 * (Set)m_setsForAccept.elementAt(i); if
		 * (currentSet.size()==(set1.size()+set2.size()) &&
		 * currentSet.contains(set1) && currentSet.contains(set2) ) foundSet =
		 * currentSet; } if (foundSet==null) {
		 * //System.out.println("creating one set"); foundSet =
		 * set1.createUnion(set2); m_setsForAccept.addElement(foundSet); }
		 */
		// else
	...
	protected void skipTo(Set current, Set follow, boolean isCurrentOptionnal) {
		...
		// System.out.println("Parser - skipTo("+ targetSet + ") from " +
		// token.getValue() );
		m_automata.setDefinition(DefinitionFactory.getDefinition(
				targetSet.getTypes(), m_abcVersion));
		m_scanner.setFinaleStateAutomata(m_automata);
		// m_scanner.setFinaleStateAutomata(getAutomataFor(targetSet.getTypes()));
		// ===old
		/*
		 * while (!targetSet.contains(token.getType())) token =
		 * m_scanner.nextToken();
		 */
		try {
			...
			/*
			 * do { token = m_scanner.nextToken(); m_tokenType=token.getType();
		...
		// System.out.println("AbcHeaderParser - skipedTo : "+
		// ParserTools.convertToTypeName(token.getType()));
	}

	protected void notifyListenersForTuneBegin() {
		for (int i = 0; i < m_listeners.size(); i++)
			((TuneParserListenerInterface) m_listeners.elementAt(i))
					.tuneBegin();
	}
	...
	protected void notifyListenersForInvalidToken(Token token,
			CharStreamPosition position, TokenType expectedTokenType) {
	...
	protected void notifyListenersForInvalidCharacter(InvalidCharacterEvent evt) {
		// System.out.println("Parser - ivalid char " + evt + " for " +
		// typesForAutomata);
		for (int i = 0; i < m_listeners.size(); i++)
			((TuneParserListenerInterface) m_listeners.elementAt(i))
					.invalidCharacter(evt);
	}
