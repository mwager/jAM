...
public class TupletSpecDefinition extends AutomataDefinition {
	...
	public TupletSpecDefinition() {
		 ...
		 * state1.addTransition(new Transition(state2, ':')); State state3 = new
		 * State(AbcTokenType.TUPLET_SPEC, true); state2.addTransition(new
		 * IsDigitTransition(state3));
		 * 
		 * State state4 = new State(AbcTokenType.UNKNOWN, false);
		 * state3.addTransition(new Transition(state4, ':')); State state5 = new
		 * State(AbcTokenType.TUPLET_SPEC, true); state4.addTransition(new
		 * IsDigitTransition(state5));
		 */
