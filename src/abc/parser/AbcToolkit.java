...
/**
 * A class that provides convenience methods to manipulate abc related strings
 * and their representation using objects from the <TT>abc.notation</TT>
 * package.
 */
public class AbcToolkit {
	public static byte convertToRepeatBarLine(String barLine) {
		if (barLine.equals("[1"))
			return 1;
		else if (barLine.equals("[2"))
			return 2;
		else if (barLine.equals("|1"))
			return 1;
		else if (barLine.equals(":|2"))
			return 2;
		else
	 ...
	 *                impossible.
	 */
	public static DurationDescription getAbsoluteDurationFor(
			Fraction relativeDuration, short defaultDuration)
		...
		if ((defaultDuration != Note.HALF) && (defaultDuration != Note.QUARTER)
				&& (defaultDuration != Note.EIGHTH)
				&& (defaultDuration != Note.SIXTEENTH))
			throw new IllegalArgumentException("Invalid default note duration "
					+ defaultDuration);

		/*
		 * if (relativeDuration.getNumerator()==1) { // default note duration is
		 * divided by x absoluteDuration = (short)(defaultDuration /
		 * relativeDuration.getDenominator()); if
		 * (absoluteDuration<Note.SIXTY_FOURTH) throw new
		 * IllegalArgumentException("The relative duration " + relativeDuration
		 * + " has an absolute duration that is less than SIXTY_FOURTH " +
		 * " with a default duration equals to " + defaultDuration); } else {
		 */
		...
		if (absoluteDuration >= 2 * Note.LONG)
			throw new IllegalArgumentException("Cannot calculate the dots for "
					...
					+ defaultDuration + " : absolute note length was equal to "
					+ absoluteDuration);
		else {
		...
		// dots)
		// and remainingDurTemp contains the part that should be expressed using
		// dots.
