...
public class TuneParser extends AbcParserAbstract {
	 ...
	 * 
	 * Use {@link #TuneParser(AbcVersion)} to specify which version to use
	 */
	public TuneParser() {
	...
	public Tune parse(Reader charStream) {
		try {
			...
			try {
				...
				// notifyListenersForInvalidToken(null, new
				// CharStreamPosition(0,0,0), AbcTokenType.FIELD_NUMBER);
				// return new Tune();
			}
	...
	//
	// //jAM
	// public String parseTune(Tune tune) {
	// String tuneAsString = "";
	// System.out.println("tune.getInformation()" + tune.getInformation());
	//
	// System.out.println("tune.getReferenceNumber()" +
	// tune.getReferenceNumber());
	// tuneAsString += "X:" + tune.getReferenceNumber();
	//
	// return tuneAsString;
	// }
}
