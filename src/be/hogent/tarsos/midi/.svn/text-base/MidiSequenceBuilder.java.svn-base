...
public final class MidiSequenceBuilder {
	...
	private static final Logger LOG = Logger
			.getLogger(MidiSequenceBuilder.class.getName());
	...
	public void addNoteByFrequency(final double frequency,
			final int numberOfTicks) {
		final int closestMidiNumber = (int) Math.round(69 + 12
				* Math.log(frequency / 440.0) / Math.log(2.0));
		final double frequencyClosestMidiNumber = Math.pow(2.0,
				(closestMidiNumber - 69.0) / 12.0) * 440.0;
		final double deviationInCents = 1200
	...
	public void addNoteByAbsoluteCents(final double absoluteCents,
			final int numberOfTicks) {
		final double frequency = PitchConverter
				.absoluteCentToHertz(absoluteCents);
		this.addNoteByFrequency(frequency, numberOfTicks);
	}

	public void addNoteByDeviationInCents(final int midiKey,
			final int numberOfTicks, final double deviationInCents) {
	...
	public static MidiEvent createPitchBendEvent(final double deviationInCents,
			final int startTick) {
	 ...
	 * LSB </pre>
	 */
	public static MidiEvent createPitchBendEvent(final int bendFactor,
	...
	private MidiEvent createNoteEvent(final int nCommand, final int nKey,
			final int nVelocity, final long lTick) {
		final ShortMessage message = new ShortMessage();
		try {
