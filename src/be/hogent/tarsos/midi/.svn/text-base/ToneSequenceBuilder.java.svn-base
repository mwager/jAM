...
public final class ToneSequenceBuilder {
	 ...
	 */
	public void writeFile(final String fileName, final int smootFilterWindowSize)
		...
		if (smootFilterWindowSize > 0) {
			frequencies = PitchFunctions.medianFilter(frequencies,
		...
		for (int i = 0; i < frequencies.size(); i++) {
			...
			for (int sample = startSample; sample < floatBuffer.length; sample++) {
				...
				if (startSample + overlappingsamples > sample) {
					fadingAmplitude = fadingAmplitude * (sample - startSample)
							/ overlappingsamples;
				}
				final double fundamental = fadingAmplitude
						* Math.sin(twoPiF * time);
				// adding some harmonics makes the sound somewhat nicer
				final double firstHarmonic = fadingAmplitude / 8
						* Math.sin(twoPiF * 2 * time);
				final double secondHarmonic = fadingAmplitude / 16
						* Math.sin(twoPiF * 4 * time);
				floatBuffer[sample] = floatBuffer[sample]
						+ (float) (fundamental + firstHarmonic + secondHarmonic);

			}
			// fade out
			for (int sample = stopSample; sample < stopSample
					+ overlappingsamples
					&& sample < numberOfSamples; sample++) {
				final double time = sample / sampleRate;
				double fadingAmplitude = amplitude
						* (overlappingsamples - (sample - stopSample))
						/ overlappingsamples;
				final double fundamental = fadingAmplitude
						* Math.sin(twoPiF * time);
				// adding some harmonics makes the sound somewhat nicer
		...
		final AudioFormat audioFormat = new AudioFormat((float) sampleRate, 16,
				1, true, false);
	 ...
	 */
	public static void saveAsWav(final String csvFileName,
			final CSVFileHandler handler, final int smootFilterWindowSize) {
		final String correctedFileName = new File(csvFileName)
				.getAbsolutePath();
		try {
			final ToneSequenceBuilder builder = new ToneSequenceBuilder();
			final List<String[]> rows = FileUtils.readCSVFile(
					correctedFileName, handler.getSeparator(),
			...
			builder.writeFile(
					"data/generated_audio/"
							+ FileUtils.basename(correctedFileName) + ".wav",
					smootFilterWindowSize);
	...
	private static class BozkurtCSVFileHandler implements CSVFileHandler {
		...
		public void handleRow(final ToneSequenceBuilder builder,
				final String[] row) {
			final double realTime = Double.parseDouble(row[0]) / SAMPLERATE;
			final double frequency = REFFREQUENCY
					* Math.pow(2.0, Double.parseDouble(row[1]) / CENTSINOCTAVE);
			if (extr == null) {
				builder.addTone(frequency, realTime);
			} else {
				builder.addTone(frequency, realTime,
						extr.powerAt(realTime, true));
			}
		}
	...
	private static class IpemCSVFileHandler implements CSVFileHandler {
		...
		private static final Logger LOG = Logger
				.getLogger(IpemCSVFileHandler.class.getName());
