...
public interface AudioProcessor {
	 ...
	 */
	void processFull(final float[] audioFloatBuffer,
			final byte[] audioByteBuffer);

	/**
	 * Do the actual signal processing on an overlapping buffer. Once the first
	 * complete buffer is processed the remaining buffers are overlapping
	 * buffers and are processed using the processOverlapping method. Even if
	 * overlap is zero.
	 * 
	 * @param audioFloatBuffer
	 *            The buffer to process using the float data type.
	 ...
	 *            The buffer to process using raw bytes.
	 */
	void processOverlapping(final float[] audioFloatBuffer,
			final byte[] audioByteBuffer);
