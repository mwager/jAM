...
public final class PitchFunctions {
	 ...
	 *         remains unchanged.
	 */
	public static List<Double> convertHertzTo(final PitchUnit unit,
			final List<Double> pitchValuesInHertz) {
		final List<Double> convertedValues = new ArrayList<Double>(
				pitchValuesInHertz);
		switch (unit) {
		case ABSOLUTE_CENTS:
	 ...
	 */
	private static void convertHertzToAbsoluteCent(
			final List<Double> convertedValues) {
		for (int i = 0; i < convertedValues.size(); i++) {
			final Double valueInHertz = convertedValues.get(i);
			convertedValues.set(i,
					PitchConverter.hertzToAbsoluteCent(valueInHertz));
		}
	}

	private static void convertHertzToMidiCent(
			final List<Double> convertedValues) {
		for (int i = 0; i < convertedValues.size(); i++) {
			final Double valueInHertz = convertedValues.get(i);
			convertedValues
	...
	private static void convertHertzToMidiKey(final List<Double> convertedValues) {
		for (int i = 0; i < convertedValues.size(); i++) {
			final Double valueInHertz = convertedValues.get(i);
			convertedValues.set(i,
					(double) PitchConverter.hertzToMidiKey(valueInHertz));
		}
	}
	 ...
	 */
	private static void convertHertzToRelativeCent(
			final List<Double> convertedValues) {
		for (int i = 0; i < convertedValues.size(); i++) {
			final Double hertzValue = convertedValues.get(i);
			final Double pitchValueInCentFoldedToOneOctave = PitchConverter
	 ...
	 */
	public static void bandwithFilter(final List<Double> pitchValuesInHertz,
			final double minValueInHertz, final double maxValueInHertz) {
	 ...
	 */
	public static List<Double> medianFilter(final List<Double> listToFilter,
			final int n) {

		if (n % 2 == 0) {
			throw new IllegalArgumentException(
					"Medianfilter not implemented for even n values");
		}
		...
		for (int i = numberOfZeroesToAddBefore; i < listToFilter.size()
			...
			List<Double> sublist = new ArrayList<Double>(listToFilter.subList(i
					- n / 2, i + n / 2 + 1));
	 ...
	 * @return
	 */
	public static List<Double> getGaussianSmoothed(
			final List<Double> listToSmooth, final double standardDeviation) {
	...
	public static List<Double> gaussianFilter(final List<Double> listToFilter,
		...
		for (int i = windowSize / 2; i < listToFilter.size() - windowSize / 2; i++) {
			...
			for (int j = 0; j < windowSize; j++) {
				final double weight = Math.pow(Math.E, -1
						* (j - windowSize / 2.0) * (j - windowSize / 2.0) / 2.0
						* parameter);
	 ...
	 */
	public static Histogram createFrequencyTable(final List<Double> values,
			final double classWidth, final double start, final double stop) {
		final Histogram histogram = new Histogram(start, stop,
	...
	public static void exportFrequencyTable(final Histogram histogram,
			final String fileName, final double start, final double stop) {
		...
		for (double current = start + histogram.getClassWidth() / 2; current <= stop; current += histogram
			...
			final double derivative = current + histogram.getClassWidth() > stop ? 0
			...
			sb.append(current).append(";").append(count).append(";")
					.append(cumFreq).append(";").append(derivative).append(";")
					.append(psd).append("\n");
		}
		...
		try {
			...
			ImageIO.write(image, "png",
					new File(fileName.substring(0, fileName.length() - 4)
							+ ".png"));
	...
	public static Histogram readFrequencyTable(final String fileName) {
		...
		final double classWidth = Double.parseDouble(data.get(1)[0])
				- Double.parseDouble(data.get(0)[0]);
		final double start = Double.parseDouble(data.get(0)[0]) - classWidth
				/ 2.0;
		final double stop = Double.parseDouble(data.get(data.size() - 1)[0])
				+ classWidth / 2.0;

		final Histogram table = new Histogram(start, stop,
				(int) ((stop - start) / classWidth));
