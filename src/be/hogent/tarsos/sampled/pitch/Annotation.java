...
public final class Annotation implements Comparable<Annotation> {
	...
	public Annotation(final double timeStamp, final double pitchInHz,
		...
		if (pitchInHz <= 0) {
			throw new IllegalArgumentException(
					"The pitch in Hz should be a value above zero, it is: "
							+ pitchInHz);
		}
		if (timeStamp < 0) {
			throw new IllegalArgumentException(
					"The timestamp in seconds should be equal or above zero, it is: "
							+ timeStamp);
		}
	...
	public int compareTo(final Annotation o) {
		// start time first
		final int startCompare = Double.valueOf(start).compareTo(
				Double.valueOf(o.start));
		final int compareValue;
		// then order by source name
		if (startCompare == 0) {
			compareValue = getSource().toString().compareTo(
					o.getSource().toString());
		} else {
	...
	public boolean equals(final Object o) {
		...
		if (o != null && o instanceof Annotation) {
			...
			isEqual = start == sample.start
					&& getSource().equals(sample.getSource());
		}
		return isEqual;
	}
	...
	public String toString() {
		return String.format(Locale.US, "%.5f,%.5f,%.5f,%s", start,
				getPitch(PitchUnit.HERTZ), probability, source);
	}
	...
	public static PitchHistogram pitchHistogram(
	 ...
	 */
	public static double percentAnnotated(final List<Annotation> annotations,
			final double audioLenght) {
		double percentage;
