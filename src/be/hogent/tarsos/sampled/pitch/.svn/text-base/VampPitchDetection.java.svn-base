...
public final class VampPitchDetection implements PitchDetector {
	...
	private static final Logger LOG = Logger.getLogger(VampPitchDetection.class
			.getName());

	public VampPitchDetection(final AudioFile audioFile,
	...
	private void copyDefaultSettings() {
		...
		String fileName = FileUtils.combine(FileUtils.temporaryDirectory(),
				setting);
		if (!FileUtils.exists(fileName)) {
			FileUtils.copyFileFromJar(
					"/be/hogent/tarsos/sampled/pitch/resources/" + setting,
					fileName);
			LOG.info(String.format("Copied % from jar file to %s .", setting,
					fileName));
		}
	}
	...
	public List<Annotation> executePitchDetection() {
		String setting = mode.getParametername() + ".n3";
		final String settingsFile = FileUtils.combine(
				FileUtils.temporaryDirectory(), setting);
		final String command = String
				.format("sonic-annotator -t %s  %s -w csv --csv-one-file h --csv-basedir %s --csv-force",
						settingsFile, file.transcodedPath(),
						file.transcodedDirectory());
		...
		if (mode == PitchDetectionMode.VAMP_MAZURKA_PITCH) {
			...
			csvFile = FileUtils.combine(file.transcodedDirectory(),
					FileUtils.basename(file.transcodedPath())
		...
		if (new File(csvFile).delete()) {
			...
			LOG.fine(String.format(
					"Failed to deleted intermediate CSV file %s", csvFile));
