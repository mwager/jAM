...
public final class SignalPowerExtractor {
	...
	private static final Logger LOG = Logger
			.getLogger(SignalPowerExtractor.class.getName());
		...
		try {
			...
			try {
				...
				LOG.log(Level.SEVERE,
						"Failed to initialize audio input stream.", e);
			}
		}
	...
	/*private void extractPower() {
		...
		try {
			ais = AudioSystem.getAudioInputStream(inputFile);
			final AudioFloatInputStream afis = AudioFloatInputStream
					.getInputStream(ais);
	...
	public void waveFormPlot(WaveFormDataAggregator aggregator) {
		...
		try {
			...
			try {
				...
				LOG.log(Level.SEVERE,
						"Failed to initialize audio file stream.", e);
			}
		}
	}
	...
	public void saveTextFile(final String textFileName, final boolean relative) {
		...
		for (int index = 0; index < linearPowerArray.length; index++) {
			...
			stringBuilder.append(seconds).append(";")
					.append(powerAt(seconds, relative)).append("\n");
		}
	 ...
	 */
	public void savePowerPlot(final String powerPlotFileName,
			final double silenceTreshold) {
		if (linearPowerArray == null) {
			extractPower();
		}

		final SimplePlot plot = new SimplePlot("Powerplot for "
				+ audioFile.basename());
		for (int index = 0; index < linearPowerArray.length; index++) {
