...
/**
 * A representation of a scala file. See the <a
 * href="http://www.huygens-fokker.org/scala/scl_format.html"> Scala scale file
 * format</a>.
 * 
 * @author Joren Six
 */
public final class ScalaFile {
	 ...
	 * Log messages.
	 */
	private static final Logger LOG = Logger.getLogger(ScalaFile.class
			.getName());
	...
	public ScalaFile(final String desc, final double[] notes,
		...
		if (notes == null) {
			throw new IllegalArgumentException(
					"The notes (pitches) should be defined");
		} else if (names != null && names.length != notes.length) {
			throw new AssertionError(
					"Notes and names should have the same length.");
		}
		description = desc;
		pitches = notes.clone();
		if (names == null) {
	...
	public ScalaFile(final String scalaFile) {
		...
		for (final String line : lines) {
			...
			if (numberOfDataLines == 1) {
				...
				final boolean isValidRatio = line
						.matches("\\s*[0-9]+(|/[0-9]+).*");
				final boolean isValidCent = line
						.matches("\\s*(-|\\+)?[0-9]+\\.[0-9]*.*");
	...
	private double parsePitch(final String row) {
		...
		if (row.contains("/") || !row.contains(".")) {
			...
			final double absCentDenom = PitchConverter
					.hertzToAbsoluteCent(denominator);
			final double absCentQuotient = PitchConverter
					.hertzToAbsoluteCent(quotient);
	...
	public void write(final String scalaFile) {
		if (pitches.length > 0) {
			...
			contents.append("! ").append(FileUtils.basename(scalaFile))
					.append(".scl \n");
			contents.append("!\n");
			...
			LOG.warning("No pitches defined, file: " + scalaFile
					+ " not created.");
		}
	}
	...
	public static ScalaFile westernTuning() {
		final double[] notes = { 0, 100, 200, 300, 400, 500, 600, 700, 800,
				900, 1000, 1100, };
		final String[] names = { "C", "C#", "D", "D#", "E", "F", "F#", "G",
				"G#", "A", "A#", "B" };
