...
public final class PitchClassHistogram extends Histogram {
	private static final Logger LOG = Logger
			.getLogger(PitchClassHistogram.class.getName());
	 ...
	 * program.</bufferCount>
	 */
	public void exportToScalaScaleFileFormat(final String fileName,
	 ...
	 * @param peaks
	 */
	public static void exportPeaksToScalaFileFormat(final String fileName,
			final String toneScaleName, final List<Peak> peaks) {
		if (peaks.isEmpty()) {
	...
	public static PitchClassHistogram createToneScale(final double[] peaks,
			final double[] heights, final double[] widths) {
	...
	public static PitchClassHistogram createToneScale(final double[] peaks,
		...
		for (final Double key : unWrappedHistogram.keySet()) {
			...
			for (int i = 0; i < peaks.length; i++) {
				...
				final double power = Math.pow(difference
	 ...
	 */
	public static PitchClassHistogram createToneScaleHistogram(
			final List<Annotation> annotations, final double width) {
	...
	public static void addAnnotationTo(final double[] values,
			final Annotation annotation, final double kernelWidth,
	...
	public boolean isMelodic() {
		...
		final PitchClassHistogram smoothed = (PitchClassHistogram) new PitchClassHistogram()
		...
		final double peakiness = fittingHistogram.correlation(this,
