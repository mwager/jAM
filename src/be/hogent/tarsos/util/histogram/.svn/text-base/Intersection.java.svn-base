...
public final class Intersection implements HistogramCorrelation {
	public double correlation(final Histogram thisHistogram,
			final int displacement, final Histogram otherHistogram) {
		...
		if (actualDisplacement < 0) {
			actualDisplacement = (actualDisplacement % numberOfClasses + numberOfClasses)
					% numberOfClasses;
		}
		...
		for (double current = start + classWidth / 2; current <= stop; current += classWidth) {
			...
			displacedValue = (current + actualDisplacement * classWidth)
		...
		final double biggestHistogramArea = Math.max(
				thisHistogram.getSumFreq(), otherHistogram.getSumFreq());
	...
	public void plotCorrelation(final Histogram thisHistogram,
			final int displacement, final Histogram otherHistogram,
		...
		for (double current = start + classWidth / 2; current <= stop; current += classWidth) {
			final double displacedValue = (current + actualDisplacement
					* classWidth)
					% (numberOfClasses * classWidth);
