...
public final class PeakDetector {
	...
	public static List<Peak> detect(final Histogram histogram,
		...
		// 1. CALCULATE SCORES
		final double[] localHeightScores = new double[histogram
				.getNumberOfClasses()];
		for (int i = 0; i < histogram.getNumberOfClasses(); i++) {
			...
			if (isPeak) {
				localHeightScores[i] = calculateLocalHeightScore(histogram, i,
						windowSize);
			}
		}
	 ...
	 */
	public static double calculateLocalHeightScore(final Histogram histogram,
			final int index, final int windowSize) {
		...
		final double standardDeviation = Math.pow(
				StatUtils.variance(heightRange, mean), 0.5);
		...
		if (standardDeviation == 0.0) {
			...
			localHeightScore = (histogram.getCountForClass(index) - mean)
					/ standardDeviation;
		}
	 ...
	 */
	public static List<Peak> detectNumberOfPeaks(final Histogram histogram,
	...
	public static List<Peak> detect(final Histogram histogram) {
		...
		Collections.sort(entryList,
				new Comparator<Map.Entry<Integer, Integer>>() {
					public int compare(
							final Entry<Integer, Integer> firstEntry,
