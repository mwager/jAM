...
public class NoteCollectorWorker implements Runnable {
	...
	private float YIN_TRESHOLD = 0.13f, // TODO das hier muss immer noch mit GUI übereinstimen !!!
				  MPM_TRESHOLD = 0.93f;
	
	private int MINIMUM_DURATION = 50; // TODO das hier muss immer noch mit GUI übereinstimen !!!
...
//		}
		 ...
		 * beim Nächsten Mal ignoriert werden wenn dies pausen entspricht !!!
		...
		if ((ONSET && midiKey > 0  && !silence /* level > MINIMUM_LEVEL*/) && (midiKey != lastTakenMidiKey)) { // es kommt ne andere /Note/
			...
			if (newNoteCount > (int) (MINIMUM_DURATION / duration)) { // 60/25.01 --> 2 also mind.  3  nehmen! wie  sonst auch
				ONSET = false; // dann macht er jetzt unten ne Entscheidung und beim next Mal fängt er an die neue note zu collecten
...
//					
					...
					// wir können davon ausgehen dass "note" die lastTakenNote
					// wird, da sie alle Bedingungen fuer ein note-OFFSET erfüllt
			 ...
			 * es müssen die Pausen gezählt werden!
...
//				//ignoriere extremwerte am Ende, denn dies kommt sehr oft vor! also nur die ersten X%
	...
	private float getBestPitch(float[] audioFloatBuffer) {
		...
		// if(midiKeyYIN==midiKeyMPM) { // --> wird am Häufigsten eintreten !
		// yin_cnt++;
...
//			System.out.println("====>" + midiKey);
		 ...
		 * noch ignoriert!
		 */
	// wenn letzte und diese unabghängig von #/b DIESELBE IST:
