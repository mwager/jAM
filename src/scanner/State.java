...
public class State implements Cloneable {
	 ...
	 * @param isTokenStateValue
	 *            <TT>true</TT> if the state is a token state, <TT>false</TT>
	 *            otherwise.
	 */
	...
	public Transition[] getTransitions() {
		if (m_transitionsArrayCache == null) {
			...
			for (int i = 0; i < size; i++)
				m_transitionsArrayCache[i] = (Transition) transitions
						.elementAt(i);
		}
	 ...
	 *         <TT>null</TT> is returned if no transition is specified from this
	 *         state for the given character.
	 */
	public Transition getTransitionFor(char character) {
		...
		while (matchingTransition == null && transitionIndex < m_transitionsNb) {
			if (((Transition) transitions.elementAt(transitionIndex))
					.isPossible(character))
				matchingTransition = (Transition) transitions
						.elementAt(transitionIndex);
			else
				transitionIndex++;
		}
	 ...
	 * transition is a transition whose target state is equals to the source
	 * state.
	 * 
	 ...
	 * 
	 * @param isToken
	 *            <TT>true</TT> if this state is a token state, <TT>false</TT>
	 *            otherwise.
	 */
	 ...
	 * 
	 * @return <TT>true</TT> if this state is a token state, <TT>false</TT>
	 *         otherwise.
	 */
	 ...
	 * 
	 * @param transition
	 *            The transition to be added to this state.
	...
	/**
	 * Performs an union between this state and the given one. Union between a
	 * token state and a non token state will give a token state. Union between
	 * 2 non token states will give a non token state. Union between 2 token
	 * states is possible only if only one of them has self transitions. Union
	 * between 2 token states that both have self transitions will throw a
	 * runtime exception. Union between 2 token states that both haven't any
	 * self transition will return the state given as paremeter
	 * 
	 * @param state
	 *            a State
	...
	public State union(State state) {
		if (isTokenState() && state.isTokenState()) {
			...
			// Should be there's but this is due to conflicts between token
			// types.
			...
			if (state.getType().equals(AbcTokenType.TEXT))
				return this;
			/*
			 * if (getType().equals(AbcTokenType.KEY_ACCIDENTAL)) return this;
			 * if (state.getType().equals(AbcTokenType.KEY_ACCIDENTAL)) return
			 * state;
			 */
		}
		if (isTokenState() && state.isTokenState())
			if (hasSelfTransitions() && state.hasSelfTransitions())
				throw new RuntimeException("BOTH ARE TOKENS ! : "
						+ this.getType() + " and " + state.getType());
			else if (!hasSelfTransitions() && !state.hasSelfTransitions()) {
				// System.err.println("warning : ambiguity between " +
				// this.getType() + " and "
		 ...
		 * ||(!hasSelfTransitions() && !state.hasSelfTransitions()) ) ) throw
		 * new RuntimeException("BOTH ARE TOKENS ! : " + this.getType() +
		 * " and " + state.getType());
		 */
		...
		for (int i = 0; i < targetTransitions.length; i++) {
			...
			currentTargetTransition = new Transition(
					currentTargetTransition.getTargetState(),
					currentTargetTransition.getChars());
			Transition[] returnedStateTransitions = returnedState
					.getTransitions();
			for (int j = 0; j < returnedStateTransitions.length; j++) {
				...
				if (currentTransition.isSelfTransition()
						&& currentTargetTransition.isSelfTransition())
					...
					char[] intersection = currentTransition
							.intersect(currentTargetTransition.getChars());
					...
					if (intersection.length != 0) {
						...
						State unionState = currentTransition
								.getTargetState()
								.union(currentTargetTransition.getTargetState());
						Transition newtransition = new Transition(unionState,
								intersection);
		...
		if (isTokenState()) {
			if (state.isTokenState()) {
				// both are tokens.
				// System.out.println("============================TWO TOKENS :"
				// + this + "  " + state);
				if (hasSelfTransition)
					returnedState.setType(state.getType());
				// we cannot have both with self transitions.
			} else {
				// this is the only token =>return should have is name=> already
				// the case with this .clone()
			}
		} else {
			if (state.isTokenState()) {
	...
	/**
	 * Convert the given self transition to a transition to a new state (a clone
	 * of this one).
	 * 
	 * @param selfTransition
	 *            The self transition to be converted.
	 */
	private void convertSelfTransition(Transition selfTransition) {
		if (selfTransition.isSelfTransition()
				&& selfTransition.getSourceState().equals(this)) {
			removeTransition(selfTransition);
			State tempState = (State) clone();
			Transition transition = new Transition(tempState,
					selfTransition.getChars());
	...
	public Object clone() {
		...
		int size = transitions.size();
		for (int i = 0; i < size; i++) {
			Transition currentTransition = (Transition) transitions
					.elementAt(i);
			// if (currentTransition.getTargetState().equals(this))
			if (currentTransition.isSelfTransition()) {
				Transition trans = new Transition(state,
						currentTransition.getChars());
				state.addTransition(trans);
			} else {
				Transition trans = new Transition(
						currentTransition.getTargetState(),
						currentTransition.getChars());
				state.addTransition(trans);
			}
		}
